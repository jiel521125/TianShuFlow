...
COMPOSE_CMD="docker compose -p tian-shu-dev -f docker-compose-dev.yaml"
...
detect_sandbox_mode() {
    ...
    if [[ "$sandbox_use" == *"tianshu.sandbox.local:LocalSandboxProvider"* ]]; then
        echo "local"
    elif [[ "$sandbox_use" == *"tianshu.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
init() {
    echo "=========================================="
    echo "  TianShu Init — Pull Sandbox Image"
    echo "=========================================="
    echo ""
...
start() {
    ...
    echo "=========================================="
    echo "  Starting TianShu Docker Development"
    echo "=========================================="
    echo ""
    ...
    if [ "$sandbox_mode" = "aio" ]; then
        local docker_socket="${TIAN_SHU_DOCKER_SOCKET:-/var/run/docker.sock}"
    ...
    # Set TIAN_SHU_ROOT for provisioner if not already set
    if [ -z "$TIAN_SHU_ROOT" ]; then
        export TIAN_SHU_ROOT="$PROJECT_ROOT"
        echo -e "${BLUE}Setting TIAN_SHU_ROOT=$TIAN_SHU_ROOT${NC}"
        echo ""
    fi
    ...
    if [ ! -f "$PROJECT_ROOT/config.yaml" ]; then
        if [ -f "$PROJECT_ROOT/config.example.yaml" ]; then
            ...
            echo -e "${YELLOW}  configuration before starting TianShu.                  ${NC}"
            echo -e "${YELLOW}============================================================${NC}"
            echo ""
    ...
    echo ""
    echo "=========================================="
    echo "  TianShu Docker is starting!"
    echo "=========================================="
    echo ""
...
stop() {
    # TIAN_SHU_ROOT is referenced in docker-compose-dev.yaml; set it before
    ...
    "$SCRIPT_DIR/cleanup-containers.sh" tian-shu-sandbox 2>/dev/null || true
...
restart() {
    echo "========================================"
    echo "  Restarting TianShu Docker Services"
    echo "========================================"
    echo ""
...
# Show help
help() {
    echo "TianShu Docker Management Script"
    echo ""
