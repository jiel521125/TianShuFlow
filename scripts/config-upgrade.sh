...
if [ -n "$TIAN_SHU_CONFIG_PATH" ] && [ -f "$TIAN_SHU_CONFIG_PATH" ]; then
    CONFIG="$TIAN_SHU_CONFIG_PATH"
...
MIGRATIONS = {
    1: {
        'description': 'Rename src.* module paths to tianshu.*',
        'replacements': [
            ('src.community.', 'tianshu.community.'),
            ('src.sandbox.', 'tianshu.sandbox.'),
            ('src.models.', 'tianshu.models.'),
            ('src.tools.', 'tianshu.tools.'),
        ],
    },
