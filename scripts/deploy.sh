#!/usr/bin/env bash
#
# deploy.sh - Build, start, or stop TianShu production services
#
# Commands:
...
if [ -f "$ENV_FILE" ]; then
    COMPOSE_CMD=(docker compose --env-file "$ENV_FILE" -p tian-shu -f "$DOCKER_DIR/docker-compose.yaml")
else
    COMPOSE_CMD=(docker compose -p tian-shu -f "$DOCKER_DIR/docker-compose.yaml")
fi
...
if [ -z "$TIAN_SHU_HOME" ]; then
    export TIAN_SHU_HOME="$REPO_ROOT/backend/.tian-shu"
fi
echo -e "${BLUE}TIAN_SHU_HOME=$TIAN_SHU_HOME${NC}"
mkdir -p "$TIAN_SHU_HOME"

# ── TIAN_SHU_REPO_ROOT (for skills host path in DooD) ───────────────────────

export TIAN_SHU_REPO_ROOT="$REPO_ROOT"

# ── config.yaml ───────────────────────────────────────────────────────────────

if [ -z "$TIAN_SHU_CONFIG_PATH" ]; then
    export TIAN_SHU_CONFIG_PATH="$REPO_ROOT/config.yaml"
fi

if  [ "$CMD" != "down" ] && [ ! -f "$TIAN_SHU_CONFIG_PATH" ]; then
    # Try to seed from repo (config.example.yaml is the canonical template)
    if [ -f "$REPO_ROOT/config.example.yaml" ]; then
        cp "$REPO_ROOT/config.example.yaml" "$TIAN_SHU_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded config.example.yaml → $TIAN_SHU_CONFIG_PATH${NC}"
        echo -e "${YELLOW}⚠ config.yaml was seeded from the example template.${NC}"
        echo "  Run 'make setup' to generate a minimal config, or edit $TIAN_SHU_CONFIG_PATH manually before use."
    else
    ...
    echo -e "${GREEN}✓ config.yaml: $TIAN_SHU_CONFIG_PATH${NC}"
fi

# ── extensions_config.json ───────────────────────────────────────────────────

if [ -z "$TIAN_SHU_EXTENSIONS_CONFIG_PATH" ]; then
    export TIAN_SHU_EXTENSIONS_CONFIG_PATH="$REPO_ROOT/extensions_config.json"
fi

if [ ! -f "$TIAN_SHU_EXTENSIONS_CONFIG_PATH" ]; then
    if [ -f "$REPO_ROOT/extensions_config.json" ]; then
        cp "$REPO_ROOT/extensions_config.json" "$TIAN_SHU_EXTENSIONS_CONFIG_PATH"
        echo -e "${GREEN}✓ Seeded extensions_config.json → $TIAN_SHU_EXTENSIONS_CONFIG_PATH${NC}"
    else
        # Create a minimal empty config so the gateway doesn't fail on startup
        echo '{"mcpServers":{},"skills":{}}' > "$TIAN_SHU_EXTENSIONS_CONFIG_PATH"
        echo -e "${YELLOW}⚠ extensions_config.json not found, created empty config at $TIAN_SHU_EXTENSIONS_CONFIG_PATH${NC}"
    fi
else
    echo -e "${GREEN}✓ extensions_config.json: $TIAN_SHU_EXTENSIONS_CONFIG_PATH${NC}"
fi
...
_secret_file="$TIAN_SHU_HOME/.better-auth-secret"
...
_internal_auth_token_file="$TIAN_SHU_HOME/.internal-auth-token"
if  [ "$CMD" != "down" ] && [ -z "$TIAN_SHU_INTERNAL_AUTH_TOKEN" ]; then
    if [ -f "$_internal_auth_token_file" ]; then
        export TIAN_SHU_INTERNAL_AUTH_TOKEN
        TIAN_SHU_INTERNAL_AUTH_TOKEN="$(cat "$_internal_auth_token_file")"
        echo -e "${GREEN}✓ TIAN_SHU_INTERNAL_AUTH_TOKEN loaded from $_internal_auth_token_file${NC}"
    else
        export TIAN_SHU_INTERNAL_AUTH_TOKEN
        if command -v python3 > /dev/null 2>&1 && \
            TIAN_SHU_INTERNAL_AUTH_TOKEN="$(python3 -c 'import sys; sys.version_info >= (3, 6) or sys.exit(1); import secrets; print(secrets.token_urlsafe(32))' 2>/dev/null)"; then
            true
        elif command -v python > /dev/null 2>&1 && \
            TIAN_SHU_INTERNAL_AUTH_TOKEN="$(python -c 'import sys; sys.version_info >= (3, 6) or sys.exit(1); import secrets; print(secrets.token_urlsafe(32))' 2>/dev/null)"; then
            true
        elif command -v openssl > /dev/null 2>&1 && \
            TIAN_SHU_INTERNAL_AUTH_TOKEN="$(openssl rand -hex 32)"; then
            true
        else
            echo -e "${RED}✗ Cannot generate TIAN_SHU_INTERNAL_AUTH_TOKEN: python3, python, and openssl are all unavailable.${NC}" >&2
            echo -e "${RED}  Set TIAN_SHU_INTERNAL_AUTH_TOKEN manually before running make up.${NC}" >&2
            exit 1
        fi
        echo "$TIAN_SHU_INTERNAL_AUTH_TOKEN" > "$_internal_auth_token_file"
        chmod 600 "$_internal_auth_token_file"
        echo -e "${GREEN}✓ TIAN_SHU_INTERNAL_AUTH_TOKEN generated → $_internal_auth_token_file${NC}"
    fi
fi
...
detect_sandbox_mode() {
    ...
    [ -f "$TIAN_SHU_CONFIG_PATH" ] || { echo "local"; return; }
    ...
    if [[ "$sandbox_use" == *"tianshu.community.aio_sandbox:AioSandboxProvider"* ]]; then
...
if [ "$CMD" = "down" ]; then
    ...
    export TIAN_SHU_HOME="${TIAN_SHU_HOME:-$REPO_ROOT/backend/.tian-shu}"
    export TIAN_SHU_CONFIG_PATH="${TIAN_SHU_CONFIG_PATH:-$TIAN_SHU_HOME/config.yaml}"
    export TIAN_SHU_EXTENSIONS_CONFIG_PATH="${TIAN_SHU_EXTENSIONS_CONFIG_PATH:-$TIAN_SHU_HOME/extensions_config.json}"
    export TIAN_SHU_REPO_ROOT="${TIAN_SHU_REPO_ROOT:-$REPO_ROOT}"
    export BETTER_AUTH_SECRET="${BETTER_AUTH_SECRET:-placeholder}"
    export TIAN_SHU_INTERNAL_AUTH_TOKEN="${TIAN_SHU_INTERNAL_AUTH_TOKEN:-placeholder}"
    "${COMPOSE_CMD[@]}" down
    exit 0
fi
...
echo "=========================================="
echo "  TianShu Production Deployment"
echo "=========================================="
echo ""
...
# ── TIAN_SHU_DOCKER_SOCKET (aio / pure-DooD mode only) ──────────────────────
...
if [ -z "$TIAN_SHU_DOCKER_SOCKET" ]; then
    export TIAN_SHU_DOCKER_SOCKET="/var/run/docker.sock"
fi

if [ "$sandbox_mode" = "aio" ]; then
    if [ ! -S "$TIAN_SHU_DOCKER_SOCKET" ]; then
        echo -e "${RED}⚠ Docker socket not found at $TIAN_SHU_DOCKER_SOCKET${NC}"
        echo "  AioSandboxProvider (DooD) will not work."
        exit 1
    fi
    echo -e "${GREEN}✓ Docker socket: $TIAN_SHU_DOCKER_SOCKET${NC}"
