#!/usr/bin/env bash
#
# serve.sh — Unified TianShu service launcher
#
# Usage:
...
# Every tian-shu worktree (the main checkout + each linked worktree) hardcodes
...
# TIANSHU_ROOTS is that set of roots; processes living outside all of them
...
# True if PID has an open file/cwd under any tian-shu worktree root. The
# trailing slash keeps a sibling dir like ".../tian-shu-notes" from matching
# the ".../tian-shu" root.
_is_tianshu_pid() {
    local pid=$1 files root

    # Daemon children inherit TIANSHU_DAEMON_ROOT from run_service. Checking
    ...
    if [ -r "/proc/$pid/environ" ] &&
        tr '\0' '\n' < "/proc/$pid/environ" 2>/dev/null | grep -Fxq "TIANSHU_DAEMON_ROOT=$REPO_ROOT"; then
        return 0
    fi
...
_kill_repo_processes() {
    ...
    while IFS= read -r pid; do
        if [ -n "$pid" ] && _is_tianshu_pid "$pid"; then
            case " $pids " in
                *" $pid "*) ;;
                *) pids="$pids $pid" ;;
            esac
...
stop_all() {
    ...
    # so a lingering nginx (or any tian-shu process) that _kill_repo_nginx did
    ...
    ./scripts/cleanup-containers.sh tian-shu-sandbox 2>/dev/null || true
...
if ! TIANSHU_PNPM_PYTHON="$(_pick_python)"; then
    echo "Python 3 is required to run pnpm."
    exit 1
fi
TIANSHU_PNPM_RUNNER="$REPO_ROOT/scripts/pnpm.py"
export TIANSHU_PNPM_PYTHON TIANSHU_PNPM_RUNNER

# Frontend command
if $DEV_MODE; then
    FRONTEND_CMD='"$TIANSHU_PNPM_PYTHON" "$TIANSHU_PNPM_RUNNER" run dev'
else
    FRONTEND_CMD="env BETTER_AUTH_SECRET=$($TIANSHU_PNPM_PYTHON -c 'import secrets; print(secrets.token_hex(16))') \"\$TIANSHU_PNPM_PYTHON\" \"\$TIANSHU_PNPM_RUNNER\" run preview"
fi

# Runtime path defaults. Local `make dev` launches Gateway from `backend/`,
# so pin TianShu-owned state to the expected backend runtime directory and
# create it before uvicorn builds its reload exclude filter.
if [ -z "$TIAN_SHU_PROJECT_ROOT" ]; then
    export TIAN_SHU_PROJECT_ROOT="$REPO_ROOT"
fi

BACKEND_RUNTIME_HOME="$REPO_ROOT/backend/.tian-shu"
if [ -z "$TIAN_SHU_HOME" ]; then
    export TIAN_SHU_HOME="$BACKEND_RUNTIME_HOME"
fi
...
mkdir -p "$TIAN_SHU_HOME" "$BACKEND_RUNTIME_HOME" "$REPO_ROOT/backend/sandbox"
TIAN_SHU_HOME="$(cd "$TIAN_SHU_HOME" && pwd -P)"
BACKEND_RUNTIME_HOME="$(cd "$BACKEND_RUNTIME_HOME" && pwd -P)"
export TIAN_SHU_HOME

# Extra flags for uvicorn
if $DEV_MODE && ! $DAEMON_MODE; then
    GATEWAY_EXTRA_FLAGS="--reload --reload-include='*.yaml' --reload-include='.env' --reload-exclude='*.pyc' --reload-exclude='__pycache__' --reload-exclude='$REPO_ROOT/backend/sandbox' --reload-exclude='$TIAN_SHU_HOME' --reload-exclude='$BACKEND_RUNTIME_HOME'"
else
    GATEWAY_EXTRA_FLAGS=""
fi
...
if ! { \
        [ -n "$TIAN_SHU_CONFIG_PATH" ] && [ -f "$TIAN_SHU_CONFIG_PATH" ] || \
        [ -f backend/config.yaml ] || \
        [ -f config.yaml ]; \
    }; then
    echo "✗ No TianShu config file found."
...
if ! $SKIP_INSTALL; then
    ...
    # `--all-packages` propagates extras into workspace members (tianshu-harness
    ...
    (cd frontend && "$TIANSHU_PNPM_PYTHON" "$TIANSHU_PNPM_RUNNER" install --silent) || { echo "✗ Frontend dependency install failed"; exit 1; }
...
run_service() {
    ...
    if $DAEMON_MODE; then
        ...
        # carries TIANSHU_DAEMON_ROOT in its environment, letting
        # _is_tianshu_pid recognize it at stop time.
        nohup env TIANSHU_DAEMON_ROOT="$REPO_ROOT" sh -c "$cmd" > /dev/null 2>&1 &
    else
        sh -c "$cmd" &
    fi
...
echo ""
echo "=========================================="
echo "  ✓ TianShu is running!  [$MODE_LABEL]"
echo "=========================================="
echo ""
