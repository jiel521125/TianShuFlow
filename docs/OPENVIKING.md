...
TianShu can use a remote OpenViking server as an optional long-term memory
...
- Hard isolation by hashing each TianShu `(user_id, agent_name)` scope into a
  separate OpenViking trusted user identity.
- Local bounded message watermarks under TianShu's runtime home. An ordered
...
TianShu passes trusted `X-OpenViking-Account` and
...
For a locally installed TianShu process, use
`http://127.0.0.1:1933`. For a TianShu container connecting to OpenViking on
...
Create the standard TianShu local files if they no longer exist:
...
Set at least the normal TianShu secrets in `.env`, including
...
```bash
export TIAN_SHU_CONFIG_PATH="$PWD/config.yaml"
export TIAN_SHU_EXTENSIONS_CONFIG_PATH="$PWD/extensions_config.json"
export TIAN_SHU_HOME="$PWD/backend/.tian-shu"
export TIAN_SHU_REPO_ROOT="$PWD"
```
...
```bash
docker exec -it tian-shu-openviking openviking-server init
```
...
```bash
docker exec -it tian-shu-openviking openviking-server doctor
docker restart tian-shu-openviking
curl http://localhost:1933/health
```

Then start TianShu with the same overlay:
...
Open TianShu at <http://localhost:2026> and OpenViking Studio at
...
Set `base_url: http://127.0.0.1:1933`, then start TianShu normally:
...
The TianShu entrypoint is <http://localhost:2026>.

## Failure behavior

- Invalid backend configuration fails loudly; TianShu never silently writes
  to DeerMem instead.
...
- Once a message batch is accepted, TianShu persists a submitted-message
