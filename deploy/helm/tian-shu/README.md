...
Deploys the full TianShu stack to Kubernetes: **gateway** (backend + embedded
...
- The three TianShu images — either the published ones (see "Install the
...
```bash
helm install tian-shu oci://ghcr.io/<owner>/charts/tian-shu \
  --version <version> \
  -n tian-shu --create-namespace \
  -f my-values.yaml
```
...
> It publishes to `oci://ghcr.io/<owner>/charts/tian-shu` (the `charts/` prefix
> keeps it distinct from the `tian-shu-{backend,frontend,provisioner}` image
> packages).
...
```yaml
image:
  registry: ghcr.io/<owner>     # owner prefix; images are <owner>/tian-shu-<name>
...
already match the published image names (`tian-shu-backend`,
`tian-shu-frontend`, `tian-shu-provisioner`), so only `registry` and `tag`
...
docker build -t $REGISTRY/tian-shu-backend:$TAG --build-arg UV_EXTRAS=postgres -f backend/Dockerfile .
# frontend
docker build -t $REGISTRY/tian-shu-frontend:$TAG -f frontend/Dockerfile .
# provisioner
docker build -t $REGISTRY/tian-shu-provisioner:$TAG -f docker/provisioner/Dockerfile docker/provisioner

docker push $REGISTRY/tian-shu-backend:$TAG
docker push $REGISTRY/tian-shu-frontend:$TAG
docker push $REGISTRY/tian-shu-provisioner:$TAG
```
...
ingress:
  enabled: true
  className: nginx
  host: tian-shu.example.com
  tls:
    enabled: true
    secretName: tian-shu-tls
...
config: |
  ...
  stream_bridge:
    type: redis   # cross-pod SSE; URL from TIAN_SHU_STREAM_BRIDGE_REDIS_URL
  ...
  tools:
    - name: web_search
      group: web
      use: tianshu.community.ddg_search.tools:web_search_tool
      max_results: 5
    - name: web_fetch
      group: web
      use: tianshu.community.jina_ai.tools:web_fetch_tool
      timeout: 10
    - name: image_search
      group: web
      use: tianshu.community.image_search.tools:image_search_tool
      max_results: 5
    - name: bash
      group: bash
      use: tianshu.sandbox.tools:bash_tool
...
```bash
helm install tian-shu deploy/helm/tian-shu \
  -n tian-shu --create-namespace \
  -f my-values.yaml
```
...
```bash
kubectl -n tian-shu get pods
kubectl -n tian-shu port-forward svc/nginx 2026:2026
...
```bash
kubectl -n tian-shu exec deploy/tian-shu-provisioner -- curl -s localhost:8002/health
```
...
- **Gateway replicas.** Postgres + the Redis stream bridge together make the
  ...
  #3948](https://github.com/bytedance/tian-shu/issues/3948). With >1 replica a
...
- **Redis stream bridge.** A bundled single-instance redis StatefulSet
  ...
  `TIAN_SHU_STREAM_BRIDGE_REDIS_URL`; `config.yaml` sets `stream_bridge.type:
...
- **Persistence.** A PVC (`<release>-home`) backs `/app/backend/.tian-shu`
  (sqlite DB, memory, custom agents, per-thread user-data). The gateway mounts
  it with `subPath: tian-shu` so the layout matches the provisioner's PVC
...
|---|---|---|---|
| gateway | 1000 | 1000 | `.tian-shu` PVC group-writable via fsGroup; `PYTHONDONTWRITEBYTECODE=1` suppresses `.pyc` writes; `UV_CACHE_DIR=/tmp` |
...
```bash
cat <<'EOF' | kubectl apply -n tian-shu -f -
apiVersion: v1
kind: Pod
metadata: { name: fix-home-perms, namespace: tian-shu }
spec:
  restartPolicy: Never
  containers:
    - name: chown
      image: busybox:1.36
      command: ["sh", "-c"]
      args: ["chown -R 1000:1000 /home-pvc/tian-shu && chmod -R g+rwX /home-pvc/tian-shu"]
      volumeMounts:
        - { name: home, mountPath: /home-pvc }
  volumes:
    - name: home
      persistentVolumeClaim: { claimName: tian-shu-tian-shu-home }
EOF
kubectl -n tian-shu wait --for=condition=Ready pod/fix-home-perms --timeout=30s
kubectl -n tian-shu delete pod fix-home-perms
kubectl -n tian-shu rollout restart deploy/tian-shu-tian-shu-gateway
```
...
```bash
helm lint deploy/helm/tian-shu
helm template tian-shu deploy/helm/tian-shu -n tian-shu -f my-values.yaml | \
...
```bash
helm uninstall tian-shu -n tian-shu
# the PVC is NOT deleted by default — remove it manually if desired:
kubectl -n tian-shu delete pvc -l app.kubernetes.io/instance=tian-shu
```
