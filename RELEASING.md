...
TianShu releases are **tag-driven**: pushing a `v*` git tag triggers the
...
| `deploy/helm/tian-shu/Chart.yaml`     | `version: X.Y.Z`     |
| `deploy/helm/tian-shu/Chart.yaml`     | `appVersion: "X.Y.Z"`|
...
2. **Update `CHANGELOG.md`**: rename the `## [Unreleased]` section to
   ...
   ```
   [2.1.0]: https://github.com/bytedance/tian-shu/releases/tag/v2.1.0
   ```
...
- `.github/workflows/chart.yaml` — packages the Helm chart and pushes it as an
  ...
  ```bash
  helm install tian-shu oci://ghcr.io/<owner>/charts/tian-shu --version 2.1.0
  ```
...
is gated on `github.repository == 'bytedance/tian-shu'`, so it only runs on
...
- Images: `ghcr.io/<owner>/tian-shu-{backend,frontend,provisioner}:nightly`
...
- Chart: `oci://ghcr.io/<owner>/charts/tian-shu`, version `<base>-nightly.<date>-<sha>`
...
- Builds multi-arch (`linux/amd64,linux/arm64`) and pushes
  `ghcr.io/<owner>/tian-shu-{lark-cli-init,lark-cli-broker}:<lark-cli-version>`.
- Gated on `github.repository == 'bytedance/tian-shu'`; not tied to the
  `verify-versions` gate (its version is the lark-cli release, not the TianShu
...
builds remain at the legacy bare `ghcr.io/<owner>/tian-shu` package. That
