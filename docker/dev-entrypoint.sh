#!/usr/bin/env sh
#
# TianShu gateway dev entrypoint — runs inside the docker-compose-dev gateway
...
#   3. `uv sync --all-packages` so workspace member extras (tianshu-harness's
...
# means `sandbox` must be created here too, not just `.tian-shu`.
: "${TIAN_SHU_HOME:=/app/backend/.tian-shu}"
export TIAN_SHU_HOME
mkdir -p "$TIAN_SHU_HOME" /app/backend/.tian-shu /app/backend/sandbox
...
# stream bridge to Redis (TIAN_SHU_STREAM_BRIDGE_REDIS_URL); redis is an
...
PYTHONPATH=. exec uv run uvicorn app.gateway.app:app \
    ...
    --reload-exclude="$TIAN_SHU_HOME" \
    --reload-exclude=/app/backend/.tian-shu
