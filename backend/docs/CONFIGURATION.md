...
This guide explains how to configure TianShu for your environment.
...
- Xiaomi MiMo (`tianshu.models.patched_mimo:PatchedChatMiMo`)
- Claude Code OAuth (`tianshu.models.claude_provider:ClaudeChatModel`)
- Codex CLI (`tianshu.models.openai_codex_provider:CodexChatModel`)
...
models:
  - name: gpt-5.4
    ...
    use: tianshu.models.openai_codex_provider:CodexChatModel
    model: gpt-5.4
  ...
  - name: claude-sonnet-4.6
    ...
    use: tianshu.models.claude_provider:ClaudeChatModel
...
- On macOS, TianShu does not probe Keychain automatically. Use `scripts/export_claude_code_oauth.py` to export Claude Code auth explicitly when needed
...
Standard `langchain_openai:ChatOpenAI` silently drops `thought_signature` when serialising messages.  Use `tianshu.models.patched_openai:PatchedChatOpenAI` instead — it re-injects the tool-call signatures (sourced from `AIMessage.additional_kwargs["tool_calls"]`) into every outgoing payload:

```yaml
models:
  - name: gemini-2.5-pro-thinking
    display_name: Gemini 2.5 Pro (Thinking)
    use: tianshu.models.patched_openai:PatchedChatOpenAI
...
models:
  - name: mimo-v2.5-pro
    ...
    use: tianshu.models.patched_mimo:PatchedChatMiMo
    model: mimo-v2.5-pro
...
- Multi-worker deployments (`GATEWAY_WORKERS > 1`) must use the Postgres database backend, enable run ownership heartbeats, and set `run_events.backend: db`. Memory and JSONL run-event stores are process-local and cannot enforce singleton delivery receipts across workers; startup rejects these combinations. The process-local agentic browser tool group is incompatible with multiple Gateway workers; keep `GATEWAY_WORKERS=1` while `browser_navigate` is enabled. Browser control also requires the backend `browser` extra (`cd backend && uv sync --extra browser && uv run playwright install chromium`); startup detects enabled browser config and fails fast when Playwright is missing, and `/api/features` reports `browser_control.enabled=false` until the runtime is available.
...
- `backend: db` — one row per agent in the shared SQL persistence layer (a new `agents` table), so every node in a multi-instance deployment sees the same agents. Requires `database.backend` to be `postgres`; the Gateway **fails fast at startup** if it is `memory` (a per-process database cannot share definitions).
...
tools:
  - name: web_search
    group: web
    use: tianshu.community.tavily.tools:web_search_tool
    max_results: 5
...
tools:
  - name: web_capture
    group: web
    use: tianshu.community.browserless.tools:web_capture_tool
...
TianShu supports multiple sandbox execution modes. Configure your preferred mode in `config.yaml`:

**Local Execution** (runs sandbox code directly on the host machine):
```yaml
sandbox:
   use: tianshu.sandbox.local:LocalSandboxProvider # Local execution
...
```yaml
sandbox:
   use: tianshu.community.aio_sandbox:AioSandboxProvider # Docker-based sandbox
```

**BoxLite micro-VM Sandbox** (runs sandbox code in daemonless OCI micro-VMs):
```yaml
sandbox:
   use: tianshu.community.boxlite:BoxliteProvider
...
```bash
pip install "tianshu-harness[boxlite]"
```
...
```yaml
sandbox:
   use: tianshu.community.aio_sandbox:AioSandboxProvider
   provisioner_url: http://provisioner:8002
```

When using Docker development (`make docker-start`), TianShu starts the `provisioner` service only if this provisioner mode is configured. In local or plain Docker sandbox modes, `provisioner` is skipped.

Remote/provisioner backends default to explicit file synchronization because
TianShu cannot infer whether their `/mnt/user-data` mount points reference the
...
```yaml
sandbox:
   use: tianshu.community.e2b_sandbox:E2BSandboxProvider
...
`e2b-code-interpreter` is bundled as a core dependency of `tianshu-harness`,
...
- Each TianShu thread is bound to its E2B sandbox via metadata
  (`tian_shu_user`, `tian_shu_thread`). Startup and periodic reconciliation
...
```yaml
sandbox:
  use: tianshu.sandbox.local:LocalSandboxProvider
  allow_host_bash: false
```

`allow_host_bash` is intentionally `false` by default. TianShu's local sandbox is a host-side convenience mode, not a secure shell isolation boundary. If you need `bash`, prefer `AioSandboxProvider`. Only set `allow_host_bash: true` for fully trusted single-user local workflows.

When `LocalSandboxProvider` runs under `make up`, it runs inside the `tian-shu-gateway` container. In that mode, `sandbox.mounts[].host_path` is resolved from the gateway container's filesystem, not from your Docker host. If you need a local-sandbox custom mount in production Docker, bind the host directory into the gateway service first, then use the in-container path in `config.yaml`:
...
services:
  gateway:
    volumes:
      - ${TIAN_SHU_REPO_ROOT}/.tian-shu/knowledge:/app/.tian-shu/knowledge:ro
```
...
sandbox:
  use: tianshu.sandbox.local:LocalSandboxProvider
  mounts:
    - host_path: /app/.tian-shu/knowledge
      container_path: /mnt/knowledge
      read_only: true
```

If the configured `host_path` is not visible to the gateway process, TianShu logs an error and ignores that mount.
...
sandbox:
  ...
  port: 8080
  auto_start: true
  container_prefix: tian-shu-sandbox
...
When you configure `sandbox.mounts`, TianShu exposes those `container_path` values in the agent prompt so the agent can discover and operate on mounted directories directly instead of assuming everything must live under `/mnt/user-data`.

For bare-metal Docker sandbox runs that use localhost, TianShu binds the sandbox HTTP port to `127.0.0.1` by default so it is not exposed on every host interface. Docker-outside-of-Docker deployments that connect through `host.docker.internal` keep the broad legacy bind for compatibility. Set `TIAN_SHU_SANDBOX_BIND_HOST` explicitly if your deployment needs a different bind address.
...
`AioSandboxProvider` talks to the sandbox container through the `agent-sandbox` SDK. The Dockerfile for the default `enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest` image is not part of this repository; TianShu treats that image as an upstream AIO sandbox runtime.
...
USER root
# Example user dependency; not required by TianShu itself.
RUN apt-get update \
...
If you rebuild the runtime from scratch instead of extending the published image, it must expose the same HTTP API used by `agent-sandbox`. TianShu currently depends on:

- `sandbox.get_context()`, including `home_dir`
- `shell.exec_command(...)`
- `bash.exec(...)` — only exercised for per-command environment injection (skills that declare `required-secrets`). The `/v1/bash/*` routes exist since upstream all-in-one-sandbox `1.9.3`; on older images (including a `latest` tag still frozen on the `1.0.0.x` line) TianShu fails fast with an actionable error instead of surfacing the raw 404. Pin `sandbox.image` to `1.9.3` or newer (e.g. `1.11.0`) and recreate the sandbox container to use `required-secrets` with the AIO sandbox.
- `file.read_file(...)`
...
- `/mnt/user-data` must remain writable because TianShu mounts thread workspace, uploads, and outputs there.
- `home_dir` comes from the sandbox context endpoint; do not assume TianShu hardcodes it.
- Shell command handling must remain compatible with serialized `exec_command` calls. TianShu serializes shell access on the host side to avoid corrupting the sandbox's persistent shell session.
...
**How Skills Work**:
- Skills are stored in `tian-shu/skills/{public,custom}/`
...
2. Restart the TianShu service to apply changes

## Environment Variables

TianShu supports environment variable substitution using the `$` prefix:
...
- `TIAN_SHU_PROJECT_ROOT` - Project root for relative runtime paths
- `TIAN_SHU_CONFIG_PATH` - Custom config file path
- `TIAN_SHU_EXTENSIONS_CONFIG_PATH` - Custom extensions config file path
- `TIAN_SHU_HOME` - Runtime state directory (defaults to `.tian-shu` under the project root)
- `TIAN_SHU_SKILLS_PATH` - Skills directory when `skills.path` is omitted
...
The configuration file should be placed in the **project root directory** (`tian-shu/config.yaml`). Set `TIAN_SHU_PROJECT_ROOT` when the process may start from another working directory, or set `TIAN_SHU_CONFIG_PATH` to point at a specific file.

## Configuration Priority

TianShu searches for configuration in this order:

1. Path specified in code via `config_path` argument
2. Path from `TIAN_SHU_CONFIG_PATH` environment variable
3. `config.yaml` under `TIAN_SHU_PROJECT_ROOT`, or under the current working directory when `TIAN_SHU_PROJECT_ROOT` is unset
4. Legacy backend/repository-root locations for monorepo compatibility

## Security Notes
### Sandbox Isolation and the Docker Socket (DooD)

TianShu executes agent-generated shell/code through a configurable sandbox
...
|------|---------------|--------------------|-----------|
| `local` (default) | `tianshu.sandbox.local:LocalSandboxProvider` | Not mounted | Commands run **inside the gateway container** on its filesystem. Not a strong boundary — `allow_host_bash` is `false` by default and should stay off for untrusted workloads. |
| `aio` (pure DooD) | `tianshu.community.aio_sandbox:AioSandboxProvider` (no `provisioner_url`) | **Mounted** (opt-in overlay) | Sandbox containers are started via the host Docker daemon. |
...
matters for TianShu because the gateway executes model-generated commands, so a
...
TianShu can reuse your Claude Code / Codex CLI subscription login as a model
...
adapters authenticate independently of TianShu via their own documented env —
...
1. **Place `config.yaml` in project root** - Set `TIAN_SHU_PROJECT_ROOT` if the runtime starts elsewhere
...
- Ensure `config.yaml` exists in the **project root** directory (`tian-shu/config.yaml`)
- If the runtime starts outside the project root, set `TIAN_SHU_PROJECT_ROOT`
- Alternatively, set `TIAN_SHU_CONFIG_PATH` environment variable to custom location
...
### "Skills not loading"
- Check that `tian-shu/skills/` directory exists
- Verify skills have valid `SKILL.md` files
- Check `skills.path` or `TIAN_SHU_SKILLS_PATH` if using a custom path
