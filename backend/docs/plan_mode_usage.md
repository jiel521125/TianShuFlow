...
This document describes how to enable and use the Plan Mode feature with TodoList middleware in TianShu 2.0.
...
### Agent Module
- **Location**: `packages/harness/tianshu/agents/lead_agent/agent.py`
...
TianShu uses custom `system_prompt` and `tool_description` for the TodoListMiddleware that match the overall TianShu prompt style:

### System Prompt Features
- Uses XML tags (`<todo_list_system>`) for structure consistency with TianShu's main prompt
...
The custom prompts are defined in `_create_todo_list_middleware()` in `/Users/hetao/workspace/tian-shu/backend/packages/harness/tianshu/agents/lead_agent/agent.py:57`.

## Notes

- TodoList middleware uses LangChain's built-in `TodoListMiddleware` with **custom TianShu-style prompts**
...
- Custom prompts emphasize the same principles as TianShu's main system prompt (clarity, action-oriented, critical rules)
