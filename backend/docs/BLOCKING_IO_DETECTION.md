...
This document describes how to use and maintain TianShu backend blocking-IO
...
```text
.tian-shu/blocking-io-findings.json
```
...
focused test when code under `app.*` or `tianshu.*` performs blocking IO on
...
Keeping rules centralized makes it clear which extra primitives TianShu
...
- PostgreSQL checkpointer setup, including DSN resolution and connection
  creation.
