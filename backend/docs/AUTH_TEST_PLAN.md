...
```bash
# 清除已有数据
rm -f backend/.tian-shu/data/tianshu.db
...
# 从 .tian-shu/admin_initial_credentials.txt 读取 reset 后密码
...
```bash
# 检查数据库
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "SELECT email, password_hash FROM users LIMIT 3;"
```
...
2. 从 `.tian-shu/admin_initial_credentials.txt` 读取新密码并登录
...
#### TC-UPG-05: 无 tianshu.db 时创建 schema 但不创建默认用户

```bash
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "SELECT COUNT(*) FROM users;"
```

**预期：** `tianshu` 数据库中 `users` 表存在且含 `needs_setup`、`token_version` 列；未调用 `/initialize` 前用户数为 0

#### TC-UPG-06: PostgreSQL 连接可达

```bash
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "SELECT current_database();"
```

**预期：** 返回 `tianshu`（PostgreSQL 事务已提交、连接正常）
...
**预期：**
- [ ] 服务正常启动（忽略 `tianshu.db`，无 auth 相关代码不报错）
- [ ] 旧对话数据仍然可访问
- [ ] `tianshu.db` 文件残留但不影响运行
...
**预期：**
- [ ] 识别已有 `tianshu.db`，不重新创建 admin
- [ ] 旧的 admin 账号仍可登录（如果回退期间未删 `tianshu.db`）
...
```bash
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "DELETE FROM users;"
make dev
make stop
...
```bash
python -m app.gateway.auth.reset_admin --email admin@example.com
ls -la backend/.tian-shu/admin_initial_credentials.txt
cat backend/.tian-shu/admin_initial_credentials.txt
```
...
# 检查 admin 数量
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "SELECT COUNT(*) FROM users WHERE system_role='admin';"
```
...
**预期：**
- [ ] 不报错（PostgreSQL UNIQUE 约束捕获竞争，第二个静默跳过）
- [ ] 最终只有 1 个 admin
...
# 检查用户数
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "SELECT COUNT(*) FROM users WHERE email='race@example.com';"
```
...
cp .tian-shu/admin_initial_credentials.txt /tmp/tianshu-reset-p1.txt
P1=$(awk -F': ' '/^password:/ {print $2}' /tmp/tianshu-reset-p1.txt)

python -m app.gateway.auth.reset_admin
cp .tian-shu/admin_initial_credentials.txt /tmp/tianshu-reset-p2.txt
P2=$(awk -F': ' '/^password:/ {print $2}' /tmp/tianshu-reset-p2.txt)
```

**预期：**
- [ ] `.tian-shu/admin_initial_credentials.txt` 每次都会被重写，文件权限为 `0600`
- [ ] P1 ≠ P2（每次生成新随机密码）
- [ ] P1 不可用，只有 P2 有效
- [ ] `token_version` 递增了 2
- [ ] `needs_setup` 为 True
...
> - PostgreSQL 容器 volume 持久化 `tianshu` 数据库

#### TC-DOCKER-01: PostgreSQL 数据通过 volume 持久化
...
# 检查宿主机上的 PostgreSQL
psql "postgresql://postgres:postgres@localhost:5432/tianshu" -c "SELECT email FROM users WHERE email='docker-test@example.com';"
```

**预期：** `tianshu` 数据库在宿主机 `TIAN_SHU_HOME` 环境对应的 PostgreSQL 实例中，查询可见刚注册的用户。
...
docker logs tian-shu-gateway 2>&1 | grep -E "ChannelManager|channel" | head -10
```
...
```bash
# 首次启动不会自动生成 admin 密码。先重置已有 admin，凭据文件写在挂载到宿主机的 TIAN_SHU_HOME 下。
docker exec tian-shu-gateway python -m app.gateway.auth.reset_admin --email docker-test@example.com

ls -la ${TIAN_SHU_HOME:-backend/.tian-shu}/admin_initial_credentials.txt
# 预期文件权限: -rw------- (0600)

cat ${TIAN_SHU_HOME:-backend/.tian-shu}/admin_initial_credentials.txt
# 预期内容: email + password 行

# 容器日志只输出文件路径，不输出密码本身
docker logs tian-shu-gateway 2>&1 | grep -E "Credentials written to|Admin account"
# 预期看到: "Credentials written to: /...../admin_initial_credentials.txt (mode 0600)"

# 反向验证: 日志里 NEVER 出现明文密码
docker logs tian-shu-gateway 2>&1 | grep -iE "Password: .{15,}" && echo "FAIL: leaked" || echo "OK: not leaked"
```

**预期：**
- 凭证文件存在于 `TIAN_SHU_HOME` 下，权限 `0600`
...
# 确认 gateway 容器存在
docker ps --filter name=tian-shu-gateway --format '{{.Names}}'
# 预期: tian-shu-gateway
...
**预期：** 一个 200、一个 400（current_password 已变导致验证失败）。极端并发下可能两个都 200（PostgreSQL 事务串行化），但最终只有一个密码生效。
