...
compare candidates with the same TianShu workload.
...
same TianShu workload has been measured on both the current AIO sandbox and the
candidate backend.

For an experimental provider PR, prefer `Related to #3213` unless the PR also
includes reproducible TianShu workload data that demonstrates the target memory
