...
TianShu includes automatic conversation summarization to handle long conversations that approach model token limits. When enabled, the system automatically condenses older messages while preserving recent context.
...
- **Configuration**: `packages/harness/tianshu/config/summarization_config.py`
- **Integration**: `packages/harness/tianshu/agents/lead_agent/agent.py`
