...
AsyncPostgresSaver, and the real Gateway route stack for reads. Every case
runs in a fresh child process in a fresh Postgres schema, mirroring the
...
def _worker_main(encoded_case: str, *, profile_path: Path | None = None, postgres_url: str | None = None) -> int:
    ...
    try:
        ...
        return 2
    with tempfile.TemporaryDirectory(prefix="tianshu-checkpoint-production-") as temp_dir:
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "PostgreSQL DSN for the checkpointer "
            f"(default: ${_POSTGRES_URL_ENV} or {_DEFAULT_POSTGRES_URL}). "
            "Each case runs in a fresh schema dropped after the case."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    for index, case in enumerate(cases, start=1):
        ...
        if args.postgres_url is not None:
            worker_args.extend(["--postgres-url", args.postgres_url])
...
class _TimingSaver:
    ...
    per-call latencies overlap (queueing on the single database connection is
...
async def _run_phase(case: ProductionCase, saver: Any, timing: _TimingSaver, *, dsn: str, schema: str) -> dict[str, Any]:
    ...
    from tianshu.config.app_config import AppConfig, set_app_config
    ...
    with patch("tianshu.agents.lead_agent.agent.create_chat_model", _fake_create_chat_model):
        graph = make_lead_agent({"configurable": {}})
    # Attach the timed saver post-construction, mirroring the run worker
    # (tianshu.runs.worker assigns ``agent.checkpointer`` the same way), so
...
async def _read_phase(case: ProductionCase, saver: Any, timing: _TimingSaver) -> dict[str, Any]:
    ...
    Runs in the same event loop as the run phase (the AsyncPostgresSaver is
...
def _storage_metrics(dsn: str, schema: str) -> dict[str, Any]:
    """Postgres disk footprint for the checkpointer tables in *schema*."""
...
def _run_case(case: ProductionCase, *, work_dir: Path) -> dict:
    ...
    row = _base_row(case)
    dsn = _postgres_dsn()
    schema = f"prod_{uuid.uuid4().hex[:8]}"
    ...
    async def _async_body() -> dict[str, Any]:
        ...
        async with AsyncPostgresSaver.from_conn_string(conn_string) as saver:
            ...
            run_info = await _run_phase(case, saver, timing, dsn=dsn, schema=schema)
            ...
            storage_metrics = _storage_metrics(dsn, schema)
