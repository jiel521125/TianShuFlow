...
Requires ``database.backend`` to be ``postgres`` in config.yaml.
"""
...
def main() -> int:
    ...
    if db_backend != "postgres":
        logger.error(
            "database.backend is %r; this importer needs 'postgres'. Set it in config.yaml (the same database the gateway uses).",
            db_backend,
        )
        return 1
    ...
    from tianshu.persistence.engine import init_engine_from_config
