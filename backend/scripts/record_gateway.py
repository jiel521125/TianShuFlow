...
Used by ``frontend/playwright.record.config.ts``. Env:
  ...
  TIANSHU_RECORD_OUT               - JSONL path to append captured turns to
...
def _install_capture(out_path: Path) -> None:
    ...
    import tianshu.models.factory as factory_mod
...
def main() -> int:
    ...
    record_out = os.environ.get("TIANSHU_RECORD_OUT")
    if not record_out:
        print("ERROR: set TIANSHU_RECORD_OUT to the JSONL path to append captured turns to", file=sys.stderr)
        return 2
    ...
    # TIAN_SHU_HOME can't leak in and shift prompt-affecting paths/skills.
    os.environ["TIAN_SHU_HOME"] = str(home)
    os.environ["TIAN_SHU_CONFIG_PATH"] = str(cfg)
    os.environ["TIAN_SHU_EXTENSIONS_CONFIG_PATH"] = str(prepare_hermetic_extras(home))
