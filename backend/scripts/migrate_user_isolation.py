...
def _build_owner_map_from_db(paths: Paths) -> dict[str, str]:
    ...
    Reads the configured Postgres database via the shared persistence engine
    (the same database the gateway uses). Falls back to an empty map when the
    database is unreachable so the filesystem migration can still proceed.
    """
    import asyncio
    ...
    async def _load() -> dict[str, str]:
        try:
            ...
            from tianshu.persistence.engine import (
                close_engine,
                get_session_factory,
                ...
                async with sf() as session:
                    rows = (
                        await session.execute(
                            select(ThreadMetaRow.thread_id, ThreadMetaRow.user_id).where(ThreadMetaRow.user_id.is_not(None))
                        )
                    ).all()
            ...
            logger.warning("Failed to query threads_meta; using empty owner map", exc_info=True)
            return {}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Migrate TianShu data to per-user layout")
