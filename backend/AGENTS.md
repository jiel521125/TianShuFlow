...
TianShu is a LangGraph-based AI super agent system with a full-stack architecture. The backend provides a "super agent" with sandbox execution, persistent memory, subagent delegation, and extensible tool integration - all operating in per-thread isolated environments.
...
**Runtime**:
- `make dev`, Docker dev, and production all run the agent runtime in Gateway via `RunManager` + `run_agent()` + `StreamBridge` (`packages/harness/tianshu/runtime/`). Nginx exposes that runtime at `/api/langgraph/*` and rewrites it to Gateway's native `/api/*` routers.
...
│   ├── packages/
│   │   └── harness/           # tianshu-harness package (import: tianshu.*)
│   │       ├── pyproject.toml
│   │       └── tianshu/
...
│   │           └── client.py          # Embedded Python client (TianShuClient)
...
make test-live          # Explicitly run live TianShuClient tests with real APIs
...
boundaries under `backend/app/` and `backend/packages/harness/tianshu/`. It
prints a concise count by execution domain and writes the complete, versioned
JSON payload to `.tian-shu/thread-boundary-inventory.json`. Every finding has
...
python scripts/detect_thread_boundaries.py \
  ...
  --json-output .tian-shu/thread-boundary-inventory.json
```
...
The `detect-blocking-io` target parses `app/`, `packages/harness/tianshu/`,
...
findings to `.tian-shu/blocking-io-findings.json` at the repository root
...
- Wraps every item under `tests/blocking_io/` with a strict Blockbuster
  context scoped to `app.*` and `tianshu.*` (see
  `tests/support/detectors/blocking_io_runtime.py`). Any sync blocking IO
  call whose stack passes through TianShu business code while running on
...
- `tests/test_harness_boundary.py` — ensures `packages/harness/tianshu/` never imports from `app.*`
...
- **Harness** (`packages/harness/tianshu/`): Publishable agent framework package (`tianshu-harness`). Import prefix: `tianshu.*`. Contains agent orchestration, tools, sandbox, models, MCP, skills, config — everything needed to build and run agents.
- **App** (`app/`): Unpublished application code. Import prefix: `app.*`. Contains the FastAPI Gateway API and IM channel integrations (Feishu, Slack, Telegram, DingTalk).

**Dependency rule**: App imports tianshu, but tianshu never imports app. This boundary is enforced by `tests/test_harness_boundary.py` which runs in CI.

**Import conventions**:
```python
# Harness internal
from tianshu.agents import make_lead_agent
from tianshu.models import create_chat_model
...
Package import hygiene: the `tianshu.agents` and `tianshu.subagents` package
...
**Lead Agent** (`packages/harness/tianshu/agents/lead_agent/agent.py`):
- Entry point: `make_lead_agent(config: RunnableConfig)` registered in `langgraph.json`
- Dynamic model selection via `create_chat_model()` with thinking/vision support
- Tools loaded via `get_available_tools()` - combines sandbox, built-in, MCP, community, and subagent tools
- System prompt generated by `apply_prompt_template()` with skills, memory, and subagent instructions

**ThreadState** (`packages/harness/tianshu/agents/thread_state.py`):
...
- `max_total_subagents` - Optional per-run total delegation cap override (falls back to `subagents.max_total_per_run`, clamped to 1-50)
  Gateway and `TianShuClient.stream()` always provide the runtime `run_id`; custom
...
Lead-agent middlewares are assembled in strict order across three functions: the shared base in `packages/harness/tianshu/agents/middlewares/tool_error_handling_middleware.py` (`_build_runtime_middlewares`, exposed via `build_lead_runtime_middlewares`), then the lead-only middlewares appended in `packages/harness/tianshu/agents/lead_agent/agent.py` (`build_middlewares`). Items marked *(optional)* are appended only when their config/runtime condition holds, so the live chain length varies.
...
4. **ThreadDataMiddleware** - Creates per-thread directories under the user's isolation scope (`backend/.tian-shu/users/{user_id}/threads/{thread_id}/user-data/{workspace,uploads,outputs}`); resolves identity via `resolve_runtime_user_id(runtime)`, including Gateway runtime context and standalone LangGraph Server auth, then falls back to the request ContextVar / `"default"`
...
Infrastructure fields are **restart-required**. The authoritative list lives in `packages/harness/tianshu/config/reload_boundary.py::STARTUP_ONLY_FIELDS` and is mirrored by the standardised `"startup-only:"` prefix on the corresponding `Field(description=...)` in `AppConfig`, so IDE hover on those fields surfaces the reason inline (no need to context-switch into this table). Currently registered: `database`, `checkpointer`, `run_events`, `stream_bridge`, `sandbox`, `log_level`, `logging`, `channels`, `channel_connections`, `scheduler`, `run_ownership`. Adding a new restart-required field requires updating the registry; drift is pinned by `tests/test_reload_boundary.py`.

**Persistence backend resolution**: the unified `database` section selects the
Gateway's LangGraph checkpointer, LangGraph Store, and TianShu SQL repositories.
...
2. `TIAN_SHU_CONFIG_PATH` environment variable
...
`TIAN_SHU_CONFIG_PATH` / `TIAN_SHU_EXTENSIONS_CONFIG_PATH` into that directory.
...
Configuration priority:
1. Explicit `config_path` argument
2. `TIAN_SHU_EXTENSIONS_CONFIG_PATH` environment variable
3. `extensions_config.json` in current directory (backend/)
4. `extensions_config.json` in parent directory (project root - **recommended location**)

Extensions are optional only in the fallback *search* mode (priority 3-4 above): `ExtensionsConfig.resolve_config_path()` returns `None` when neither an explicit `config_path` nor `TIAN_SHU_EXTENSIONS_CONFIG_PATH` is given and the search locations find nothing. An explicit `config_path` argument or a set `TIAN_SHU_EXTENSIONS_CONFIG_PATH` (priority 1-2) is an operator assertion that one particular file must be used, so a missing file in either of those modes raises `FileNotFoundError` instead — including when the file existed earlier and has since been deleted. The MCP tools cache's staleness check (`tianshu.mcp.cache._resolve_config_path`) is a narrow, deliberate exception to that rule: it catches that `FileNotFoundError` locally and treats it as "unconfigured" so a previously-valid config disappearing mid-run degrades the cache to serving its last-known-good tools instead of raising out of a per-request hot path (see the MCP System section below).
...
Thread identifiers use the shared `tianshu.utils.thread_id` contract
...
**Workspace change review**: `packages/harness/tianshu/workspace_changes/`
...
### Sandbox System (`packages/harness/tianshu/sandbox/`)
...
- `E2BSandboxProvider` (`packages/harness/tianshu/community/e2b_sandbox/`) provides E2B remote isolation.
  ...
  - **Cross-instance ownership store** (`aio_sandbox/ownership/`, #4206): gateway instances sharing a container backend coordinate container ownership through a pluggable lease store, selected by `sandbox.ownership.type` (`memory` | `redis`) and resolved like `stream_bridge` (`factory.py`, lazy per-branch import, `redis` optional extra, `TIAN_SHU_SANDBOX_OWNERSHIP_REDIS_URL` env escape hatch; a set `TIAN_SHU_STREAM_BRIDGE_REDIS_URL` implies a multi-instance deployment and infers `redis`). `memory` is single-instance only and declares `supports_cross_process = False`.
    ...
    - `get()` stays a pure in-memory lookup and must never call the store (that is blocking filesystem/network IO on the event loop); anchored by `tests/blocking_io/test_aio_sandbox_get.py`, which injects a deliberately-blocking probe store so the anchor keeps its teeth regardless of the configured backend. Tests: `tests/test_sandbox_ownership_store.py` (store contract, defined once for **both** backends — the redis tier is `@pytest.mark.integration`, uses `TIAN_SHU_TEST_REDIS_URL` when set, and otherwise self-skips without a reachable Redis. Backend CI provisions Redis, so the merge gate executes the real Lua tier; there is no fake-redis tier because a fake would not execute the Lua exclusions) and `tests/test_sandbox_orphan_reconciliation.py` (provider behaviour, two providers sharing one store).
- `BoxliteProvider` (`packages/harness/tianshu/community/boxlite/`) - BoxLite micro-VM isolation. The `boxlite` runtime is optional (`tianshu-harness[boxlite]`) and lazy-imported only when this provider is selected. The provider owns one private asyncio event loop on a daemon thread because BoxLite handles are loop-affine; sync `Sandbox` calls marshal onto that loop with `run_coroutine_threadsafe`.
...
### Subagent System (`packages/harness/tianshu/subagents/`)
...
**Community tools** (`packages/harness/tianshu/community/`): optional integrations, each in its own subpackage and wired through `config.yaml`. Documented examples:
...
- **Routing hints**: `extensions_config.json -> mcpServers.<server>.routing` and
  ...
  under `tianshu_mcp_routing`. Prompt rendering uses
...
### Skills System (`packages/harness/tianshu/skills/`)

- **Location**: global public skills live under `tian-shu/skills/public/`; user-authored custom skills live under `{TIAN_SHU_HOME}/users/{user_id}/skills/custom/`; globally managed integration skills live under `{TIAN_SHU_HOME}/integrations/skills/{provider}/`; per-user integration credentials remain under `{TIAN_SHU_HOME}/users/{user_id}/integrations/{provider}/{config,data}`
...
### Model Factory (`packages/harness/tianshu/models/factory.py`)
...
### vLLM Provider (`packages/harness/tianshu/models/vllm_provider.py`)
...
Bridges external messaging platforms (Feishu, Slack, Telegram, Discord, DingTalk, GitHub) to the TianShu agent via Gateway's LangGraph-compatible API.
...
- `tianshu.persistence.channel_connections` - SQL-backed user-owned connection, optional credential, connect state, and conversation store
...
**Owner-scoped file storage**: inbound files, uploads, and output artifacts are staged under the TianShu owner's bucket so they land where the agent run reads/writes (`users/{user_id}/threads/{thread_id}/user-data/{uploads,outputs}`). `ChannelManager._handle_chat` resolves the storage owner once via `_channel_storage_user_id(msg)` (sanitized owner id, falling back to `safe(msg.user_id)` for unbound auth-enabled channels — mirroring `_resolve_run_params`'s run identity; `None` only when no identity is available) and threads it as the `user_id=` kwarg through the file pipeline:
...
- In Docker Compose, IM channels run inside the `gateway` container, so `localhost` points back to that container. Use `http://gateway:8001/api` for `langgraph_url` and `http://gateway:8001` for `gateway_url`, or set `TIAN_SHU_CHANNELS_LANGGRAPH_URL` / `TIAN_SHU_CHANNELS_GATEWAY_URL`.
...
### Memory System (`packages/harness/tianshu/agents/memory/`)
...
- `pg_storage.py` - PostgreSQL memory backend (default `storage_class: postgres`): a user-global summary row plus per-agent fact rows in the configured `postgres_schema`; SQL-native case-insensitive substring search replaces the removed SQLite FTS5 retrieval adapter (no jieba / `memory-zh` extra). File layout remains available via `storage_class: file`
...
**Per-User Isolation**:
- Default (`storage_class: postgres`): memory persists to the database in the configured `postgres_schema` — a user-global summary row and per-agent fact rows, isolated by `user_id` (and `agent_name` for facts)
- File backend (`storage_class: file`): memory is stored per-user at `{base_dir}/users/{user_id}/memory.json`
...
- The `/api/memory*` endpoints resolve the owner through `_resolve_memory_user_id(request)`: trusted internal callers (IM channel workers carrying the `X-TianShu-Owner-User-Id` header, e.g. a bound `/memory` command) act for the connection owner; browser/API callers fall back to `get_effective_user_id()`. The header is only honored after `AuthMiddleware` validated the internal token, mirroring `get_trusted_internal_owner_user_id` used by the threads router
...
**Token counting** (`packages/harness/tianshu/agents/memory/prompt.py`):
...
### Reflection System (`packages/harness/tianshu/reflection/`)

- `resolve_variable(path)` - Import module and return variable (e.g., `module.path:variable_name`)
- `resolve_class(path, base_class)` - Import and validate class against base class

### Schema Migrations (`packages/harness/tianshu/persistence/migrations/`)

TianShu's application tables (`runs`, `threads_meta`, `feedback`, `users`, `run_events`, plus the four `channel_*` tables) are owned by alembic via a **hybrid bootstrap** strategy. LangGraph's checkpointer tables (`checkpoints`, `checkpoint_blobs`, `checkpoint_writes`, `checkpoint_migrations`) live in the same database but are owned by LangGraph and excluded from alembic's view via `migrations/_env_filters.py::include_object`.
...
**Mode is process-frozen and restart-required.** `make_lead_agent` and the embedded `TianShuClient` freeze the resolved mode (`runtime/checkpoint_mode.py::freeze_checkpoint_channel_mode`) before compiling the graph with the mode-matched schema (`agents/thread_state.py::get_thread_state_schema`, plus `adapt_state_schema_for_mode` / `normalize_middleware_state_schemas` for middleware state). Adapted middleware schemas are cached by schema, mode, and resolved snapshot frequency so a pre-freeze ephemeral graph cannot leave a stale default-frequency schema behind. A second, different mode or frequency in the same process raises `CheckpointModeReconfigurationError`. To switch: edit config, restart.
...
### Terminal Workbench / TUI (`packages/harness/tianshu/tui/`)

A terminal-native UI over the embedded harness, exposed as the `tianshu` console script (`[project.scripts]` in `packages/harness/pyproject.toml`). It is a UI shell over `TianShuClient` and does **not** fork agent behavior. `textual` is an optional dependency (`tianshu-harness[tui]`; also in the backend dev group); the console script degrades to headless help when it is absent. Full guide: [docs/TUI.md](docs/TUI.md).

**Module layout** (all layers except `app.py` are pure / Textual-free and unit-tested directly):
- `cli.py` — `plan_launch()` (pure launch-mode decision) + headless `--print` / `--json` + `main()` entry point. TTY → TUI, else headless help. Uses an **absolute** `from tianshu.tui.app import run_tui` so the `app.py` module name doesn't trip `test_harness_boundary.py` (which records relative import module names verbatim).
...
**Trace-attribute injection points**: both `runtime/runs/worker.py::run_agent` (gateway path) and `client.py::TianShuClient.stream` (embedded path) merge the metadata into `config["metadata"]` right before constructing the graph. `subagents/executor.py::_aexecute` does the same for every subagent run so subagent traces group under the parent thread's session card (carrying the parent `thread_id` → `langfuse_session_id`, the user_id captured at `task_tool` → `langfuse_user_id`, and a `subagent:<normalized-name>` trace name). Caller-supplied keys win via `setdefault`, so an external `session_id` override is preserved. Field mapping:
...
| `langfuse_tags`       | `env:<TIAN_SHU_ENV>` + `model:<model_name>`  |
| `tianshu_trace_id`   | Current request/entry trace id from `tianshu.trace_context`; matches `X-Trace-Id` for enhanced Gateway HTTP requests. Gated by `logging.enhance.enabled` in both gateway and embedded paths via `is_trace_correlation_enabled` — off by default; embedded callers can still opt in per-turn by wrapping `stream()` in `request_trace_context(...)` |

Returns `{}` when Langfuse is not in the enabled providers — LangSmith-only deployments are unaffected. Set `TIAN_SHU_ENV` (or `ENVIRONMENT`) to tag traces by deployment environment. Tests live in `tests/test_tracing_factory.py`, `tests/test_tracing_metadata.py`, `tests/test_worker_langfuse_metadata.py`, `tests/test_client_langfuse_metadata.py`, and `tests/test_subagent_executor.py::TestSubagentTracingWiring`.

**Monocle telemetry** is a third provider, structurally unlike LangSmith/Langfuse. It is **not** a LangChain callback: `tracing/monocle.py::setup_monocle_tracing_if_enabled()` calls `monocle_apptrace.setup_monocle_telemetry()` once, which installs a **process-global OTel `TracerProvider`**, patches span serialization, and auto-instruments the openai/langchain/langgraph clients. Because that is a one-time, process-global side effect (not a per-run callback), it is initialized from the **Gateway lifespan** (`app/gateway/app.py`) — never from `build_tracing_callbacks()` — and it is **off by default**. The setup call was deliberately moved out of `agents/__init__.py`, so `import tianshu.agents` must never start tracing (pinned by `tests/test_monocle_tracing.py::test_no_import_time_setup`). The Gateway lifespan is the **sole call site** (pinned by `test_gateway_lifespan_initializes_monocle`), so unlike LangSmith/Langfuse — which attach at the graph roots and cover every path — the embedded `TianShuClient` and the TUI are not instrumented; embedded users who want Monocle traces call `setup_monocle_tracing_if_enabled()` themselves before running the agent.

Unlike the Langfuse metadata above, TianShu injects **no** per-run fields into Monocle traces — the only attribute it sets is `workflow_name="tian-shu"`; every span attribute (`span.type`, `entity.*`, token usage, span inputs/outputs, `scope.agentic.session`) is produced by Monocle's own metamodel and auto-instrumentation, so there is no TianShu trace-attribute layer to maintain here.
...
- `logging.enhance` - Optional request trace correlation (`enabled`, `format`) for Gateway `X-Trace-Id`, log `trace_id`, and Langfuse `tianshu_trace_id`
- vLLM reasoning models should use `tianshu.models.vllm_provider:VllmChatModel`; for Qwen-style parsers prefer `when_thinking_enabled.extra_body.chat_template_kwargs.enable_thinking`, and TianShu will also normalize the older `thinking` alias
...
Gateway API endpoints and `TianShuClient` methods can modify MCP servers and skill state at runtime; their `extensions_config.json` writes use the shared atomic replacement helper, while `middlewares` remains an operator-controlled config-file extension point.

### Embedded Client (`packages/harness/tianshu/client.py`)

`TianShuClient` provides direct in-process access to all TianShu capabilities without HTTP services. All return types align with the Gateway API response schemas, so consumer code works identically in HTTP and embedded modes.

**Architecture**: Imports the same `tianshu` modules that Gateway API uses. Shares the same config files and data directories. No FastAPI dependency.
...
- `stream(message, thread_id)` — subscribes to LangGraph `stream_mode=["values", "messages", "custom"]` and yields `StreamEvent`:
  ...
  - `"custom"` — forwarded from `StreamWriter`; TianShu-built-in custom events are dual-emitted through `tianshu.utils.custom_events`, so `astream_events(version="v2")` consumers also receive one `on_custom_event` with `name=payload["type"]` and the unchanged payload as `data`
  - `"end"` — stream finished (carries cumulative `usage` counted once per message id)
- **Custom-event invariant** — production TianShu emitters must use `emit_custom_event` / `aemit_custom_event`, not call `StreamWriter` alone. Every built-in payload must carry a non-empty string `type`; typeless payloads remain writer-only and are intentionally absent from `astream_events`. The writer runs first and remains authoritative for Gateway, Web UI, and embedded-client compatibility; callback dispatch is best-effort and must not break that path. Async graph hooks must await the async helper rather than invoking synchronous dispatch on a running event loop.
- Agent created lazily via `create_agent()` + `build_middlewares()`, same as `make_lead_agent`
- Supports `checkpointer` parameter for state persistence across turns
- `reset_agent()` forces agent recreation (e.g. after memory or skill changes)
- See [docs/STREAMING.md](docs/STREAMING.md) for the full design: why Gateway and TianShuClient are parallel paths, LangGraph's `stream_mode` semantics, the per-id dedup invariants, and regression testing strategy
...
**Key difference from Gateway**: Upload accepts local `Path` objects instead of HTTP `UploadFile`, rejects directory paths before copying, and reuses a single worker when document conversion must run inside an active event loop. Artifact returns `(bytes, mime_type)` instead of HTTP Response. The new Gateway-only thread cleanup route deletes `.tian-shu/threads/{thread_id}` after LangGraph thread deletion; there is no matching `TianShuClient` method yet. `update_mcp_config()` and `update_skill()` automatically invalidate the cached agent.
...
`make test-live` or `TIAN_SHU_RUN_LIVE_TESTS=1`). The live suite calls real
...
- If a module causes circular import issues in tests, add a `sys.modules` mock in `tests/conftest.py` (see existing example for `tianshu.subagents.executor`)
...
skipped unless `TIAN_SHU_RUN_LIVE_TESTS=1` is set. Do not add that opt-in to
...
- Gateway HTTP upload/list/delete handlers offload filesystem work through `tianshu.utils.file_io.run_file_io`, a dedicated ContextVar-preserving file IO executor. Non-mounted sandbox uploads acquire sandboxes with `SandboxProvider.acquire_async()` and offload `read_bytes()` plus `sandbox.update_file()` together.
...
- Manual compaction uses `POST /api/threads/{id}/compact`, reuses the same
  `TianShuSummarizationMiddleware`, writes a new checkpoint with updated
