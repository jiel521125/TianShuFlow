"""ChannelManager — consumes inbound messages and dispatches them to the TianShu agent via Gateway."""
...
from tianshu.runtime import END_SENTINEL, StreamBridge
from tianshu.runtime.goal import parse_goal_command
from tianshu.runtime.user_context import get_effective_user_id
from tianshu.skills.slash import parse_slash_skill_reference
from tianshu.skills.storage import get_or_new_skill_storage
from tianshu.skills.storage.skill_storage import SkillStorage
...
BOUND_IDENTITY_REQUIRED_MESSAGE = "Connect this channel from TianShu Settings, complete the in-channel connect step, then send your message again."
BOUND_IDENTITY_UNAVAILABLE_MESSAGE = "Channel connection verification is temporarily unavailable. Please try again later or contact the TianShu operator."
...
def _channel_storage_user_id(msg: InboundMessage) -> str | None:
    """Resolve the canonical TianShu user id for a channel-triggered message.
    ...
    matches where channel files are staged. Prefer the bound TianShu owner,
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the TianShu agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        # ``user_id`` drives TianShu-owned memory, files, and thread buckets.
        # For browser-connected IM channels, prefer the TianShu account that
    ...
    async def _get_bound_identity_rejection(self, msg: InboundMessage) -> _BoundIdentityRejection | None:
        ...
        # enforced at the webhook route by HMAC, and the "sender → TianShu
        ...
        # the binding by provider identity before creating TianShu threads or
    ...
    async def _handle_chat(
        ...
        *,
        ...
        # Look up the existing TianShu thread, creating one if this is the
