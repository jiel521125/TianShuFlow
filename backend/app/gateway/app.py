...
from tianshu.config import app_config as tianshu_app_config
from tianshu.logging_config import DEFAULT_LOG_DATE_FORMAT, DEFAULT_LOG_FORMAT, configure_logging
from tianshu.tracing.monocle import setup_monocle_tracing_if_enabled
from tianshu.uploads.manager import cleanup_stale_upload_staging_files

AppConfig = tianshu_app_config.AppConfig
get_app_config = tianshu_app_config.get_app_config
...
async def _ensure_admin_user(app: FastAPI) -> None:
    ...
    "no-auth → with-auth" upgrade path: users who ran TianShu without
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    from tianshu.skills.projection import ensure_public_skill_projection
    ...
    # plain `import tianshu.agents` never installs a process-global tracer.
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="TianShu API Gateway",
        description="""
## TianShu API Gateway

API Gateway for TianShu - A LangGraph-based AI agent backend with sandbox execution capabilities.
...
This gateway provides runtime endpoints for agent runs plus custom endpoints for models, MCP configuration, skills, and artifacts.
        ...
        openapi_tags=[
            ...
            {
                "name": "threads",
                "description": "Manage TianShu thread-local filesystem data",
            },
            {
                "name": "agents",
    ...
    # Folder ↔ Git repository binding API is mounted at /api/folders
    app.include_router(workspaces.folder_git_router)

    # Git integration API is mounted at /api/git
    ...
    # Workflow orchestration API is mounted at /api/workflows
    app.include_router(workflows.router)

    # Chat-Workflow integration API is mounted at /api/chat/workflows
    app.include_router(chat_workflows.router)
    ...
    # (or when the explicit TIAN_SHU_ALLOW_UNVERIFIED_GITHUB_WEBHOOKS=1
    ...
    if github_webhooks.is_route_enabled():
        ...
        logger.warning("GitHub webhooks route NOT mounted: GITHUB_WEBHOOK_SECRET unset and TIAN_SHU_ALLOW_UNVERIFIED_GITHUB_WEBHOOKS not set. /api/webhooks/github will respond 404. Configure either env var to enable the route.")
    ...
    async def health_check() -> dict[str, str]:
        ...
        """
        return {"status": "healthy", "service": "tian-shu-gateway"}
