...
``tianshu.persistence.engine`` — the ``users`` table lives in the
same PostgreSQL database as ``threads_meta``, ``runs``, ``run_events``,
and ``feedback``.

Constructor takes the session factory directly (same pattern as the
other four repositories in ``tianshu.persistence.*``). Callers
...
class PostgresUserRepository(UserRepository):
    """Async user repository backed by the shared PostgreSQL engine."""
    ...
    def _row_to_user(row: UserRow) -> User:
        return User(
            ...
            # Reattach UTC when the driver returns a naive datetime so
            # downstream code can compare timestamps reliably.
