...
(``database.backend: postgres``); returns 503 on the memory backend,
...
def _session_factory_or_503():
    ...
    if sf is None:
        raise HTTPException(
            status_code=503,
            detail="Console requires a SQL database backend; set database.backend to postgres in config.yaml.",
        )
    return sf


def _as_utc(dt: datetime | None) -> datetime | None:
    """Normalize DB timestamps: the driver may round-trip them naive, Postgres stores aware."""
    if dt is None:
        return None
