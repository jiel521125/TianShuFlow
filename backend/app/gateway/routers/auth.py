    ...
    if getattr(request.state, "auth_source", None) == AUTH_SOURCE_AUTH_DISABLED:
        raise HTTPException(
            ...
            detail=AuthErrorResponse(
                ...
                message="Password changes are not available when TIAN_SHU_AUTH_DISABLED=1.",
            ).model_dump(),
        )
...
async def oauth_callback(
    ...
    the ID token, provisions/links the TianShu user, and sets the
    session cookie.
    """
    from tianshu.config.app_config import get_app_config
