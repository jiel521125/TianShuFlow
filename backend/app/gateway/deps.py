...
run path resolve it through :func:`tianshu.config.app_config.get_app_config`,
...
from tianshu.community.browser_automation.session import browser_multi_worker_error
...
from tianshu.runtime import ORPHAN_RECOVERY_STOP_REASON, STARTUP_ORPHAN_RECOVERY_ERROR, RunContext, RunManager, StreamBridge
from tianshu.runtime.events.store.base import RunEventStore
...
def _enforce_postgres_for_multi_worker(config: AppConfig) -> None:
    ...
    2. The DB backend must be Postgres — only Postgres supports
    ...
    if backend != "postgres":
        raise SystemExit(f"GATEWAY_WORKERS={workers} requires database.backend='postgres', but database.backend is '{backend}'. Only Postgres supports concurrent multi-process access. Set GATEWAY_WORKERS=1 or switch to Postgres.")
...
def _validate_agent_storage(config: AppConfig) -> None:
    ...
    if backend == "db" and db_backend != "postgres":
        raise SystemExit(
            f"agent_storage.backend='db' requires database.backend='postgres', "
            f"but database.backend is '{db_backend}'. A 'memory' database is per-process and cannot "
            "share agent definitions across nodes. Set database.backend to 'postgres', or use agent_storage.backend='file'."
        )
    try:
        workers = int(os.environ.get("GATEWAY_WORKERS", "1"))
    except (TypeError, ValueError):
...
if TYPE_CHECKING:
    ...
    from tianshu.persistence.thread_meta.base import ThreadMetaStore
    from tianshu.runtime import RunRecord
...
def get_config() -> AppConfig:
    ...
    Routes through :func:`tianshu.config.app_config.get_app_config`, which
    ...
    :func:`get_app_config` closes the bytedance/tian-shu issue #3107 BUG-001
    ...
    :mod:`tianshu.config.reload_boundary` and is mirrored by the
...
async def langgraph_runtime(app: FastAPI, startup_config: AppConfig) -> AsyncGenerator[None, None]:
    ...
    *startup-time* run-events configuration the underlying ``event_store``
    ...
    from tianshu.runtime import make_store, make_stream_bridge
    from tianshu.runtime.checkpoint_mode import freeze_checkpoint_channel_mode, freeze_checkpoint_snapshot_frequency
    from tianshu.runtime.checkpointer.async_provider import make_checkpointer
    from tianshu.runtime.events.store import make_run_event_store

    # ------------------------------------------------------------------
    # Multi-worker safety gate: only Postgres can support concurrent
    # multi-process access when GATEWAY_WORKERS > 1.
    # ------------------------------------------------------------------
    ...
    async with AsyncExitStack() as stack:
        ...
        from tianshu.persistence.thread_meta import make_thread_store

        app.state.thread_store = make_thread_store(sf, app.state.store)
        if sf is not None:
            from tianshu.persistence.scheduled_task_runs import (
                ScheduledTaskRunRepository,
            )
            from tianshu.persistence.scheduled_tasks import ScheduledTaskRepository
        ...
        # In single-worker mode (backend=memory), no run has a lease, so
        ...
        from tianshu.utils.time import now_iso
...
_cached_repo: PostgresUserRepository | None = None
...
def get_local_provider() -> LocalAuthProvider:
    ...
    if _cached_repo is None:
        ...
        _cached_repo = PostgresUserRepository(sf)
