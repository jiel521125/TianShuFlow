...
from tianshu.agents.middlewares.dynamic_context_middleware import _DYNAMIC_CONTEXT_REMINDER_KEY, _REMINDER_DATE_KEY
from tianshu.agents.middlewares.view_image_middleware import _IMAGE_CONTEXT_MESSAGE_MARKER_KEY
from tianshu.config.app_config import get_app_config
from tianshu.config.database_config import resolve_checkpoint_graph_cache_max
from tianshu.persistence.user_models.sql import UserModelRepository
from tianshu.runtime import (
    END_SENTINEL,
    HEARTBEAT_SENTINEL,
...
from tianshu.runtime.checkpoint_state import graph_state_schema
from tianshu.runtime.goal import goal_thread_lock
from tianshu.runtime.journal import build_checkpoint_history_seed_events
from tianshu.runtime.runs.naming import resolve_root_run_name
from tianshu.runtime.secret_context import (
    LegacyRunMetadataSecretError,
    redact_config_secrets,
    validate_run_metadata_secrets,
)
from tianshu.runtime.stream_modes import normalize_stream_modes
from tianshu.runtime.user_context import get_effective_user_id, reset_current_user, set_current_user
from tianshu.utils.messages import ORIGINAL_USER_CONTENT_KEY
from tianshu.utils.thread_id import validate_thread_id
...
async def resolve_trusted_internal_owner_for_attribution(request: Request, owner_user_id: str | None) -> Any | None:
    """Resolve the TianShu user used only for trusted internal attribution."""
...
def resolve_agent_factory(assistant_id: str | None):
    ...
    ``cfg["agent_name"]``.
    """
    from tianshu.agents.lead_agent.agent import make_lead_agent
...
async def start_run(
    ...
    if model_name:
        ...
        if resolved is None:
            # User-registered models live in the ``user_models`` table, not
            # ``config.yaml``. Fall back to the caller's rows so a model the
            # user added in Settings (and selected in the chat picker) is
            # accepted here; the user-aware model factory resolves it later.
            user_id = get_effective_user_id()
            if user_id:
                user_rows = await UserModelRepository().list_for_user(user_id)
                resolved = next(
                    (
                        row
    ...
    # X-TianShu-Owner-User-Id, so they are scoped to that owner instead of
    ...
    try:
        ...
        # Merge TianShu-specific context overrides into both ``configurable`` and ``context``.
        ...
        # Per-user MCP server allowlist ("千人千面"): the Settings → Tools page
        # (and the chat input toolbar menu) let a user opt out of the operator's
        # global MCP server set. Resolve the override here (async, DB-backed)
        # and pass it through the run config so make_lead_agent withholds MCP
        # tools from servers the user disabled. Fail-soft: any resolution error
        # inherits the global set rather than breaking the conversation.
        try:
            ...
            inject_mcp_enabled_servers(
                config,
                await resolve_mcp_enabled_servers(get_effective_user_id()),
            )
        except Exception:
            logger.exception("Failed to resolve per-user MCP server allowlist")

        # Per-user MCP server registry ("user_mcp" table): the runtime MCP tool
        # source for user sessions. Resolve the current user's own servers here
        # (async, DB-backed, per-user cached) and inject the built tools through
        # the run config. ``_make_lead_agent`` REPLACES the global MCP tools
        # with these, so system-global ``extensions_config.json`` servers never
        # enter a user session -- user A's tools can never surface in user B's
        # chat. Fail-soft: on any resolution error the user simply gets no MCP
        # tools (never someone else's).
        try:
            from tianshu.mcp.user_registry import (
                USER_MCP_TOOLS_CONTEXT_KEY,
                build_user_mcp_tools,
            )

            context = config.setdefault("context", {})
            context[USER_MCP_TOOLS_CONTEXT_KEY] = await build_user_mcp_tools(
                get_effective_user_id()
            )
        except Exception:
            logger.exception("Failed to resolve per-user MCP tools")

        # Per-user model map ("user_models" table): the Settings → Models page
        # lets a user register their own provider rows. Resolve them here
        # (async, DB-backed, api_key decrypted) and pass them through the run
        # config so ``make_lead_agent`` can instantiate models that are absent
        # from config.yaml. Fail-soft: any resolution error keeps the current
        ...
        try:
            ...
            inject_user_models(
                config,
                await resolve_user_model_configs(get_effective_user_id()),
            )
        except Exception:
            logger.exception("Failed to resolve per-user model allowlist")
