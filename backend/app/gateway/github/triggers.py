...
def _mentions(body: str, login: str) -> bool:
    ...
    ``@tianshu`` would falsely match ``@tianshu-bot`` (a different,
    ...
    character (e.g. ``foo@tianshu`` inside an email address) to avoid
...
def event_should_fire(
    ...
    if trigger.require_mention:
        ...
        # Boundary-aware @-mention match: ``@tianshu`` must NOT match
        # ``@tianshu-bot`` (a distinct, legitimate GitHub login). See
