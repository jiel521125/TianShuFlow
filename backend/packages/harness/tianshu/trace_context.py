...
The value stored here is TianShu's request-level correlation id. It is
separate from Langfuse's own trace id and from TianShu run ids.
"""
...
TIANSHU_TRACE_METADATA_KEY: Final[str] = "tianshu_trace_id"
_MAX_TRACE_ID_LENGTH: Final[int] = 512

_current_trace_id: Final[ContextVar[str | None]] = ContextVar("tianshu_current_trace_id", default=None)
_trace_id_from_request_header: Final[ContextVar[bool]] = ContextVar(
    "tianshu_trace_id_from_request_header",
    default=False,
)
...
def resolve_tianshu_trace_id(metadata_trace_id: object) -> str | None:
    """Resolve the effective ``tianshu_trace_id`` for a run.

    When Gateway ``TraceMiddleware`` bound a valid inbound ``X-Trace-Id``,
    that value wins over ``config.metadata.tianshu_trace_id`` so logs,
