...
from tianshu.agents.lead_agent.prompt import apply_prompt_template
from tianshu.agents.middlewares.clarification_middleware import ClarificationMiddleware
from tianshu.agents.middlewares.configured_extensions import load_configured_extension_middlewares
from tianshu.agents.middlewares.loop_detection_middleware import LoopDetectionMiddleware
from tianshu.agents.middlewares.memory_middleware import MemoryMiddleware
from tianshu.agents.middlewares.model_length_finish_reason_middleware import ModelLengthFinishReasonMiddleware
from tianshu.agents.middlewares.safety_finish_reason_middleware import SafetyFinishReasonMiddleware
from tianshu.agents.middlewares.subagent_limit_middleware import SubagentLimitMiddleware
from tianshu.agents.middlewares.summarization_middleware import TianShuSummarizationMiddleware, create_summarization_middleware
from tianshu.agents.middlewares.terminal_response_middleware import TerminalResponseMiddleware
from tianshu.agents.middlewares.title_middleware import TitleMiddleware
from tianshu.agents.middlewares.todo_middleware import TodoMiddleware
from tianshu.agents.middlewares.token_usage_middleware import TokenUsageMiddleware
from tianshu.agents.middlewares.tool_error_handling_middleware import build_lead_runtime_middlewares
from tianshu.agents.middlewares.view_image_middleware import ViewImageMiddleware
from tianshu.agents.thread_state import get_thread_state_schema, normalize_middleware_state_schemas
from tianshu.authz.principal import build_principal_from_context
...
from tianshu.config.agents_config import load_agent_config, validate_agent_name
from tianshu.config.app_config import AppConfig, get_app_config
from tianshu.config.memory_config import should_use_memory_tools
from tianshu.config.subagents_config import DEFAULT_MAX_TOTAL_SUBAGENTS_PER_RUN
...
from tianshu.skills.types import Skill
...
def _append_memory_tools_without_name_conflicts(tools: list) -> None:
    ...
    from tianshu.agents.memory.tools import get_memory_tools
...
def _apply_user_mcp_tools(raw_tools: list, cfg: dict) -> list:
    """Replace global MCP tools with the current user's own registry tools.

    The gateway injects the user's tools (built from the ``user_mcp`` table)
    into ``cfg[USER_MCP_TOOLS_CONTEXT_KEY]``. When present, every tool tagged
    as MCP is dropped from *raw_tools* and the user's own tools are added --
    system-global ``extensions_config.json`` servers never enter a user
    session ("运行时来源唯一"). When absent (``None``), the legacy
    behaviour is kept (callers without the gateway injection path).
    """
    from tianshu.mcp.user_registry import USER_MCP_TOOLS_CONTEXT_KEY
    from tianshu.tools.mcp_metadata import is_mcp_tool

    user_tools = cfg.get(USER_MCP_TOOLS_CONTEXT_KEY)
    if user_tools is None:
        return raw_tools
    return [tool for tool in raw_tools if not is_mcp_tool(tool)] + list(user_tools)
...
def _create_summarization_middleware(*, app_config: AppConfig | None = None, run_model_name: str | None = None) -> TianShuSummarizationMiddleware | None:
...
def _create_todo_list_middleware(is_plan_mode: bool) -> TodoMiddleware | None:
    ...
    # Custom prompts matching TianShu's style
    system_prompt = """
<todo_list_system>
...
def build_middlewares(
    ...
    *,
    ...
    ``make_lead_agent`` and by the embedded ``TianShuClient`` (a lead-agent variant
    ...
    # Build fallback model chain for cross-model automatic fallback.
    # The primary model is created by the caller (make_lead_agent) and passed
    # to create_agent directly; here we only build the fallback candidates so
    # TianShuModelFallbackMiddleware can switch to them when the primary fails.
    from tianshu.models.factory import _build_fallback_chain

    fallback_models = _build_fallback_chain(
    ...
    from tianshu.agents.middlewares.dynamic_context_middleware import DynamicContextMiddleware
    ...
    from tianshu.agents.middlewares.skill_activation_middleware import SkillActivationMiddleware
    ...
    from tianshu.agents.middlewares.skill_tool_policy_middleware import SkillToolPolicyMiddleware
    ...
    from tianshu.agents.middlewares.durable_context_middleware import DurableContextMiddleware
    ...
    if should_use_memory_tools(resolved_app_config.memory):
        from tianshu.agents.memory.manager import backend_requires_passive_writes_in_tool_mode
    ...
    if deferred_setup is not None and deferred_setup.deferred_names:
        from tianshu.agents.middlewares.deferred_tool_filter_middleware import DeferredToolFilterMiddleware

        middlewares.append(DeferredToolFilterMiddleware(deferred_setup.deferred_names, deferred_setup.catalog_hash))
        from tianshu.agents.middlewares.mcp_routing_middleware import assert_mcp_routing_before_deferred_filter
    ...
    from tianshu.agents.middlewares.system_message_coalescing_middleware import SystemMessageCoalescingMiddleware
    ...
    if token_budget_config.enabled:
        from tianshu.agents.middlewares.token_budget_middleware import TokenBudgetMiddleware
...
def _load_enabled_available_skills(available_skills: set[str] | None, *, app_config: AppConfig, user_id: str | None = None) -> list[Skill]:
    try:
        from tianshu.agents.lead_agent.prompt import get_enabled_skills_for_config
...
def _make_lead_agent(config: RunnableConfig, *, app_config: AppConfig):
    ...
    from tianshu.tools import get_available_tools
    from tianshu.tools.builtins import setup_agent, update_agent
    from tianshu.tools.builtins.tool_search import assemble_deferred_tools, build_mcp_routing_middleware, get_mcp_routing_hints_prompt_section
    from tianshu.tools.mcp_filter import MCP_ENABLED_SERVERS_CONTEXT_KEY, filter_mcp_tools
    from tianshu.models.factory_user import USER_MODELS_CONTEXT_KEY
    ...
    from tianshu.runtime.user_context import resolve_config_user_id
    ...
    # User-registered models (from the ``user_models`` table) are injected
    # into ``config["context"][USER_MODELS_CONTEXT_KEY]`` by the gateway's
    # async layer. ``config.yaml`` has no row for them, so honour them here
    # instead of letting ``_resolve_model_name`` silently swap them for the
    # config.yaml default model.
    user_model_configs = cfg.get(USER_MODELS_CONTEXT_KEY) or {}
    requested = requested_model_name or agent_model_name
    if requested and requested in user_model_configs:
        model_name = requested
    else:
        # Final model name resolution: request → agent config → global default, with fallback for unknown names
        model_name = _resolve_model_name(requested, app_config=resolved_app_config)
    ...
    if model_config is None and model_name in user_model_configs:
        # The requested model only exists in the user's ``user_models`` row;
        # the ``create_chat_model(model_config=...)`` call below instantiates it.
        user_model_config = user_model_configs[model_name]
    ...
    from tianshu.skills.describe import build_skill_search_setup
    ...
    if is_bootstrap:
        ...
        raw_tools = _apply_user_mcp_tools(
            get_available_tools(model_name=model_name, subagent_enabled=subagent_enabled, app_config=resolved_app_config) + [setup_agent],
            cfg,
        )
        raw_tools = filter_mcp_tools(raw_tools, cfg.get(MCP_ENABLED_SERVERS_CONTEXT_KEY))
        ...
        return create_agent(
            model=create_chat_model(name=model_name, thinking_enabled=thinking_enabled, app_config=resolved_app_config, attach_tracing=False, model_config=user_model_config),
            tools=final_tools,
    ...
    raw_tools = _apply_user_mcp_tools(
        get_available_tools(model_name=model_name, groups=agent_config.tool_groups if agent_config else None, subagent_enabled=subagent_enabled, app_config=resolved_app_config),
        cfg,
    )
    raw_tools = filter_mcp_tools(raw_tools, cfg.get(MCP_ENABLED_SERVERS_CONTEXT_KEY))
    ...
    return create_agent(
        model=create_chat_model(name=model_name, thinking_enabled=thinking_enabled, reasoning_effort=reasoning_effort, app_config=resolved_app_config, attach_tracing=False, model_overrides=agent_model_overrides, model_config=user_model_config),
        tools=final_tools,
        middleware=normalize_middleware_state_schemas(
            build_middlewares(
                config,
