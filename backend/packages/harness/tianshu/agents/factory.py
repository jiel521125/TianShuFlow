"""Pure-argument factory for TianShu agents.

``create_tianshu_agent`` accepts plain Python arguments — no YAML files, no
...
from tianshu.agents.features import RuntimeFeatures
...
from tianshu.agents.middlewares.tool_error_handling_middleware import ToolErrorHandlingMiddleware
from tianshu.agents.thread_state import adapt_state_schema_for_mode, get_thread_state_schema, normalize_middleware_state_schemas
from tianshu.config.database_config import CheckpointChannelMode
from tianshu.tools.builtins import ask_clarification_tool
...
def create_tianshu_agent(
    ...
    *,
    ...
    """Create a TianShu agent from plain Python arguments.
    ...
    if checkpoint_channel_mode == "delta" and checkpointer is not None:
        raise ValueError(
            "create_tianshu_agent does not support checkpoint_channel_mode='delta' with a checkpointer: "
            "persisted graphs built here bypass checkpoint mode marker injection and the fail-closed "
            "compatibility gate (see tianshu.runtime.checkpoint_mode), so a mixed-mode store would "
            "silently corrupt thread state.  Use the guarded application paths (make_lead_agent or "
            "TianShuClient) for delta persistence; delta without a checkpointer is ephemeral and allowed."
        )
    ...
    *,
    ...
    if feat.memory is not False:
        if isinstance(feat.memory, AgentMiddleware):
            ...
            from tianshu.config.memory_config import get_memory_config, should_use_memory_tools
    ...
    if feat.vision is not False:
        ...
        if feat.sandbox is not False:
            from tianshu.tools.builtins import view_image_tool
    ...
    if feat.subagent is not False:
        ...
        from tianshu.tools.builtins import task_tool
