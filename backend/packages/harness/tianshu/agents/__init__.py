...
def __getattr__(name: str):
    if name == "create_tianshu_agent":
        from .factory import create_tianshu_agent

        globals()[name] = create_tianshu_agent
        return create_tianshu_agent
    if name == "make_lead_agent":
        from .lead_agent import make_lead_agent
        from .lead_agent.prompt import prime_enabled_skills_cache

        # LangGraph resolves tianshu.agents:make_lead_agent when registering
