...
``backend_config`` dict. The ONLY ``from tianshu`` import allowed in this
folder is the ABC contract line below -- change that one line to port the
backend to another agent. Do NOT import tian-shu path helpers, config
...
Writing a new backend:
  ...
  2. ``config.py``: declare your config knobs + ``from_backend_config`` (parse
     ``backend_config``; read ``storage_path`` from it, NOT from tian-shu).
...
# ABC contract -- the ONE allowed `from tianshu` in this backend folder.
