...
DeerMem wraps the TianShu memory machinery (the five ``core/`` modules:
storage / queue / updater / prompt / message_processing) behind the
backend-neutral :class:`~tianshu.agents.memory.manager.MemoryManager`
...
from tianshu.agents.memory.manager import MemoryConflictError, MemoryCorruptionError, MemoryManager
...
def _resolve_agent_name(agent_name: str | None) -> str:
    """Return TianShu's case-insensitive canonical agent identifier."""
