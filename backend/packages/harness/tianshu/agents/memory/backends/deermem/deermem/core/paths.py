"""DeerMem's own storage path resolution (no tian-shu ``get_paths`` / ``AGENT_NAME_PATTERN``).
...
# agent_name validation (inlined; was tian-shu's AGENT_NAME_PATTERN).
...
def memory_file_path(
    ...
    *,
    ...
    (``$DEERMEM_DATA_DIR`` / ``~/.deermem/``). The host (tian-shu factory)
