...
class DeerMemConfig(BaseModel):
    ...
    storage_path: str = Field(
        default="",
        description=("DeerMem data root (file backend only). Empty = default (``$DEERMEM_DATA_DIR`` or ``~/.deermem/``); per-user memory at ``{root}/users/{user_id}/memory.json``. Ignored by the postgres backend."),
    )
    postgres_url: str = Field(
        default="",
        description="PostgreSQL connection URL for memory state (postgres backend). Required when storage_class is 'postgres'.",
    )
    postgres_schema: str = Field(
        default="",
        description=("PostgreSQL schema holding the memory tables (``memory_documents`` / ``memory_facts``). Empty = 'public'. Only plain identifiers are allowed."),
    )
    storage_class: str = Field(
        default="postgres",
        description=("Storage provider: 'postgres' (default) = PostgresMemoryStorage; 'file' = FileMemoryStorage (legacy); or a dotted class path for an alternative MemoryStorage subclass."),
    )
    ...
    manifest_filename: str = Field(
        default="memory.json",
        description="User-global summary JSON filename (file backend only). Kept under this name for config compatibility; must be a plain .json filename.",
    )
    file_lock_timeout_seconds: int = Field(
        default=10,
        ge=1,
        le=120,
        description="Maximum wait for the per-scope cross-process advisory file lock (file backend only).",
    )
    retrieval_adapter: str = Field(
        default="",
        description=(
            "Retrieval adapter factory (file backend only): 'fts5', an empty string to disable, or a dotted "
            "factory receiving DeerMemConfig and implementing RetrievalPort. The postgres backend searches "
            "SQL natively and ignores this."
        ),
    )
    # ── Queue ────────────────────────────────────────────────────────────
    ...
    host_llm: Any = Field(
        ...
        description=(
            ...
            "UX). The tian-shu factory injects its default model here when "
