...
and set ``manager_class: <name>``. Nothing else in tian-shu changes.
...
class MemoryManager(BaseModel):
    ...
    with the tian-shu conversation thread. The contract is deliberately
    neutral so a third-party memory system can be adapted without tian-shu
    code changes:
...
def _scan_backends() -> dict[str, type[MemoryManager]]:
    ...
    for entry in sorted(_BACKENDS_DIR.iterdir()):
        ...
        if not (entry / "__init__.py").is_file():
            continue
        dotted = f"tianshu.agents.memory.backends.{entry.name}"
        try:
...
# LLM). The portable backend package never names a tian-shu concept; the host
    ...
    def on_memory_llm_call(
        ...
        *,
        ...
        inject_langfuse_metadata(
            ...
            tianshu_trace_id=trace_id,
        )


def _host_default_should_keep_hidden_message(additional_kwargs: Any) -> bool:
    """tian-shu default for DeerMem's ``should_keep_hidden_message`` slot.
...
def _host_default_llm() -> Any:
    """tian-shu default for DeerMem's ``host_llm`` slot (zero-config extraction).
    ...
    try:
        ...
        return create_chat_model(name=None, with_fallbacks=True)
...
def _host_default_extraction_callback(payload: Any) -> None:
    """tian-shu default for DeerMem's ``extraction_callback`` slot.
...
def _collect_host_hooks() -> dict[str, Any]:
    ...
    *consumer* that decides which hooks to use (so adding a backend does not
    ...
    """
    from tianshu.trace_context import request_trace_context
...
def get_memory_manager() -> MemoryManager:
    ...
    # tian-shu is multi-threaded: memory injection runs via asyncio.to_thread,
    ...
    with _manager_lock:
        ...
        storage_class = str(backend_config.get("storage_class") or "postgres")
        if storage_class == "postgres":
            # Zero-config UX: default DeerMem storage to the host's PostgreSQL
            # (same postgres_url as the app database, dedicated memory schema)
            # unless the host explicitly sets postgres_url / postgres_schema.
            if not backend_config.get("postgres_url"):
                ...
                if database.backend != "postgres" or not database.postgres_url:
                    raise ValueError(
                        "memory.backend_config.storage_class='postgres' requires database.backend='postgres' "
                        "with a postgres_url (see the config.yaml database section). Refusing to fall back "
                        "to local files because memory is persistent state."
                    )
                backend_config["postgres_url"] = database.postgres_url
                if not backend_config.get("postgres_schema"):
                    backend_config["postgres_schema"] = f"{(database.postgres_schema or 'tianshu')}_memory"
        else:
            # file backend (legacy): default DeerMem storage to tian-shu's
            # state dir (absolute, CWD-independent) so memory lands at
            # {runtime_home}/users/{user_id}/memory.json (tian-shu's base_dir,
