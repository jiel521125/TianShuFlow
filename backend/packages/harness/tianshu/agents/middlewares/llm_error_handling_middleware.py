...
from tianshu.utils.custom_events import aemit_custom_event, emit_custom_event
...
def _is_fallback_eligible(reason: str, *, fallback_config: Any) -> bool:
    """Return True when this error reason should trigger a cross-model fallback.

    Shared between :class:`LLMErrorHandlingMiddleware` (to decide whether to
    re-raise after retries exhausted) and :class:`TianShuModelFallbackMiddleware`
    (to decide whether to try the next model). ``fallback_config`` is a
    :class:`~tianshu.config.app_config.ModelFallbackConfig` instance.
    """
    if reason == "quota":
        return fallback_config.fallback_on_quota
    if reason == "auth":
        return fallback_config.fallback_on_auth
    if reason in ("transient", "busy", "burst_rate"):
        return fallback_config.fallback_on_transient
    # "circuit_open", "generic" — not eligible
    return False
...
class LLMErrorHandlingMiddleware(AgentMiddleware[AgentState]):
    ...
    def __init__(self, *, app_config: AppConfig, propagate_to_fallback: bool = False, **kwargs: Any) -> None:
        ...
        # When True, re-raise fallback-eligible exceptions after retries are
        # exhausted so the outer TianShuModelFallbackMiddleware can catch them
        # and try a different model. When False (default), preserve the legacy
        # behavior of returning a user-facing fallback AIMessage.
        self._propagate_to_fallback = propagate_to_fallback and app_config.model_fallback.enabled
        ...
        while True:
            try:
                ...
                if retriable and reason != "burst_rate":
                    ...
                    self._release_half_open_probe()
                if self._propagate_to_fallback and _is_fallback_eligible(reason, fallback_config=self._fallback_config):
                    raise exc
