...
from tianshu.agents.middlewares.skill_context import (
    SKILL_CONTEXT_ENTRY_KEY,
    _tool_call_path,
    build_skill_entry_metadata_from_read,
)
from tianshu.agents.middlewares.tool_result_meta import (
    normalize_tool_result,
    stamp_exception_meta,
)
from tianshu.config.app_config import AppConfig
from tianshu.config.summarization_config import DEFAULT_SKILL_FILE_READ_TOOL_NAMES
from tianshu.constants import DEFAULT_SKILLS_CONTAINER_PATH
from tianshu.subagents.status_contract import (
...
def _build_runtime_middlewares(
    *,
    ...
    from tianshu.agents.middlewares.tool_result_sanitization_middleware import ToolResultSanitizationMiddleware
    ...
    # Model fallback middleware (outermost of the error-handling pair) — must
    # wrap LLMErrorHandlingMiddleware so same-model retries exhaust first, then
    # cross-model switching kicks in. Only inserted when fallback models exist.
    # primary_model is optional: the middleware only uses fallback models to
    # switch to; the primary is already bound in the agent's model parameter.
    fallback_cfg = app_config.model_fallback
    has_fallbacks = bool(fallback_models) and fallback_cfg.enabled
    if has_fallbacks:
        from tianshu.agents.middlewares.model_fallback_middleware import TianShuModelFallbackMiddleware

        tail.append(TianShuModelFallbackMiddleware(
            primary=primary_model,
            fallbacks=fallback_models,
            app_config=app_config,
        ))

    tail.append(LLMErrorHandlingMiddleware(
        app_config=app_config,
        propagate_to_fallback=has_fallbacks,
    ))
        ...
        if "framework" not in provider_kwargs:
            try:
                ...
                if "framework" in sig.parameters or any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig.parameters.values()):
                    provider_kwargs["framework"] = "tianshu"
    ...
    # a ToolProgress slot.  The middleware stamps tianshu_tool_meta on the blocked
    ...
    # chain includes ToolErrorHandlingMiddleware (inner), which stamps tianshu_tool_meta
    ...
    if tool_progress_config.enabled:
        from tianshu.agents.middlewares.tool_progress_middleware import ToolProgressMiddleware as _ToolProgressMiddleware
...
def build_subagent_runtime_middlewares(
    *,
    ...
    # TianShuSummarizationMiddleware — subagents inherit none of the lead's
    ...
    from tianshu.agents.middlewares.summarization_middleware import create_summarization_middleware
