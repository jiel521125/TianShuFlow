...
from tianshu.config import get_enabled_tracing_providers
from tianshu.trace_context import TIANSHU_TRACE_METADATA_KEY, get_current_trace_id, normalize_trace_id

# Lazy-imported below to avoid a circular import: ``tianshu.runtime`` eagerly
# imports the run worker, which in turn needs ``tianshu.tracing``.
...
def build_langfuse_trace_metadata(
    *,
    ...
    Args:
        ...
        tianshu_trace_id: Optional TianShu request trace id; falls back to
    ...
    request_trace_id = normalize_trace_id(tianshu_trace_id) or get_current_trace_id()
    if request_trace_id:
        metadata[TIANSHU_TRACE_METADATA_KEY] = request_trace_id
