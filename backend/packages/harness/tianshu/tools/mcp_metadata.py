...
A tool is "MCP-sourced" when it carries the ``tianshu_mcp`` metadata flag.
...
MCP_TOOL_METADATA_KEY = "tianshu_mcp"
MCP_TOOL_SERVER_KEY = "tianshu_mcp_server"
MCP_TOOL_ROUTING_METADATA_KEY = "tianshu_mcp_routing"
...
def tag_mcp_server(tool: BaseTool, server_name: str) -> BaseTool:
    """Record the MCP server that produced ``tool``.

    Used by per-user MCP filtering: when a user opts out of the global MCP
    server set, tools from servers not in their allowlist are withheld at agent
    build time. Mutates in place and returns the tool for chaining.
    """
    tool.metadata = {
        **(tool.metadata or {}),
        MCP_TOOL_SERVER_KEY: server_name,
    }
    return tool


def get_mcp_server(tool: BaseTool) -> str | None:
    """Return the source MCP server name recorded by :func:`tag_mcp_server`."""
    value = (getattr(tool, "metadata", None) or {}).get(MCP_TOOL_SERVER_KEY)
