...
from tianshu.runtime.user_context import AUTO, _AutoSentinel, get_current_user, resolve_user_id
...
class DbRunEventStore(RunEventStore):
    ...
    def _user_id_from_context() -> str | None:
        ...
        is ``VARCHAR(64)`` and a raw UUID object cannot be bound to a
        VARCHAR column — the INSERT would silently roll back and the
        worker would hang.
        """
    ...
    async def put_if_absent(
        ...
        *,
        ...
        every normal writer, so the existence check cannot race another
        ``put_if_absent`` or journal write.
