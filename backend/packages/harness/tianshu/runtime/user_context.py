...
consumer side of this module lives in ``tianshu.persistence.*``):
...
_current_user: Final[ContextVar[CurrentUser | None]] = ContextVar("tianshu_current_user", default=None)
...
def _storage_user_id_from_auth_identity(identity: object | None) -> str | None:
    ...
    # BaseUser.identity, while TianShu's user directories require a narrower
...
def resolve_config_user_id(config: object | None) -> str:
    ...
    values. Gateway runtime context remains the next source for TianShu's
...
def resolve_user_id(
    ...
    *,
    ...
    if isinstance(value, _AutoSentinel):
        ...
        # stores ``user_id`` as ``String(64)`` and cannot bind a raw
        # UUID object to a VARCHAR column. Honour the documented return
        # type here rather than ripple a type change through every caller.
        return str(user.id)
    return value
