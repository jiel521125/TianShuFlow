...
Usage::
    ...
    from tianshu.runtime.checkpointer.provider import get_checkpointer, checkpointer_context
...
from tianshu.config.checkpointer_config import CheckpointerConfig, ensure_config_loaded, get_checkpointer_config
from tianshu.persistence.postgres_schema import dsn_with_search_path, ensure_postgres_schema
from tianshu.runtime.checkpoint_mode import frozen_checkpoint_channel_mode
...
POSTGRES_INSTALL = (
    "langgraph-checkpoint-postgres is required for the PostgreSQL checkpointer. Install the package extra with: pip install 'tianshu-harness[postgres]' (or use: uv sync --all-packages --extra postgres when developing locally)"
)
...
def _resolve_checkpointer_config(app_config: AppConfig) -> CheckpointerConfig:
    ...
    :func:`~tianshu.runtime.checkpointer.async_provider.make_checkpointer`
...
def _wrap_sync_if_delta(saver: Checkpointer, app_config: AppConfig) -> Checkpointer:
    ...
    from tianshu.runtime.checkpoint_cache.provider import checkpoint_cache_key_prefix
