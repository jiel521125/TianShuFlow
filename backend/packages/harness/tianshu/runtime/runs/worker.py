...
from tianshu.agents.goal_state import GoalEvaluation, GoalState
...
from tianshu.runtime.checkpoint_state import (
...
from tianshu.runtime.context_keys import CURRENT_RUN_PRE_EXISTING_MESSAGE_IDS_KEY
from tianshu.runtime.goal import (
...
from tianshu.runtime.serialization import serialize
from tianshu.runtime.stream_bridge import StreamBridge
from tianshu.runtime.stream_modes import normalize_stream_modes, to_langgraph_stream_modes
from tianshu.runtime.user_context import get_effective_user_id, resolve_runtime_user_id
from tianshu.trace_context import (
    TIANSHU_TRACE_METADATA_KEY,
    is_trace_id_from_request_header,
    resolve_tianshu_trace_id,
)
from tianshu.tracing import inject_langfuse_metadata
from tianshu.utils.messages import message_to_text
from tianshu.workspace_changes import capture_workspace_snapshot, get_changed_output_paths, record_workspace_changes
from tianshu.workspace_changes.types import WorkspaceSnapshot
...
def _install_runtime_context(config: dict, runtime_context: dict[str, Any]) -> None:
    ...
    if isinstance(existing_context, dict):
        ...
        if TIANSHU_TRACE_METADATA_KEY in runtime_context:
            existing_context.setdefault(TIANSHU_TRACE_METADATA_KEY, runtime_context[TIANSHU_TRACE_METADATA_KEY])
...
class _SubagentEventBuffer:
    ...
    async def add(self, chunk: Any) -> None:
        ...
        if self._event_store is None:
            return
        # Lazy import: importing tianshu.subagents at module load triggers its
        # package __init__ (executor → agents → tools → task_tool), which imports
        # back from tianshu.subagents and deadlocks at gateway startup. Deferring
        # it to call time (after all modules are loaded) breaks that cycle.
        from tianshu.subagents.step_events import subagent_run_event
...
async def run_agent(
    ...
    *,
    ...
    # this set to mask out ``tianshu_error_fallback`` markers that belong to
    ...
    try:
        ...
        if event_store is not None:
            from tianshu.runtime.journal import RunJournal
        ...
        tianshu_trace_id = resolve_tianshu_trace_id(incoming_metadata.get(TIANSHU_TRACE_METADATA_KEY))
        if tianshu_trace_id:
            runtime_ctx[TIANSHU_TRACE_METADATA_KEY] = tianshu_trace_id
            if is_trace_id_from_request_header():
                merged_metadata = dict(incoming_metadata)
                merged_metadata[TIANSHU_TRACE_METADATA_KEY] = tianshu_trace_id
        ...
        # Shared helper with ``TianShuClient.stream`` so both entry points stay
...
async def _linearize_delta_checkpoint_resume(
    *,
    ...
    Reproduced on postgres and the in-memory saver; ``full`` mode is
...
def _bump_channel_version(checkpointer: Any, current_version: Any) -> Any:
    ...
    DB-backed LangGraph savers (PostgresSaver blob layout)
...
def _try_extract_from_message(obj: Any, pre_existing_ids: set[str] | None = None) -> str | None:
    ...
    if isinstance(additional_kwargs, dict) and additional_kwargs.get("tianshu_error_fallback"):
    ...
    if isinstance(obj, dict):
        ...
        if isinstance(nested_kwargs, dict) and nested_kwargs.get("tianshu_error_fallback"):
