...
from tianshu.runtime.user_context import AUTO, _AutoSentinel, resolve_user_id
from tianshu.utils.time import is_lease_expired
from tianshu.utils.time import now_iso as _now_iso
...
# Transient Postgres failure SQLSTATE values. These are stable across psycopg
# versions and locales: 40001 serialization_failure and 40P01
# deadlock_detected are the classic retryable write conflicts; 57P03
# cannot_connect_now and the 08xxx connection codes cover restart/HA blips.
_RETRYABLE_PGCODES = {
...
# Driver-native unique-constraint signal. It is stable across driver and
# SQLAlchemy versions — message text is not ("duplicate key value violates
# unique constraint" on Postgres).
...
def _is_unique_violation(exc: BaseException) -> bool:
    ...
    reachable via ``exc.orig`` (and ``__cause__`` / ``__context__``). Prefer the
    driver-native signal — psycopg ``pgcode`` / ``sqlcode`` / ``sqlstate`` =
    "23505" — over message matching, then fall back to message substrings for
    cases where the driver exception isn't reachable through the chain.

    Message text drifts across drivers and locales (Postgres raises
    ``duplicate key value violates unique constraint``), so the code/attribute
    check is the load-bearing path.
    """
...
def _is_retryable_persistence_error(exc: BaseException) -> bool:
    """Return True for transient Postgres persistence failures.

    Postgres writer pressure normally surfaces through psycopg errors carrying
    a retryable SQLSTATE (see ``_RETRYABLE_PGCODES``) or SQLAlchemy wrappers.
    The short bounded retry here protects run status finalization from
    transient writer pressure without hiding permanent failures forever.
    """
    ...
    while pending:
        ...
        for code_attr in ("pgcode", "sqlcode", "sqlstate"):
            code = getattr(current, code_attr, None)
...
class RunManager:
    ...
    async def _call_store_with_retry(
        ...
        """Run a short store operation with bounded retries for Postgres pressure."""
    ...
    async def _admit_thread_operation(
        ...
        *,
        ...
        try:
            ...
            cleanup.set_name(f"tianshu-close-cancelled-admission-{record.run_id}")
    ...
    async def start_heartbeat(self) -> None:
        ...
        task.set_name("tianshu-run-lease-heartbeat")
    ...
    def _schedule_orphan_reconciliation(self) -> None:
        ...
        task.set_name("tianshu-periodic-orphan-recovery")
    ...
    async def shutdown(self, *, timeout: float = 5.0) -> None:
        ...
        shutdown (bytedance/tian-shu issue #3373).
