...
from tianshu.agents.thread_state import SandboxState, ThreadDataState, ThreadState
from tianshu.authz.principal import normalize_authz_attributes
...
from tianshu.subagents.config import SubagentConfig, resolve_subagent_model_name
from tianshu.subagents.step_events import capture_new_step_messages
from tianshu.subagents.token_collector import SubagentTokenCollector
from tianshu.trace_context import TIANSHU_TRACE_METADATA_KEY
from tianshu.tracing import build_tracing_callbacks, inject_langfuse_metadata
from tianshu.utils.messages import message_content_to_text
...
if TYPE_CHECKING:
    ...
    # `from tianshu.subagents import SubagentExecutor`, which re-enters this
...
def _extract_llm_error_fallback(final_state: Any) -> str | None:
    ...
    for message in reversed(final_state.get("messages", [])):
        ...
        if metadata.get("tianshu_error_fallback") is not True:
            return None
...
def _copy_isolated_subagent_context() -> Context:
    ...
    if isinstance(callbacks, BaseCallbackManager):
        ...
        isolated_callbacks.handlers = [handler for handler in callbacks.handlers if not getattr(handler, "tianshu_loop_bound", False)]
        isolated_callbacks.inheritable_handlers = [handler for handler in callbacks.inheritable_handlers if not getattr(handler, "tianshu_loop_bound", False)]
    elif isinstance(callbacks, (list, tuple)):
        isolated_callbacks = [handler for handler in callbacks if not getattr(handler, "tianshu_loop_bound", False)]
    elif getattr(callbacks, "tianshu_loop_bound", False):
...
class SubagentExecutor:
    ...
    def __init__(
        ...
        tianshu_trace_id: str | None = None,
    ):
        ...
        Args:
            ...
            tianshu_trace_id: TianShu request-level correlation id propagated
        ...
        self.tianshu_trace_id = tianshu_trace_id
    ...
    def _create_agent(self, tools: list[BaseTool] | None = None, *, deferred_setup: "DeferredToolSetup | None" = None):
        ...
        from tianshu.agents.middlewares.tool_error_handling_middleware import build_subagent_runtime_middlewares
        ...
        if deferred_setup is not None and deferred_setup.deferred_names:
            from tianshu.tools.builtins.tool_search import build_mcp_routing_middleware
        ...
        # Build fallback chain for cross-model automatic fallback.
        from tianshu.models.factory import _build_fallback_chain

        fallback_models = _build_fallback_chain(
            primary_name=self.model_name,
            thinking_enabled=False,
            app_config=app_config,
            attach_tracing=False,
        )
        if fallback_models:
            middleware_kwargs["fallback_models"] = fallback_models
    ...
    async def _build_initial_state(self, task: str) -> tuple[dict[str, Any], list[BaseTool], "DeferredToolSetup"]:
        ...
        from tianshu.tools.builtins.tool_search import assemble_deferred_tools, get_deferred_tools_prompt_section, get_mcp_routing_hints_prompt_section
        ...
        from tianshu.skills.describe import build_skill_search_setup, get_skill_index_prompt_section
    ...
    async def _aexecute(self, task: str, result_holder: SubagentResult | None = None) -> SubagentResult:
        ...
        try:
            ...
            inject_langfuse_metadata(
                ...
                model_name=self.model_name,
                environment=os.environ.get("TIAN_SHU_ENV") or os.environ.get("ENVIRONMENT"),
                tianshu_trace_id=self.tianshu_trace_id,
            )
            ...
            if self.tianshu_trace_id:
                context[TIANSHU_TRACE_METADATA_KEY] = self.tianshu_trace_id
