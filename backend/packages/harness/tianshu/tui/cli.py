"""Command-line entry point and launch-mode planning for the TianShu TUI.
...
headless one-shot. ``main`` wires that decision to the embedded ``TianShuClient``
and lazily imports the Textual app only when actually launching the UI, so the
``tianshu`` console script still runs headless commands without Textual present.
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="tianshu",
        description="TianShu terminal workbench — a TUI over the embedded TianShu harness.",
        add_help=True,
    )
...
def plan_launch(
    ...
    *,
    ...
    if args.cli:
        ...
        return LaunchPlan(
            mode="headless-help",
            reason='--cli needs a message. Try: tianshu --print "your question".',
        )

    forced_tui = bool(args.tui)
    if forced_tui or _truthy(env.get("TIAN_SHU_TUI")) or (stdin_isatty and stdout_isatty):
        return LaunchPlan(
            mode="tui",
            message=positional,
            thread_id=resume,
...
_HEADLESS_HELP = """\
tianshu — TianShu terminal workbench

  tianshu                      launch the terminal UI (TTY required)
  tianshu --tui                force the terminal UI
  tianshu --continue           resume the most recent thread in the UI
  tianshu --resume THREAD      resume a thread by id or title
  tianshu --print "question"   one-shot answer to stdout
  tianshu --json "question"    stream newline-delimited JSON events
  tianshu --recursion-limit N --print "question"
                              set the headless agent-loop super-step limit
  echo "question" | tianshu --print
"""
...
def _run_tui(plan: LaunchPlan) -> int:
    try:
        ...
        # `tianshu.tui.app` module for the forbidden top-level `app` package.
        from tianshu.tui.app import run_tui
    except ModuleNotFoundError as exc:  # textual missing
        if getattr(exc, "name", "") == "textual" or "textual" in str(exc):
            msg = "The terminal UI needs the optional 'textual' dependency.\nInstall it with:  uv pip install 'tianshu-harness[tui]'   (or: pip install textual)\n"
            if plan.forced_tui:
