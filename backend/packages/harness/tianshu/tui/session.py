...
Owns construction of the ``TianShuClient`` (with a persistent checkpointer),
...
class Session:
    ...
    def close(self) -> None:
        ...
        self._loop = None
        try:
            from tianshu.persistence.engine import close_engine
...
def open_session(persistence: bool = True) -> Session:
    ...
    client = TianShuClient(checkpointer=checkpointer)
    if not persistence:
