...
from tianshu.agents.thread_state import ThreadDataState
from tianshu.config import get_app_config
from tianshu.config.paths import VIRTUAL_PATH_PREFIX
...
from tianshu.sandbox.exceptions import (
    SandboxError,
    SandboxNotFoundError,
    SandboxRuntimeError,
)
from tianshu.sandbox.file_operation_lock import get_file_operation_lock
from tianshu.sandbox.overwrite import unwrap_sandbox
from tianshu.sandbox.path_patterns import build_output_mask_pattern
from tianshu.sandbox.sandbox import Sandbox
from tianshu.sandbox.sandbox_provider import get_sandbox_provider
from tianshu.sandbox.search import GrepMatch
from tianshu.sandbox.security import LOCAL_HOST_BASH_DISABLED_MESSAGE, is_host_bash_allowed
...
# Deployments can override via env var TIANSHU_WRITE_FILE_MAX_BYTES; set to
# 0 (or negative) to disable the guard entirely.
_WRITE_FILE_CONTENT_MAX_BYTES = 80 * 1024
_WRITE_FILE_MAX_BYTES_ENV = "TIANSHU_WRITE_FILE_MAX_BYTES"
...
def _get_mcp_allowed_paths() -> list[str]:
    ...
    allowed_paths = []
    try:
        from tianshu.config.extensions_config import get_extensions_config
...
def _compiled_mask_patterns(sources: tuple[tuple[str, str], ...]) -> tuple[tuple[re.Pattern[str], str, str], ...]:
    ...
    # ``tianshu.sandbox.path_patterns``, which owns that rule so the two copies
...
CHANNEL_USER_ID_ENV = "TIANSHU_CHANNEL_USER_ID"
...
def _lark_cli_env_from_runtime(runtime: Runtime, command: str, *, sandbox_paths: bool) -> dict[str, str] | None:
    ...
    Settings authorizes ``lark-cli`` under TianShu's per-user integration
    ...
    if not _LARK_CLI_COMMAND_RE.search(command):
        return None
    try:
        from tianshu.integrations.lark_cli import lark_cli_env_overlay, sandbox_lark_broker_active
...
def _effective_write_file_max_bytes() -> int:
    ...
    Reads ``TIANSHU_WRITE_FILE_MAX_BYTES`` at call time (not import time)
...
def write_file_tool(
    ...
    Operators can override the cap via env var `TIANSHU_WRITE_FILE_MAX_BYTES`
