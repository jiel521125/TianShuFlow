...
Lives at the top-level package (not ``tianshu.runtime``) so it can be
imported from ``tianshu.agents.thread_state`` without pulling in the heavy
``tianshu.runtime`` package __init__ (which eagerly imports the runs
machinery). Anchored from ``tianshu.agents.thread_state`` so every process
that builds a TianShu graph (gateway, workers, in-process LangGraph
...
_PATCH_FLAG = "_tianshu_delta_history_patched"
...
def ensure_inmemory_delta_history_patch() -> None:
    ...
    Both the base implementation and the Postgres override collect the
    terminating checkpoint's writes *before* treating its blob as the seed,
    which is the correct order. This patch delegates
...
_BINOP_PATCH_FLAG = "_tianshu_overwrite_first_write_patched"
...
def _as_overwrite(value: Any) -> tuple[bool, Any]:
    ...
    TianShu's write paths produce for these Union channels (the branch and
    ...
    an internal serialization detail TianShu never emits into these channels.
    """
...
def _binop_first_write_stores_overwrite_wrapper() -> bool:
    ...
    channel.key = "tianshu-overwrite-probe"
