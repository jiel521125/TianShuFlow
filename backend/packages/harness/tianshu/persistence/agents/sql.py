...
and uses its own small engine (see :mod:`tianshu.persistence.agents.base` for
...
The postgres (psycopg) sync driver ships with the app, so this adds no
dependency.
"""
...
from sqlalchemy import Engine, MetaData, create_engine, delete, event, func, select
from sqlalchemy.exc import IntegrityError
from sqlalchemy.orm import Session, sessionmaker

from tianshu.config.agents_config import AgentConfig
from tianshu.config.paths import get_paths
from tianshu.persistence.agents.base import (
    AgentDeleteOutcome,
    AgentExistsError,
    AgentStore,
    parse_agent_config,
...
from tianshu.utils.time import coerce_iso
...
def _build_engine(url: str) -> Engine:
    ...
    if url.startswith("postgresql"):
        # Determine the application schema (default: 'tianshu').
        from tianshu.config.app_config import (
            AppConfig,
        ...
        except Exception:  # noqa: BLE001 — config may not be loaded yet
            cfg = None
        if cfg is not None:
            db = cfg.database
            if db.backend == "postgres":
                schema = (db.postgres_schema or "").strip()

        if schema:

            @event.listens_for(engine, "connect")
            def _set_search_path(dbapi_conn, _record):  # noqa: ARG001
            ...
            # Schema translation at SQL compile time — qualifies every
            # unqualified ORM table reference as `<schema>.<table>`. This is
            # robust against server-side prepared statements that ignore
            # session-level search_path.
            new_metadata = MetaData(schema=schema)
            for table in list(Base.metadata.tables.values()):
                if table.schema is None:
                    table.tometadata(new_metadata, schema=schema)
