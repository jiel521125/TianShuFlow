...
:mod:`tianshu.config.agents_config` dispatch to. ``file`` (default) preserves
...
def make_agent_store(config: AppConfig) -> AgentStore:
    ...
    ``db`` requires ``database.backend`` to be ``postgres``; a
    ...
    if config.agent_storage.backend == "db":
        ...
        if db_backend != "postgres":
            raise ValueError(
                ...
                "share agent definitions across nodes; set database.backend to 'postgres' or use "
