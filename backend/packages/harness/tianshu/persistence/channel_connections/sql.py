...
class ChannelConnectionRepository:
    ...
    async def create_oauth_state_within_cap(
        ...
        *,
        ...
        async with self.session_factory() as session:
            ...
            # POST. Issuing this write first also locks the row the count below
            # reads, so the count cannot race a concurrent inserter between
    ...
    async def _serialize_oauth_owner_scope(self, session: AsyncSession, owner_user_id: str, provider: str) -> None:
        ...
        Takes a transaction-scoped PostgreSQL advisory lock so concurrent
