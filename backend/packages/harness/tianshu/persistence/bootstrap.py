"""Hybrid schema bootstrap for TianShu's application tables.
...
1. ``create_all`` stays the empty-DB fast path -- it renders ``Base.metadata``
   faithfully on the Postgres dialect (JSONB, server
...
| legacy (TianShu tables, no alembic)  | ``create_all`` (baseline tables only, as backfill) + ``stamp 0001_baseline`` + ``upgrade head`` |
...
TianShu-owned table. ``create_all`` runs first because stamping at
...
Postgres has true cross-process serialisation: ``pg_advisory_lock`` runs
...
def _get_alembic_config(engine: AsyncEngine, *, postgres_schema: str = "") -> AlembicConfig:
    ...
    When *postgres_schema* is set it is forwarded as the ``tianshu_pg_schema``
    ...
    if postgres_schema:
        cfg.set_main_option("tianshu_pg_schema", postgres_schema)
    return cfg
...
def _reflect_state(sync_conn: Any) -> dict[str, bool]:
    ...
    - ``has_tianshu_tables``: True iff at least one table that ``Base.metadata``
    ...
    try:
        ...
        logger.debug("tianshu.persistence.models not found; metadata may be incomplete")
    ...
    return {
        ...
        "has_tianshu_tables": bool(reflected & metadata_tables),
    }
...
def _decide_state(state: dict[str, bool]) -> str:
    ...
    if state["has_alembic_version"]:
        return "versioned"
    if not state["has_tianshu_tables"]:
...
def _run_create_all_sync(sync_conn: Any) -> None:
    """Create all TianShu-owned tables on *sync_conn*."""
    ...
    try:
        ...
        logger.debug("tianshu.persistence.models not found; bootstrap will create empty schema")
...
def _run_baseline_create_all_sync(sync_conn: Any) -> None:
    ...
    try:
        ...
        logger.debug("tianshu.persistence.models not found; baseline backfill may be incomplete")
