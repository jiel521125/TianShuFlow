...
from tianshu.runtime.runs.store.base import (
    LeaseRenewal,
    RunStore,
    StatusFinalization,
)
...
class RunRepository(RunStore):
    ...
    def _row_to_dict(row: RunRow) -> dict[str, Any]:
        ...
        # The driver may return naive datetimes despite
        # ``DateTime(timezone=True)`` — ``coerce_iso`` normalizes them as UTC.
    ...
    async def put(
        ...
        *,
        ...
        ``RunManager`` retries ``put`` after transient persistence failures.
        Making this operation idempotent prevents a successful-but-unacknowledged
        first commit from turning the retry into a primary-key failure.
        """
            ...
            if multitask_strategy in ("interrupt", "rollback"):
                ...
                for row in result.scalars():
                    ...
                    if row.lease_expires_at is not None:
                        # The driver may return a naive datetime despite
