"""baseline -- chain root for TianShu application schema.
...
every TianShu-owned table at the point alembic was wired in. Under the hybrid
bootstrap strategy (``tianshu.persistence.bootstrap.bootstrap_schema``), the
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("channel_connections", schema=None) as batch_op:
        ...
        batch_op.create_index("uq_channel_connection_active_identity", ["provider", "external_account_id", "workspace_id"], unique=True, postgresql_where=sa.text("status != 'revoked'"))
    ...
    with op.batch_alter_table("users", schema=None) as batch_op:
        batch_op.create_index("idx_users_oauth_identity", ["oauth_provider", "oauth_id"], unique=True, postgresql_where=sa.text("oauth_provider IS NOT NULL AND oauth_id IS NOT NULL"))
...
def downgrade() -> None:
    ...
    with op.batch_alter_table("users", schema=None) as batch_op:
        ...
        batch_op.drop_index("idx_users_oauth_identity", postgresql_where=sa.text("oauth_provider IS NOT NULL AND oauth_id IS NOT NULL"))
    ...
    with op.batch_alter_table("channel_connections", schema=None) as batch_op:
        batch_op.drop_index("uq_channel_connection_active_identity", postgresql_where=sa.text("status != 'revoked'"))
