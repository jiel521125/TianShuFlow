"""Dialect-aware JSON value matching for SQLAlchemy (PostgreSQL)."""
...
# PostgreSQL overflows when binding values outside the signed 64-bit range.
...
def validate_metadata_filter_value(value: object) -> bool:
    ...
    ``[-2**63, 2**63 - 1]``: PostgreSQL overflows when binding larger values.
    """
...
class JsonMatch(ColumnElement):
    ...
    Compiles to ``json_typeof``/``->>`` on PostgreSQL, with type-safe
    comparison that distinguishes bool vs int and NULL vs missing key.
