...
from tianshu.persistence.json_compat import json_match
from tianshu.persistence.thread_meta.base import THREAD_PINNED_METADATA_KEY, InvalidMetadataFilterError, ThreadMetaStore
...
class ThreadMetaRepository(ThreadMetaStore):
    ...
    def _row_to_dict(row: ThreadMetaRow) -> dict[str, Any]:
        ...
        for key in ("created_at", "updated_at"):
            ...
            if isinstance(val, datetime):
                ...
                # ``DateTime(timezone=True)``; ``coerce_iso`` normalizes naive
                # values as UTC so the wire format always carries tz.
    ...
    async def update_metadata(
        ...
        *,
        ...
        callers cannot replace each other's keys via
