...
Controls BOTH the LangGraph checkpointer and the TianShu application
...
from tianshu.config.postgres_schema import POSTGRES_SCHEMA_PATTERN, validate_postgres_schema
...
class CheckpointCacheConfig(BaseModel):
    ...
    redis_url: str | None = Field(
        default=None,
        description=("Redis URL for type=redis. If omitted, TIAN_SHU_CHECKPOINT_CACHE_REDIS_URL, REDIS_URL, or redis://localhost:6379/0 is used."),
    )
...
class DatabaseConfig(BaseModel):
    backend: Literal["memory", "postgres"] = Field(
        default="memory",
        description=("Storage backend for both checkpointer and application data. 'memory' for development (no persistence across restarts), 'postgres' for production deployments."),
    )
    ...
    postgres_url: str = Field(
        ...
        description=(
            ...
            "Example: postgresql://user:pass@host:5432/tianshu "
    ...
    def app_sync_sqlalchemy_url(self) -> str:
        ...
        async engine cannot be driven. Points at the same database server as
        :meth:`app_sqlalchemy_url`; only the driver differs (psycopg ships with
