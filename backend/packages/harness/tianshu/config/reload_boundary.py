...
Bytedance/tian-shu issue #3144: gateway request dependencies resolve
...
STARTUP_ONLY_FIELDS: dict[str, str] = {
    ...
    "checkpointer": ("make_checkpointer() binds the persistent checkpointer once at startup; the PostgreSQL connection pool is not rebuilt on config.yaml edits."),
    ...
    "log_level": (
        "apply_logging_level() runs only during app.py startup; it sets the tianshu/app logger levels and may lower root handler thresholds so configured messages can propagate. A freshly reloaded AppConfig does not retrigger it."
    ),
    "logging": (
        ...
        "tianshu_trace_id stay coherent. A freshly reloaded AppConfig does not retrigger any of this."
    ),
