...
class ExtensionsConfig(BaseModel):
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path | None:
        ...
        2. If provided `TIAN_SHU_EXTENSIONS_CONFIG_PATH` environment variable, use it.
        ...
        Returns:
            ...
            `TIAN_SHU_EXTENSIONS_CONFIG_PATH` is an operator assertion that
            ...
            MCP tools-cache staleness check in `tianshu.mcp.cache`) rely on
            as a clean, expected signal.

        Raises:
            FileNotFoundError: If `config_path` is given, or
                `TIAN_SHU_EXTENSIONS_CONFIG_PATH` is set, and the resolved
                path does not exist.
        """
        if config_path:
            ...
            return path
        elif env_path := os.getenv("TIAN_SHU_EXTENSIONS_CONFIG_PATH"):
            path = Path(env_path)
            if not path.exists():
                raise FileNotFoundError(f"Extensions config file specified by environment variable `TIAN_SHU_EXTENSIONS_CONFIG_PATH` not found at {path}")
            return path
        else:
