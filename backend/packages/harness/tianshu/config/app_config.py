...
from tianshu.config.acp_config import ACPAgentConfig, load_acp_config_from_dict
from tianshu.config.agent_storage_config import AgentStorageConfig
from tianshu.config.agents_api_config import AgentsApiConfig, load_agents_api_config_from_dict
from tianshu.config.auth_config import AuthAppConfig
from tianshu.config.authorization_config import AuthorizationConfig, load_authorization_config_from_dict
from tianshu.config.channel_connections_config import ChannelConnectionsConfig
from tianshu.config.checkpointer_config import CheckpointerConfig, load_checkpointer_config_from_dict
from tianshu.config.database_config import DatabaseConfig
from tianshu.config.dedupe_storage_config import DedupeStorageConfig
from tianshu.config.extensions_config import ExtensionsConfig
from tianshu.config.file_signature import ConfigSignature as _ConfigSignature
from tianshu.config.file_signature import get_config_signature as _get_config_signature
from tianshu.config.guardrails_config import GuardrailsConfig, load_guardrails_config_from_dict
from tianshu.config.input_polish_config import InputPolishConfig
from tianshu.config.loop_detection_config import LoopDetectionConfig
from tianshu.config.memory_config import MemoryConfig, load_memory_config_from_dict
from tianshu.config.model_config import ModelConfig
from tianshu.config.read_before_write_config import ReadBeforeWriteConfig
from tianshu.config.reload_boundary import format_field_description
from tianshu.config.run_events_config import RunEventsConfig
from tianshu.config.run_ownership_config import RunOwnershipConfig
from tianshu.config.runtime_paths import existing_project_file
from tianshu.config.safety_finish_reason_config import SafetyFinishReasonConfig
from tianshu.config.sandbox_config import SandboxConfig
from tianshu.config.scheduler_config import SchedulerConfig
from tianshu.config.skill_evolution_config import SkillEvolutionConfig
from tianshu.config.skill_scan_config import SkillScanConfig
from tianshu.config.skills_config import SkillsConfig
from tianshu.config.stream_bridge_config import StreamBridgeConfig, load_stream_bridge_config_from_dict
from tianshu.config.subagents_config import SubagentsAppConfig, load_subagents_config_from_dict
from tianshu.config.suggestions_config import SuggestionsConfig
from tianshu.config.summarization_config import SummarizationConfig, load_summarization_config_from_dict
from tianshu.config.title_config import TitleConfig, load_title_config_from_dict
from tianshu.config.token_budget_config import TokenBudgetConfig
from tianshu.config.token_usage_config import TokenUsageConfig
from tianshu.config.tool_config import ToolConfig, ToolGroupConfig
from tianshu.config.tool_output_config import ToolOutputConfig
from tianshu.config.tool_progress_config import ToolProgressConfig
from tianshu.config.tool_search_config import ToolSearchConfig, load_tool_search_config_from_dict
...
class ModelFallbackConfig(BaseModel):
    """Cross-model automatic fallback configuration.

    When the primary model fails with a fallback-eligible error (quota exhausted,
    rate limit, transient 5xx), the system automatically retries on the next
    configured model. This is distinct from same-model retry (handled by
    ``LLMErrorHandlingMiddleware``) and circuit breaker (handled by
    ``CircuitBreakerConfig``).
    """

    enabled: bool = Field(
        default=True,
        description="Auto-fallback to alternative models when the primary exhausts quota/rate-limit. Active only when multiple models are configured.",
    )
    fallback_chain: list[str] = Field(
        default_factory=list,
        description="Ordered fallback model names. Empty = use config.models[1:] (all configured models except the primary, in declaration order).",
    )
    fallback_on_quota: bool = Field(default=True, description="Fallback on quota/billing exhaustion errors.")
    fallback_on_auth: bool = Field(
        default=False,
        description="Fallback on auth errors. Default False — auth errors usually mean misconfiguration. Set True if different models use different keys.",
    )
    fallback_on_transient: bool = Field(default=True, description="Fallback on transient errors (5xx, timeout, burst_rate) after same-model retries exhausted.")
    propagate_to_fallback: bool = Field(
        default=True,
        description="When True, LLMErrorHandlingMiddleware re-raises eligible exceptions so the outer ModelFallbackMiddleware can catch them.",
    )
...
def is_trace_correlation_enabled(config: Any) -> bool:
    ...
    the Gateway ``TraceMiddleware`` and the embedded ``TianShuClient`` so
    the two entry points cannot drift on when ``tianshu_trace_id`` is
...
def apply_logging_level(name: str | None) -> None:
    """Resolve *name* to a logging level and apply it to the ``tianshu``/``app`` logger hierarchies.

    Only the ``tianshu`` and ``app`` logger levels are changed so that
    ...
    for logger_name in ("tianshu", "app"):
...
class AppConfig(BaseModel):
    """Config for the TianShu application"""

    log_level: str = Field(
        default="info",
        description=format_field_description(
            "log_level",
            field_doc="Logging level for tianshu and app modules (debug/info/warning/error); third-party libraries are not affected.",
        ),
    )
    ...
    model_fallback: ModelFallbackConfig = Field(default_factory=ModelFallbackConfig, description="Cross-model automatic fallback configuration")
    ...
    database: DatabaseConfig = Field(
        ...
        description=format_field_description(
            "database",
            field_doc="Unified database backend for run/feedback metadata (memory or postgres).",
        ),
    )
    ...
    def resolve_config_path(cls, config_path: str | None = None) -> Path:
        ...
        2. If provided `TIAN_SHU_CONFIG_PATH` environment variable, use it.
        ...
        if config_path:
            ...
            return path
        elif os.getenv("TIAN_SHU_CONFIG_PATH"):
            path = Path(os.getenv("TIAN_SHU_CONFIG_PATH"))
            if not Path.exists(path):
                raise FileNotFoundError(f"Config file specified by environment variable `TIAN_SHU_CONFIG_PATH` not found at {path}")
            return path
        else:
    ...
    def _apply_singleton_configs(cls, config: Self, acp_agents: dict[str, ACPAgentConfig]) -> None:
        from tianshu.config.checkpointer_config import get_checkpointer_config
        ...
        if previous_checkpointer_config != config.checkpointer:
            ...
            from tianshu.runtime.checkpointer import reset_checkpointer
            from tianshu.runtime.store import reset_store
...
_current_app_config: ContextVar[AppConfig | None] = ContextVar("tianshu_current_app_config", default=None)
_current_app_config_stack: ContextVar[tuple[AppConfig | None, ...]] = ContextVar("tianshu_current_app_config_stack", default=())
...
def get_app_config() -> AppConfig:
    """Get the TianShu config instance.
