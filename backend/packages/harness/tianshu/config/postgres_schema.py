...
# A ``$``-anchored ``re.match`` would accept a trailing newline (``"tianshu\n"``):
# Python's ``$`` matches just before a single trailing ``\n``, which then
# creates a *quoted* schema literally named ``tianshu\n`` while the *unquoted*
# ``search_path`` folds to ``tianshu`` and misses it, silently landing tables
# in ``public``.
