...
def _default_local_base_dir() -> Path:
    """Return the caller project's writable TianShu state directory."""
    return runtime_home()
...
def _join_host_path(base: str, *parts: str) -> str:
    ...
    path form (for example ``C:\\repo\\backend\\.tian-shu``).  Using
...
class Paths:
    """
    Centralized path configuration for TianShu application data.
    ...
    BaseDir resolution (in priority order):
        ...
        2. TIAN_SHU_HOME environment variable
        3. Caller project fallback: `{project_root}/.tian-shu`
    """
    ...
    def host_base_dir(self) -> Path:
        ...
        Set TIAN_SHU_HOST_BASE_DIR to the host-side path that corresponds to this
        ...
        """
        if env := os.getenv("TIAN_SHU_HOST_BASE_DIR"):
            return Path(env)
    ...
    def base_dir(self) -> Path:
        ...
        if env_home := os.getenv("TIAN_SHU_HOME"):
