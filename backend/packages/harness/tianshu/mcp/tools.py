...
from tianshu.config.extensions_config import ExtensionsConfig, resolve_effective_mcp_routing
from tianshu.config.paths import VIRTUAL_PATH_PREFIX, Paths, get_paths
from tianshu.mcp.client import build_servers_config
from tianshu.mcp.oauth import build_oauth_tool_interceptor, get_initial_oauth_headers
from tianshu.mcp.session_pool import get_session_pool
from tianshu.reflection import resolve_variable
from tianshu.runtime.user_context import resolve_runtime_user_id
from tianshu.tools.mcp_metadata import tag_mcp_routing, tag_mcp_server, tag_mcp_tool
from tianshu.tools.sync import make_sync_tool_wrapper
from tianshu.tools.types import Runtime
...
# references may be absolute paths (POSIX ``/tmp/x`` or Windows ``C:\\x``),
# ``file://`` URIs, or paths relative to the server process cwd (e.g.
# ``temp/page.yml``, ``./shot.png``). Each match is only rewritten when it
# resolves to an existing file inside the thread's user-data tree, so an
# over-eager match is harmless (left untouched).
...
# Windows drive-letter absolute paths (``C:\...`` / ``C:/...``). urlparse
# mistakes the drive letter for a URL scheme (scheme="c"), so these are
# recognized directly before any URL parsing.
_WINDOWS_DRIVE_PATH_RE = re.compile(r"^[A-Za-z]:[\\/]")

# ``file://`` URIs that reference Windows drive paths. Real servers emit the
# canonical ``file:///C:/...`` form, but ``file://C:\...`` (what
# ``f"file://{WindowsPath}"`` produces) and ``file://C:/...`` also occur.
# Canonical form: urlparse → scheme="file", netloc="", path="/C:/...".
# Drive-in-netloc forms: urlparse puts "C" (or the whole "C:\..." with
# backslashes) into netloc. All are normalized to a bare drive path here.
_WINDOWS_FILE_URI_RE = re.compile(r"^file:///?([A-Za-z]:[\\/].*)$", re.IGNORECASE)
...
def _local_path_from_uri(uri: str, *, base_dir: Path | None = None) -> Path | None:
    ...
    Accepts bare paths and ``file://`` URIs, in both POSIX (``/tmp/x``,
    ``file:///tmp/x``) and Windows (``C:\\x``, ``file:///C:/x``) forms. Remote
    URIs (``http``/``https``/``data``/...) return ``None`` so the caller leaves
    them untouched. Relative paths are resolved only when *base_dir* is
    supplied.
    """
    if not uri:
        return None
    if os.name == "nt":
        # Windows drive paths look like a URL scheme to urlparse; handle them
        # (and their ``file://`` wrappers) directly.
        windows_uri = _WINDOWS_FILE_URI_RE.match(uri)
        if windows_uri:
            return _resolve_local_path(unquote(windows_uri.group(1)), base_dir=base_dir)
        if _WINDOWS_DRIVE_PATH_RE.match(uri):
            return _resolve_local_path(uri, base_dir=base_dir)
    try:
        parsed = urlparse(uri)
    except ValueError:
        return None
    ...
    if not raw:
        return None
    return _resolve_local_path(raw, base_dir=base_dir)


def _resolve_local_path(raw: str, *, base_dir: Path | None) -> Path | None:
    """Build an absolute ``Path`` from a raw path string, or ``None``.

    ``Path.is_absolute()`` is False on Windows for POSIX-style rooted paths
    (``/tmp/x`` has a root but no drive), which would make ``file:///tmp/...``
    references from Unix-style MCP servers unreachable. Any drive or root is
    therefore treated as absolute; only drive-less, root-less strings fall back
    to *base_dir*.
    """
    path = Path(raw)
...
def _local_uri_to_virtual_path(
    ...
    *,
    ...
    only thing missing is the virtual prefix the rest of TianShu addresses them
...
async def build_mcp_tools(
    ...
    *,
    tool_interceptors: list[Any] | None = None,
    server_options: Mapping[str, Mapping[str, Any]] | None = None,
) -> list[BaseTool]:
    """Build tagged, session-pool-wrapped MCP tools from *servers_config*.

    Shared by the global entry point (:func:`get_mcp_tools`) and the
    per-user registry (:mod:`tianshu.mcp.user_registry`) so both paths
    exercise exactly the same construction pipeline: adapter tool
    discovery, name canonicalization, MCP tags, routing metadata, stdio
    session pooling, and the sync-invocation patch.

    Args:
        servers_config: Mapping of server name -> langchain-mcp-adapters
            connection params (``transport`` / ``command`` / ``args`` /
            ``env`` / ``url`` / ``headers``).
        tool_interceptors: Interceptors forwarded to the adapter and to
            the session-pool wrapper (OAuth, custom).
        server_options: Optional per-server extras keyed by server name:
            ``tool_name_prefix`` (bool), ``tool_call_timeout``
            (float | None), and ``routing_config`` (a
            ``McpServerConfig | None`` used only for routing metadata).
            Missing entries fall back to the adapter defaults (prefix on,
            no timeout, no routing hints).

    Returns:
        List of LangChain tools from the given MCP servers, or ``[]`` on
        discovery failure (callers treat that as "no MCP tools").
    """
    try:
    ...
    if not servers_config:
        logger.info("No MCP servers to build tools from")
        return []
    ...
    try:
        ...
        async def load_server_tools(server_name: str) -> list[BaseTool]:
            try:
                server_opts = options.get(server_name) or {}
                tool_name_prefix = server_opts.get("tool_name_prefix", True)
                if tool_name_prefix:
        ...
        for source_name, server_tools in zip(servers_config.keys(), tools_by_server, strict=True):
            ...
            server_opts = options.get(source_name) or {}
            tool_name_prefix = server_opts.get("tool_name_prefix", True)
            for tool in server_tools:
                if not _VALID_MCP_TOOL_NAME.fullmatch(tool.name or ""):
                    ...
                    continue
                tag_mcp_tool(tool)
                tag_mcp_server(tool, source_name)
                prefix = f"{source_name}_"
                original_name = tool.name[len(prefix) :] if tool_name_prefix and tool.name.startswith(prefix) else tool.name
                routing = resolve_effective_mcp_routing(server_opts.get("routing_config"), original_name)
                ...
                if transport == "stdio":
                    wrapped_tools.append(
                        _make_session_pool_tool(
                            ...
                            interceptors,
                            tool_call_timeout=server_opts.get("tool_call_timeout"),
                            tool_name_prefix=tool_name_prefix,
                        )
                    )
                else:
                    if server_opts.get("tool_call_timeout") is not None:
                        logger.warning(
        ...
        # Patch tools to support sync invocation, as tianshu client streams synchronously
...
async def get_mcp_tools() -> list[BaseTool]:
    """Get all tools from the system-global enabled MCP servers.

    Reads ``extensions_config.json`` from disk (not the in-memory config)
    so changes made through the Gateway API are reflected immediately,
    then delegates the actual construction to :func:`build_mcp_tools`.

    NOTE: Since the per-user MCP registry (``user_mcp`` table) became the
    runtime tool source for user sessions, this global entry point is no
    longer injected into any user chat. It remains for system-global
    tool loading paths and as the reference implementation.
    """
