...
class MCPSessionPool:
    ...
    async def close_user(self, user_id: str) -> None:
        """Close every session owned by *user_id*.

        Scope keys are ``f"{user_id}:{thread_id}"`` (see
        :func:`tianshu.mcp.tools._make_session_pool_tool`), so a user's
        sessions are exactly those whose scope starts with ``"{user_id}:"``.
        Called on user-MCP CRUD so a config edit is picked up by the next
        tool call instead of reusing a stale pooled session.
        """
        prefix = f"{user_id}:"
        with self._lock:
            keys = [k for k in self._entries if k[1].startswith(prefix)]
            entries = [(self._entries.pop(k)) for k in keys]
            inflight_keys = [k for k in self._inflight if k[1].startswith(prefix)]
            inflight = [self._inflight.pop(k) for k in inflight_keys]
        for _session, loop, task, close_evt in entries:
            await self._shutdown_entry(loop, task, close_evt)
        for loop, _ready, task, close_evt in inflight:
