...
This lets the existing :class:`~tianshu.guardrails.middleware.GuardrailMiddleware`
enforce :class:`~tianshu.authz.provider.AuthorizationProvider` decisions at
tool-call time — no new middleware class required (see RFC §6.1).

The adapter maps :class:`~tianshu.guardrails.provider.GuardrailRequest`
fields to :class:`~tianshu.authz.provider.AuthzRequest` fields, calls the
authorization provider, and converts the :class:`~tianshu.authz.provider.AuthzDecision`
back to a :class:`~tianshu.guardrails.provider.GuardrailDecision`.

Principal construction delegates to
:func:`~tianshu.authz.principal.build_principal_from_context` so Layer 1
...
from tianshu.authz.provider import AuthorizationProvider, AuthzDecision, AuthzRequest
from tianshu.guardrails.provider import GuardrailDecision, GuardrailReason, GuardrailRequest
...
class GuardrailAuthorizationAdapter:
    ...
    def evaluate(self, request: GuardrailRequest) -> GuardrailDecision:
        ...
        adapter is consumed by :class:`~tianshu.guardrails.middleware.GuardrailMiddleware`,
