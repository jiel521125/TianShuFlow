...
Runs each TianShu sandbox as a [Tenki](https://tenki.cloud) cloud sandbox — an
...
```yaml
sandbox:
  use: tianshu.community.tenki:TenkiSandboxProvider
...
```bash
pip install "tianshu-harness[tenki]"
```

The `tenki-sandbox` package is an optional TianShu harness extra, not part of
...
API; directory and content search shell out and reuse `tianshu.sandbox.search`,
...
TianShu's `/mnt/user-data` virtual prefix is remapped under the writable
