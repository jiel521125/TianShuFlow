"""``TenkiSandbox`` — TianShu :class:`Sandbox` backed by a Tenki cloud sandbox.
...
with the shared ``tianshu.sandbox.search`` helpers, the same approach as
...
from tianshu.sandbox.search import GrepMatch, path_matches, should_ignore_path, truncate_line
...
# ``/mnt`` is root-owned, so TianShu's ``/mnt/user-data`` virtual prefix is not
...
class TenkiSandbox(Sandbox):
    """TianShu Sandbox adapter that delegates to a live Tenki cloud sandbox.

    Args:
        id: TianShu-side sandbox id (the provider's cache key).
    ...
    def _resolve_path(self, path: str) -> str:
        """Map TianShu virtual paths into the writable sandbox home dir.
    ...
    def execute_command(
        ...
        TianShu passes a bash command *string*; it runs through ``sh -lc``.
