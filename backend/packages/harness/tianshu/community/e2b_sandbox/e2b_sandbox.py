...
# Where TianShu's ``/mnt/user-data`` virtual prefix is materialised inside
...
class E2BSandbox(Sandbox):
    """TianShu Sandbox adapter that delegates to an e2b cloud sandbox.

    Args:
        id: TianShu-side sandbox id (used as cache key in the provider).
    ...
    def sandbox_id(self) -> str:
        """e2b-side sandbox id (different from TianShu's ``self.id`` cache key)."""
    ...
    def _resolve_path(self, path: str) -> str:
        """Map TianShu virtual paths into the e2b sandbox filesystem.
