...
from tianshu.sandbox.tools import read_file_tool
...
def test_read_file_tool_binary_file_returns_actionable_hint(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("tianshu.sandbox.tools.ensure_sandbox_initialized", lambda runtime: LocalSandbox("t1"))
...
def test_read_file_tool_passes_line_range_into_sandbox(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tianshu.sandbox.tools.ensure_sandbox_initialized", lambda runtime: RangeAwareSandbox())
    monkeypatch.setattr("tianshu.sandbox.tools.ensure_thread_directories_exist", lambda runtime: None)
...
def test_read_file_tool_validates_range_order(monkeypatch) -> None:
    runtime = SimpleNamespace(state={"sandbox": {"sandbox_id": "aio:test"}}, context={"thread_id": "t1"})
    monkeypatch.setattr("tianshu.sandbox.tools.ensure_sandbox_initialized", lambda runtime: object())
