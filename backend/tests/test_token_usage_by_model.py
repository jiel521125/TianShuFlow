...
async def _make_sql_repo(pg_test_engine):
...
@pytest.mark.anyio
async def test_sql_store_by_model_invariant_and_fallback(pg_test_engine):
    repo = await _make_sql_repo(pg_test_engine)
...
@pytest.mark.anyio
async def test_memory_and_sql_stores_agree(pg_test_engine):
    ...
    mem = MemoryRunStore()
    sql = await _make_sql_repo(pg_test_engine)
    await _seed_all(mem)
    await _seed_all(sql)
...
@pytest.mark.anyio
async def test_include_active_picks_up_running_progress_snapshot(pg_test_engine):
