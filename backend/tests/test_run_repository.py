...
from tianshu.runtime import CancelOutcome, RunManager, RunStatus, ThreadOperationKind
from tianshu.runtime.runs.manager import ConflictError
from tianshu.runtime.runs.store.base import RunStore
...
class TestRunRepository:
    @pytest.mark.anyio
    async def test_put_and_get(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_put_is_idempotent_for_retried_writes(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_get_missing_returns_none(self, pg_test_engine):
        repo = await _make_repo()
        assert await repo.get("nope") is None

    @pytest.mark.anyio
    async def test_update_status(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_update_status_returns_false_for_missing_row(self, pg_test_engine):
        repo = await _make_repo()
        updated = await repo.update_status("missing", "error", error="lost")
        assert updated is False

    @pytest.mark.anyio
    async def test_start_run_only_updates_pending_rows(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_update_status_with_error(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1")
    ...
    @pytest.mark.anyio
    async def test_run_history_excludes_internal_thread_operations(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1", status="success")
        await repo.put(
            "checkpoint-write-1",
    ...
    @pytest.mark.anyio
    async def test_list_by_thread_owner_filter(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_delete_nonexistent_is_noop(self, pg_test_engine):
        repo = await _make_repo()
        await repo.delete("nope")  # should not raise

    @pytest.mark.anyio
    async def test_list_pending(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1", status="pending")
    ...
    @pytest.mark.anyio
    async def test_list_inflight_returns_pending_and_running_before_cutoff(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_update_run_completion(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_update_run_completion_returns_false_for_missing_row(self, pg_test_engine):
        repo = await _make_repo()
        updated = await repo.update_run_completion("missing", status="error", total_tokens=1)
        assert updated is False

    @pytest.mark.anyio
    async def test_update_run_completion_does_not_replace_terminal_status(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_metadata_preserved(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_kwargs_with_non_serializable(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_update_run_completion_preserves_existing_fields(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_update_run_progress_keeps_status_running(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1", status="running")
    ...
    @pytest.mark.anyio
    async def test_update_run_progress_preserves_omitted_fields(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1", status="running")
        await repo.update_run_progress(
            "r1",
    ...
    @pytest.mark.anyio
    async def test_update_run_progress_skips_terminal_runs(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1", status="running")
    ...
    @pytest.mark.anyio
    async def test_aggregate_tokens_by_thread_counts_completed_runs_only(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_aggregate_tokens_by_thread_can_include_active_runs(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("success-run", thread_id="t1", status="running")
    ...
    @pytest.mark.anyio
    async def test_list_by_thread_ordered_desc(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_list_by_thread_limit(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_owner_none_returns_all(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put("r1", thread_id="t1", user_id="alice", status="success")
        await repo.put("r2", thread_id="t1", user_id="bob", status="pending")
    ...
    @pytest.mark.anyio
    async def test_model_name_persistence(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_run_manager_hydrates_store_only_run_from_sql(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_run_manager_cancel_persists_interrupted_status_to_sql(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_update_model_name(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_update_model_name_normalizes_value(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_update_model_name_to_none(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_run_manager_update_model_name_persists_to_sql(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_run_manager_update_model_name_twice(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_create_thread_operation_atomic_rejects_unique_violation(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_checkpoint_write_reservation_blocks_interrupt_run_on_sql_store(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_reservation_release_uses_record_user_without_ambient_context(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_interrupt_reclaims_expired_checkpoint_write_reservation_on_sql_store(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_is_unique_violation_detects_real_postgres_integrity_error(self, pg_test_engine):
        """``_is_unique_violation`` must return True for a real Postgres IntegrityError.

        Postgres raises ``duplicate key value violates unique constraint``
        (SQLSTATE 23505) when the partial unique index ``uq_runs_thread_active``
        rejects a second active run on the same thread. This test triggers a real
        violation against the index and feeds the resulting SQLAlchemy
        IntegrityError (with the wrapped driver exception on ``.orig``) through
        the detector to assert True.
        """
        from sqlalchemy.exc import IntegrityError

        from tianshu.runtime.runs.manager import _is_unique_violation
        ...
        # The wrapped driver exception must carry the Postgres unique-violation
        # SQLSTATE (23505) that the detector keys off.
        driver = exc_info.value.orig
        assert driver.sqlstate == "23505"
    ...
    async def test_is_unique_violation_does_not_misclassify_application_exception(self):
        ...
        ``isinstance(current, SAIntegrityError)``.
        """
        from tianshu.runtime.runs.manager import _is_unique_violation
    ...
    @pytest.mark.anyio
    async def test_create_thread_operation_atomic_interrupt_blocked_by_other_worker(self, pg_test_engine):
        """Interrupt path must raise ConflictError when another worker holds a live lease.

        Postgres preserves tzinfo on ``DateTime(timezone=True)`` reads, so the
        aware ``cutoff = datetime.now(UTC) - ...`` comparison in the interrupt
        branch of ``create_thread_operation_atomic`` works as designed: a
        still-valid lease owned by another worker blocks the interrupt with a
        clean ConflictError instead of a raw exception.
        """
        from datetime import UTC, datetime, timedelta
        ...
        # The interrupt path must surface a clean ConflictError, not a raw error.
    ...
    @pytest.mark.anyio
    async def test_claim_for_takeover_succeeds_with_expired_lease(self, pg_test_engine):
        repo = await _make_repo()
        grace = 10
    ...
    @pytest.mark.anyio
    async def test_claim_for_takeover_fails_on_valid_lease(self, pg_test_engine):
        repo = await _make_repo()
        grace = 10
    ...
    @pytest.mark.anyio
    async def test_request_cancel_is_returned_by_owner_lease_renewal(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_request_cancel_rejects_terminal_run(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_cancel_request_wins_before_owner_completion(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put(
            "run-1",
            thread_id="t1",
    ...
    @pytest.mark.anyio
    async def test_owner_completion_wins_before_cancel_request(self, pg_test_engine):
        repo = await _make_repo()
        await repo.put(
            "run-1",
            thread_id="t1",
    ...
    @pytest.mark.anyio
    async def test_reconciliation_skips_run_renewed_after_scan(self, pg_test_engine):
    ...
    @pytest.mark.anyio
    async def test_claim_for_takeover_succeeds_with_null_lease(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_claim_for_takeover_fails_on_terminal_row(self, pg_test_engine):
        repo = await _make_repo()
    ...
    @pytest.mark.anyio
    async def test_claim_for_takeover_nonexistent_run(self, pg_test_engine):
        repo = await _make_repo()
