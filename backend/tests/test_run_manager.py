...
from tianshu.runtime import DisconnectMode, RunManager, RunStatus, ThreadOperationKind
from tianshu.runtime.events.store.memory import MemoryRunEventStore
from tianshu.runtime.runs.manager import CancelOutcome, ConflictError, PersistenceRetryPolicy, RunStartOutcome
...
class FlakyStatusRunStore(MemoryRunStore):
    """Memory run store that simulates transient Postgres status-write failures."""
    ...
    async def update_status(self, run_id, status, *, error=None, stop_reason=None):
        ...
        if self.status_failures > 0:
            ...
            raise psycopg.errors.SerializationFailure("database is locked")
...
class PermanentStatusRunStore(MemoryRunStore):
    ...
    async def update_status(self, run_id, status, *, error=None, stop_reason=None):
        ...
        raise SQLAlchemyDatabaseError(
            ...
            psycopg.errors.UndefinedTable('no such table: runs'),
        )
...
async def test_status_persistence_does_not_retry_permanent_sqlalchemy_errors():
    """Permanent SQLAlchemy failures should not be retried as transient pressure."""
...
async def test_completion_persistence_warns_when_recreated_row_still_missing(caplog):
    ...
    record = await manager.create("thread-1")
    await manager.set_status(record.run_id, RunStatus.success)
    caplog.set_level(logging.WARNING, logger="tianshu.runtime.runs.manager")
...
async def test_list_by_thread(manager: RunManager, monkeypatch: pytest.MonkeyPatch):
    ...
    # Distinct increasing timestamps keep ordering deterministic (wall-clock
    # ties on coarse-resolution clocks would otherwise make this flaky).
    timestamps = iter(
        [
    ...
    monkeypatch.setattr("tianshu.runtime.runs.manager._now_iso", lambda: next(timestamps))
...
async def test_list_by_thread_is_stable_when_timestamps_tie(manager: RunManager, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr("tianshu.runtime.runs.manager._now_iso", lambda: "2026-01-01T00:00:00+00:00")
...
async def test_model_name_create_or_reject():
    ...
    from tianshu.runtime.runs.schemas import DisconnectMode
