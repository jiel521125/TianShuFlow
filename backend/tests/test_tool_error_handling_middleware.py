...
from tianshu.agents.middlewares.tool_error_handling_middleware import (
    ToolErrorHandlingMiddleware,
    build_lead_runtime_middlewares,
    build_subagent_runtime_middlewares,
)
from tianshu.agents.middlewares.tool_result_meta import TOOL_META_KEY
from tianshu.agents.middlewares.view_image_middleware import ViewImageMiddleware
from tianshu.config import summarization_config
from tianshu.config.app_config import AppConfig, CircuitBreakerConfig
from tianshu.config.extensions_config import ExtensionsConfig
from tianshu.config.guardrails_config import GuardrailsConfig
from tianshu.config.model_config import ModelConfig
from tianshu.config.sandbox_config import SandboxConfig
from tianshu.subagents.status_contract import SUBAGENT_ERROR_KEY, SUBAGENT_STATUS_KEY
...
def _stub_runtime_middleware_imports(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setitem(
        sys.modules,
        "tianshu.agents.middlewares.llm_error_handling_middleware",
        _module(
            "tianshu.agents.middlewares.llm_error_handling_middleware",
    ...
    monkeypatch.setitem(
        sys.modules,
        "tianshu.agents.middlewares.thread_data_middleware",
        _module("tianshu.agents.middlewares.thread_data_middleware", ThreadDataMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "tianshu.sandbox.middleware",
        _module("tianshu.sandbox.middleware", SandboxMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "tianshu.agents.middlewares.dangling_tool_call_middleware",
        _module("tianshu.agents.middlewares.dangling_tool_call_middleware", DanglingToolCallMiddleware=FakeMiddleware),
    )
    monkeypatch.setitem(
        sys.modules,
        "tianshu.agents.middlewares.sandbox_audit_middleware",
        _module("tianshu.agents.middlewares.sandbox_audit_middleware", SandboxAuditMiddleware=FakeMiddleware),
    )
...
def test_build_subagent_runtime_middlewares_threads_app_config_to_llm_middleware(monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "tianshu.agents.middlewares.input_sanitization_middleware",
        _module(
            "tianshu.agents.middlewares.input_sanitization_middleware",
    ...
    from tianshu.agents.middlewares.tool_output_budget_middleware import ToolOutputBudgetMiddleware
...
def test_tool_progress_middleware_is_outer_relative_to_error_handling(monkeypatch: pytest.MonkeyPatch):
    ...
    # Only then can it read tianshu_tool_meta stamped by ToolErrorHandlingMiddleware.
    from tianshu.agents.middlewares.tool_progress_middleware import ToolProgressMiddleware
...
def test_lead_runtime_middlewares_thread_app_config_to_tool_error_handling(monkeypatch: pytest.MonkeyPatch):
    monkeypatch.setitem(
        sys.modules,
        "tianshu.agents.middlewares.input_sanitization_middleware",
        _module("tianshu.agents.middlewares.input_sanitization_middleware", InputSanitizationMiddleware=object),
    )
    app_config = _make_app_config()
    _stub_runtime_middleware_imports(monkeypatch)
...
def test_build_lead_runtime_middlewares_orders_thread_data_before_uploads():
    ...
    upload endpoint.
    """
    from tianshu.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from tianshu.agents.middlewares.uploads_middleware import UploadsMiddleware
...
def test_build_lead_runtime_middlewares_chain_order_matches_agents_md():
    ...
    """
    from tianshu.agents.middlewares.dangling_tool_call_middleware import DanglingToolCallMiddleware
    from tianshu.agents.middlewares.input_sanitization_middleware import InputSanitizationMiddleware
    from tianshu.agents.middlewares.llm_error_handling_middleware import LLMErrorHandlingMiddleware
    from tianshu.agents.middlewares.read_before_write_middleware import ReadBeforeWriteMiddleware
    from tianshu.agents.middlewares.sandbox_audit_middleware import SandboxAuditMiddleware
    from tianshu.agents.middlewares.thread_data_middleware import ThreadDataMiddleware
    from tianshu.agents.middlewares.tool_output_budget_middleware import ToolOutputBudgetMiddleware
    from tianshu.agents.middlewares.uploads_middleware import UploadsMiddleware
    from tianshu.sandbox.middleware import SandboxMiddleware
...
def test_subagent_runtime_middlewares_attach_deferred_filter_when_setup_has_names(monkeypatch):
    ...
    from tianshu.agents.middlewares.safety_finish_reason_middleware import SafetyFinishReasonMiddleware
    from tianshu.tools.builtins.tool_search import build_deferred_tool_setup
...
def test_subagent_runtime_middlewares_place_mcp_routing_before_deferred_filter(monkeypatch):
    ...
    from tianshu.tools.builtins.tool_search import DeferredToolSetup
...
def test_subagent_runtime_middlewares_attach_durable_context_before_summarization(monkeypatch):
    ...
    """
    from tianshu.agents.middlewares import summarization_middleware as sm
    from tianshu.agents.middlewares.durable_context_middleware import DurableContextMiddleware
    ...
    from tianshu.config.summarization_config import SummarizationConfig
...
def test_subagent_compaction_injects_summary_before_assistant_tool_tail(monkeypatch):
    ...
    from tianshu.agents.middlewares.durable_context_middleware import DurableContextMiddleware
    from tianshu.agents.middlewares.summarization_middleware import TianShuSummarizationMiddleware
    from tianshu.agents.middlewares.system_message_coalescing_middleware import SystemMessageCoalescingMiddleware
    from tianshu.agents.thread_state import ThreadState
    from tianshu.config.summarization_config import ContextSize, SummarizationConfig
    ...
    monkeypatch.setattr(
        "tianshu.agents.middlewares.summarization_middleware.create_chat_model",
    ...
    compaction_middlewares = [middleware for middleware in runtime_middlewares if isinstance(middleware, (DurableContextMiddleware, TianShuSummarizationMiddleware, SystemMessageCoalescingMiddleware))]
    agent = create_agent(
        model=strict_model,
        tools=[],
...
def test_subagent_runtime_middlewares_omit_summarization_when_factory_returns_none(monkeypatch):
    ...
    assert not any(isinstance(m, TianShuSummarizationMiddleware) for m in middlewares)
...
def test_subagent_summarization_fires_mid_run_and_produces_usable_result(monkeypatch):
    ...
    ``TianShuSummarizationMiddleware`` (the exact instance the subagent chain
    ...
    from tianshu.agents.middlewares.summarization_middleware import (
        TianShuSummarizationMiddleware,
        create_summarization_middleware,
    )
    from tianshu.agents.thread_state import ThreadState
    from tianshu.config.memory_config import MemoryConfig
    from tianshu.config.summarization_config import ContextSize, SummarizationConfig
    ...
    assert isinstance(middleware, TianShuSummarizationMiddleware), "the real middleware must be built"
    ...
    before_model_chunks = [c for c in chunks if "TianShuSummarizationMiddleware.before_model" in c]
    assert before_model_chunks, "summarization before_model must fire when messages exceed the trigger"
    summary_update = before_model_chunks[0]["TianShuSummarizationMiddleware.before_model"]
