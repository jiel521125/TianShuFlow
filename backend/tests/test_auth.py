...
@pytest.mark.anyio
async def test_round_trip_new_fields(pg_test_engine):
    ...
    run_events, and feedback use) on an isolated PostgreSQL schema.
    """
    ...
    repo = PostgresUserRepository(get_session_factory())
    user = User(
...
@pytest.mark.anyio
async def test_update_user_raises_when_row_concurrently_deleted(pg_test_engine):
    ...
    repo = PostgresUserRepository(sf)
    user = User(
...
@pytest.mark.anyio
async def test_email_lookup_is_case_insensitive(pg_test_engine):
...
@pytest.mark.anyio
async def test_create_user_rejects_email_differing_only_in_case(pg_test_engine):
...
@pytest.mark.anyio
async def test_create_user_rejects_legacy_mixed_case_email(pg_test_engine):
...
@pytest.mark.anyio
async def test_update_user_normalizes_email(pg_test_engine):
...
@pytest.mark.anyio
async def test_distinct_emails_remain_distinct(pg_test_engine):
...
@pytest.mark.anyio
async def test_legacy_mixed_case_duplicate_rows_resolve_without_error(pg_test_engine):
...
@pytest.mark.anyio
async def test_update_user_on_legacy_mixed_case_row_does_not_collide(pg_test_engine):
...
@pytest.mark.anyio
async def test_oidc_login_blocked_by_existing_local_account_across_case(pg_test_engine):
    ...
    from tianshu.config.auth_config import OIDCProviderConfig
    from tianshu.persistence.engine import get_session_factory

    provider = LocalAuthProvider(PostgresUserRepository(get_session_factory()))
    await provider.create_user(email="Victim@x.com", password="pw-abc-123!", system_role="user")

    cfg = OIDCProviderConfig(display_name="Test SSO", issuer="https://issuer.example.com", client_id="tian-shu", auto_create_users=True)
