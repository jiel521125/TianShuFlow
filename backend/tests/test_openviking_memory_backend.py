...
from tianshu.agents.memory.backends.openviking.client import OpenVikingAuthenticationError, OpenVikingHttpClient, OpenVikingUnavailableError
from tianshu.agents.memory.backends.openviking.config import OpenVikingConfig
from tianshu.agents.memory.backends.openviking.models import (
    OpenVikingCommitResult,
    OpenVikingIdentity,
    OpenVikingMessage,
    OpenVikingSearchHit,
)
from tianshu.agents.memory.backends.openviking.openviking_manager import OpenVikingMemoryManager
from tianshu.agents.memory.manager import _scan_backends, reset_memory_manager
...
def test_http_client_sends_trusted_identity_and_maps_search(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    identity = OpenVikingIdentity(account="tianshu", user="df_user")

    hits = client.search(identity, "editor preference", top_k=3)

    assert [hit.abstract for hit in hits] == ["Uses Vim."]
    assert requests[0].headers["X-OpenViking-Account"] == "tianshu"
...
def test_http_client_maps_authentication_error(tmp_path: Path) -> None:
    ...
    with pytest.raises(OpenVikingAuthenticationError) as exc_info:
        client.ensure_session(OpenVikingIdentity(account="tianshu", user="df_user"), "session")
