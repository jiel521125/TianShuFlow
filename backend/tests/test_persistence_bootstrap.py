"""Tests for ``tianshu.persistence.bootstrap.bootstrap_schema``.
...
| legacy (TianShu tables, no alembic_version) | create_all (baseline tables only, backfill) + stamp baseline + upgrade head |
| versioned                             | upgrade head                            |

Each test seeds an isolated PostgreSQL schema to the relevant pre-state, runs
...
from tianshu.persistence.bootstrap import (
...
from tianshu.persistence.migrations._helpers import _normalize_default
...
HEAD = "0014_workspaces"
BASELINE = "0001_baseline"

POSTGRES_URL = os.getenv("TIANSHU_TEST_POSTGRES_URL")
if not POSTGRES_URL:
    pytest.skip("set TIANSHU_TEST_POSTGRES_URL to run live PostgreSQL bootstrap tests", allow_module_level=True)


def _async_url() -> str:
    """Async (asyncpg) URL derived from the test PostgreSQL URL."""
    url = POSTGRES_URL or ""
...
def _new_schema() -> str:
    """Create an isolated schema and return its name."""
...
def _make_engine(schema: str):
    """Async engine pinned to *schema* via connection-level search_path."""
...
async def _seed_legacy_missing_channel_tables(engine) -> None:
    ...
    order (credentials/conversations reference channel_connections) with
    ``CASCADE`` so Postgres FK constraints cannot block the drop.
    """
    async with engine.begin() as conn:
        await conn.run_sync(Base.metadata.create_all)
    async with engine.begin() as conn:
...
@asyncio_test
async def test_empty_branch_creates_all_and_stamps_head() -> None:
    schema = _new_schema()
...
@asyncio_test
async def test_legacy_without_column_branch_upgrades() -> None:
    schema = _new_schema()
    engine = _make_engine(schema)
    try:
...
@asyncio_test
async def test_legacy_missing_channel_tables_get_backfilled() -> None:
    schema = _new_schema()
    engine = _make_engine(schema)
    try:
        ...
        # branch (has_tianshu_tables=True, no alembic_version) while the
...
@asyncio_test
async def test_legacy_with_column_branch_upgrade_is_idempotent() -> None:
    schema = _new_schema()
    engine = _make_engine(schema)
    try:
...
async def test_legacy_with_manual_workaround_column_warns_on_drift(
    ...
    try:
        ...
        with caplog.at_level("WARNING", logger="tianshu.persistence.migrations._helpers"):
            await bootstrap_schema(engine, backend="postgres", postgres_schema=schema)
        ...
        drift_warnings = [r for r in caplog.records if r.levelname == "WARNING" and r.name == "tianshu.persistence.migrations._helpers" and "safe_add_column" in r.getMessage() and "token_usage_by_model" in r.getMessage()]
...
def test_type_equivalent_matches_known_dialect_synonyms() -> None:
    from tianshu.persistence.migrations._helpers import _type_equivalent
...
@asyncio_test
async def test_versioned_branch_is_noop_at_head() -> None:
    schema = _new_schema()
    engine = _make_engine(schema)
    try:
...
@asyncio_test
async def test_token_usage_column_parity_between_fresh_and_upgraded() -> None:
    fresh_schema = _new_schema()
    upgraded_schema = _new_schema()
    fresh = _make_engine(fresh_schema)
    upgraded = _make_engine(upgraded_schema)
    try:
        # Fresh DB -> empty branch -> create_all
        await bootstrap_schema(fresh, backend="postgres", postgres_schema=fresh_schema)
        ...
        await bootstrap_schema(upgraded, backend="postgres", postgres_schema=upgraded_schema)
        ...
        _drop_schema(fresh_schema)
        _drop_schema(upgraded_schema)
# ---------------------------------------------------------------------------
...
@asyncio_test
async def test_create_all_and_alembic_upgrade_produce_same_schema() -> None:
    fresh_schema = _new_schema()
    upgraded_schema = _new_schema()
    fresh = _make_engine(fresh_schema)
    upgraded = _make_engine(upgraded_schema)
    try:
        ...
        # versioned-DB upgrade actually takes. ``postgres_schema`` is
        # forwarded so the alembic-spawned engine pins its search_path to the
        # isolated schema instead of ``public``.
        cfg = _get_alembic_config(upgraded, postgres_schema=upgraded_schema)
...
@asyncio_test
async def test_baseline_table_names_constant_matches_0001() -> None:
    schema = _new_schema()
    engine = _make_engine(schema)
    try:
        ...
        # our schema -- the constant is about TianShu-owned baseline tables.
...
@asyncio_test
async def test_baseline_index_names_constant_matches_0001() -> None:
...
@asyncio_test
async def test_legacy_backfill_skips_non_baseline_tables() -> None:
...
@asyncio_test
async def test_legacy_backfill_creates_missing_index_on_existing_table() -> None:
    schema = _new_schema()
    engine = _make_engine(schema)
    try:
...
@asyncio_test
async def test_legacy_backfill_idempotent_when_index_already_exists() -> None:
...
@asyncio_test
async def test_legacy_backfill_rejects_post_baseline_indexes() -> None:
...
@asyncio_test
async def test_legacy_backfill_duplicate_channel_connections_does_not_crash() -> None:
...
class TestDecideState:
    def test_empty(self):
        assert _decide_state({"has_alembic_version": False, "has_tianshu_tables": False}) == "empty"

    def test_empty_with_unrelated_tables(self):
        # LangGraph checkpointer tables present but TianShu has nothing yet.
        # ``has_tianshu_tables`` is derived from the metadata intersection in
        # production, so the only thing the decision function needs is the
        # bool itself.
        assert _decide_state({"has_alembic_version": False, "has_tianshu_tables": False}) == "empty"

    def test_legacy(self):
        assert _decide_state({"has_alembic_version": False, "has_tianshu_tables": True}) == "legacy"

    def test_versioned(self):
        assert _decide_state({"has_alembic_version": True, "has_tianshu_tables": True}) == "versioned"
    ...
    def test_versioned_takes_precedence_over_empty(self):
        ...
        assert _decide_state({"has_alembic_version": True, "has_tianshu_tables": False}) == "versioned"
...
def test_baseline_revision_id_is_known() -> None:
    ...
    migrations_dir = Path(__file__).resolve().parents[1] / "packages/harness/tianshu/persistence/migrations"
    cfg = Config()
