...
This test drives the **entire** FastAPI gateway through ``starlette.testclient.TestClient``:
  ...
  starlette.testclient.TestClient (real ASGI stack)
    ...
    -> /api/v1/auth/register endpoint (real password hash + postgres write)
...
@pytest.fixture
def isolated_tian_shu_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    """Stand up an isolated TianShu data root + config under tmp_path.

    - Sets ``TIAN_SHU_HOME`` so paths land under tmp_path, not the real
      ``.tian-shu`` directory.
    - Stages a copy of the project's ``config.yaml`` (or ``config.example.yaml``
      ...
      ``TIAN_SHU_CONFIG_PATH`` to it, so lifespan boot doesn't depend on the
    ...
    # config to tmp_path and pin ``TIAN_SHU_CONFIG_PATH`` to it.
...
def _reset_process_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    This fixture stands up a full FastAPI app + postgres DB + LangGraph runtime
    ...
    so they pick up our test-only ``TIAN_SHU_HOME`` and postgres config:

    - ``tianshu.config.app_config`` caches the parsed ``config.yaml``.
    - ``tianshu.config.paths`` caches the ``Paths`` singleton derived from
      ``TIAN_SHU_HOME`` at first access.
    - ``tianshu.persistence.engine`` caches the SQLAlchemy engine and
...
def isolated_app(isolated_tian_shu_home: Path, monkeypatch: pytest.MonkeyPatch):
    """Build a fresh FastAPI app inside a clean TIAN_SHU_HOME.

    Each test gets its own TIAN_SHU_HOME (filesystem state) and talks to the
    postgres database configured via ``TIANSHU_TEST_POSTGRES_URL``, with no
...
def test_real_http_create_agent_lands_in_authenticated_user_dir(
    isolated_app: Any,
    isolated_tian_shu_home: Path,
    monkeypatch: pytest.MonkeyPatch,
    windows_selector_loop,
):
    ...
    """
    # ``tianshu.agents.lead_agent.agent`` imports ``create_chat_model`` with
    # ``from tianshu.models import create_chat_model`` at module load time,
    ...
    # ``tianshu.models.create_chat_model`` would be too late.
    ...
    with (
        ...
        register = client.post(
            ...
            json={"email": f"e2e-user-{uuid.uuid4().hex[:8]}@example.com", "password": "very-strong-password-123"},
        )
        ...
        expected_dir = isolated_tian_shu_home / "users" / auth_uid / "agents" / agent_name
        default_dir = isolated_tian_shu_home / "users" / "default" / "agents" / agent_name
        ...
        assert _wait_for_file(expected_dir / "SOUL.md", timeout=15.0), (
            ...
            f"tmp tree: {sorted(str(p.relative_to(isolated_tian_shu_home)) for p in isolated_tian_shu_home.rglob('SOUL.md'))}. "
