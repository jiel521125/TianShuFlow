...
Covers: DI construction (owns storage/updater/queue/llm), the default-postgres
storage backend, ``trace_id`` threading to the optional ``callbacks`` hook,
langfuse being optional, ``hide_from_ui`` default-skip + hook-keep, and
portability -- ``backends/deermem/`` has exactly one ``from tianshu`` line (the
ABC contract) and can be vendored into another agent by copying the folder and
repointing that one line.

Storage defaults to PostgreSQL (matching production): storage-behaviour tests
use ``deermem_pg_backend_config`` (live Postgres; skipped unless
``TIANSHU_TEST_POSTGRES_URL`` is set). The few tests that exercise legacy
file-backend-only behaviour (path layout, vendored zero-dependency runs) pin
``storage_class: file`` explicitly. The LLM is a fake injected onto the updater
...
from tianshu.agents.memory.backends.deermem.deermem.core.message_processing import (
    filter_messages_for_memory,
)
from tianshu.agents.memory.backends.deermem.deermem.core.pg_storage import PostgresMemoryStorage
from tianshu.agents.memory.backends.deermem.deermem.core.storage import FileMemoryStorage
from tianshu.agents.memory.backends.deermem.deermem.core.updater import _trim_facts_to_max
from tianshu.agents.memory.manager import MemoryCallbacks
...
def test_add_swallows_queue_full_so_backpressure_does_not_break_caller(deermem_pg_backend_config, caplog) -> None:
    ...
    dm = _deermem_with_fake_llm(backend_config={**deermem_pg_backend_config, "queue_max_depth": 1})
    ...
    with caplog.at_level(logging.WARNING, logger="tianshu.agents.memory.backends.deermem.deer_mem"):
...
def test_di_construction_owns_dependencies(deermem_pg_backend_config):
    dm = DeerMem(backend_config={**deermem_pg_backend_config, "max_facts": 50})
...
def test_from_config_keeps_backend_config_pure_of_injected_hooks(deermem_pg_backend_config):
    ...
    fake_llm = _FakeLLM()
    backend_config = {**deermem_pg_backend_config, "max_facts": 20}
    ...
    assert dm.backend_config == backend_config
...
def test_no_model_config_runs_non_llm_ops(deermem_pg_backend_config):
    dm = DeerMem(backend_config=deermem_pg_backend_config)  # no model -> no LLM
    assert dm._llm is None
    dm.import_memory(
...
def test_get_context_injects_facts_only_in_middleware_mode(deermem_pg_backend_config):
    backend_config = {**deermem_pg_backend_config, "token_counting": "char"}
    middleware = DeerMem(backend_config=backend_config, mode="middleware")
    middleware.import_memory(
        {
            "user": {
                "workContext": {"summary": "Works on TianShu memory."},
            },
            "history": {
            ...
            "facts": [
                {
                    "id": "fact_tool_only",
                    "content": "Use PostgreSQL for active fact recall.",
                    "category": "constraint",
                    "confidence": 0.9,
                    "source": "manual",
                }
    ...
    assert "Works on TianShu memory." in middleware_context
    assert "Recently redesigned storage." in middleware_context
    assert "Use PostgreSQL for active fact recall." in middleware_context
    assert "Works on TianShu memory." in tool_context
    assert "Recently redesigned storage." in tool_context
    assert "Use PostgreSQL for active fact recall." not in tool_context
    assert "Facts:" not in tool_context


def test_import_without_agent_name_persists_facts_in_default_agent_scope(deermem_pg_backend_config):
    dm = DeerMem(backend_config=deermem_pg_backend_config)
    dm.import_memory(
        {
            "user": {},
            "history": {},
    ...
    # The fact landed in the reserved __default__ agent scope, not a custom one.
    assert [fact["id"] for fact in dm.get_memory(agent_name="__default__", user_id="alice")["facts"]] == ["fact_default_import"]


def test_import_empty_summary_sections_replace_existing_summaries_with_complete_defaults(deermem_pg_backend_config):
    dm = DeerMem(backend_config=deermem_pg_backend_config)
...
def test_trace_id_threads_through_to_callbacks(deermem_pg_backend_config):
    ...
    dm = _deermem_with_fake_llm({**deermem_pg_backend_config, "model": {"provider": "openai", "model": "gpt-x", "api_key": "k", "base_url": "u"}}, callbacks=_RecordingCallbacks())
    dm.add(
        thread_id="t1",
...
def test_default_passive_update_persists_fact_in_default_agent_scope(deermem_pg_backend_config):
    dm = _deermem_with_fake_llm(
        deermem_pg_backend_config,
        payload='{"user":{},"history":{},"newFacts":[{"content":"Default agent fact","category":"context","confidence":0.9,"scope":"user","durability":"durable","authority":"descriptive"}],"factsToRemove":[]}',
    )
    ...
    # The passive update landed in the reserved __default__ agent scope.
    assert [fact["content"] for fact in dm.get_memory(agent_name="__default__", user_id="alice")["facts"]] == ["Default agent fact"]


def test_clear_all_memory_removes_global_summaries_and_every_agent_fact(deermem_pg_backend_config):
    dm = DeerMem(backend_config=deermem_pg_backend_config)
...
def test_scoped_clear_preserves_shared_summaries(deermem_pg_backend_config):
    dm = DeerMem(backend_config=deermem_pg_backend_config)
    imported = dm.get_memory(user_id="alice")
    imported["user"]["workContext"] = {"summary": "shared profile", "updatedAt": "now"}
    dm.import_memory(imported, user_id="alice")
...
def test_callbacks_optional_no_langfuse(deermem_pg_backend_config):
    dm = _deermem_with_fake_llm({**deermem_pg_backend_config, "model": {"provider": "openai", "model": "gpt-x", "api_key": "k", "base_url": "u"}})
...
def test_storage_class_postgres_uses_postgresmemorystorage(deermem_pg_backend_config):
    # default storage_class ('postgres') -> PostgresMemoryStorage directly, no importlib (portable, zero noise)
    dm = DeerMem(backend_config=deermem_pg_backend_config)
    assert dm._config.storage_class == "postgres"
    assert isinstance(dm._storage, PostgresMemoryStorage)


def test_storage_class_file_uses_filememorystorage():
    # explicit storage_class='file' -> legacy FileMemoryStorage
    dm = DeerMem(backend_config={"storage_class": "file"})
    assert dm._config.storage_class == "file"
    assert isinstance(dm._storage, FileMemoryStorage)


def test_portability_only_abc_contract_imports_tianshu():
    """backends/deermem/ has exactly ONE `from tianshu` line: the ABC contract in deer_mem.py."""
    import tianshu.agents.memory.backends.deermem as pkg
    ...
    for p in root.rglob("*.py"):
        for line in p.read_text(encoding="utf-8").splitlines():
            s = line.strip()
            if s.startswith("from tianshu") or s.startswith("import tianshu"):
                tianshu_imports.append((p.relative_to(root).as_posix(), s))
    assert len(tianshu_imports) == 1, tianshu_imports
    assert tianshu_imports[0][0] == "deer_mem.py"
    assert "memory.manager import MemoryConflictError, MemoryCorruptionError, MemoryManager" in tianshu_imports[0][1]
...
def test_portability_vendor_to_other_agent(tmp_path, monkeypatch):
    ...
    run portability (zero tianshu dependency at runtime). The vendored copy must
    not depend on an external PostgreSQL, so it pins the legacy file backend."""
    import importlib
    import shutil
    ...
    contract_import = "from tianshu.agents.memory.manager import MemoryConflictError, MemoryCorruptionError, MemoryManager"
    ...
    try:
        ...
        dm = mod.DeerMem(backend_config={"storage_class": "file"})  # file backend: no external PostgreSQL
        dm.import_memory(
...
def test_per_user_memory_path_matches_host_safe_user_id(tmp_path):
    """Pin the legacy file-backend per-user memory path across the abstraction.
    ...
    """
    from tianshu.config.paths import make_safe_user_id

    user_id = "test-user-123@example.com"
    # storage_path mirrors what the host factory injects (runtime_home / base_dir)
    dm = DeerMem(backend_config={"storage_class": "file", "storage_path": str(tmp_path)})
    ...
    expected_file = tmp_path / "users" / expected_safe / "memory.json"
...
def test_create_fact_trims_to_max_and_signals_eviction(deermem_pg_backend_config):
    ...
    dm = DeerMem(backend_config={**deermem_pg_backend_config, "max_facts": 10})
    for i in range(10):
...
def test_search_survives_non_float_confidence(deermem_pg_backend_config):
...
def test_is_human_clarification_response_matches_host_read():
    ...
    from tianshu.agents.human_input import read_human_input_response
    from tianshu.agents.memory.backends.deermem.deermem.core.message_processing import _is_human_clarification_response
...
def test_build_llm_returns_none_when_no_model_configured():
    ...
    from tianshu.agents.memory.backends.deermem.deermem.config import DeerMemModelConfig
    from tianshu.agents.memory.backends.deermem.deermem.core.llm import build_llm
...
def test_build_llm_degrades_to_none_on_init_failure(caplog):
    ...
    llm_logger = "tianshu.agents.memory.backends.deermem.deermem.core.llm"
...
def test_from_backend_config_warns_on_unknown_keys(caplog):
    ...
    cfg_logger = "tianshu.agents.memory.backends.deermem.deermem.config"
...
def test_from_backend_config_null_values_fall_back_to_defaults():
    ...
    from tianshu.agents.memory.backends.deermem.deermem.config import (
        DeerMemConfig,
        DeerMemModelConfig,
    )
