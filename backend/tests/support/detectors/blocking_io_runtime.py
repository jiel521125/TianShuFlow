"""Strict Blockbuster runtime context scoped to TianShu business code.

Creates a `BlockBuster` instance with `scanned_modules=("app", "tianshu")`
...
sync IO whose caller stack passes through `app.*` or `tianshu.*` raises
`BlockingError`.
...
_SCANNED_MODULES: tuple[str, ...] = ("app", "tianshu")

# Add TianShu-local rules here only when Blockbuster's default rule set misses
...
def detect_blocking_io_strict() -> Iterator[BlockBuster]:
    """Activate Blockbuster scoped to app.* and tianshu.* callers only."""
