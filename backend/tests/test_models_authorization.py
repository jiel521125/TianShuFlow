...
from tianshu.config.authorization_config import AuthorizationConfig
...
def _enable_runtime_authorization(monkeypatch, provider) -> AuthorizationConfig:
    ...
    """
    monkeypatch.setattr(
        "tianshu.agents.lead_agent.agent.resolve_authorization_provider",
...
def test_authorize_model_name_disabled_is_noop():
    ...
    from tianshu.agents.lead_agent.agent import _authorize_model_name
...
# ── TianShuClient._ensure_agent path ─────────────────────────────────
# Regression for willem-bd's Round 4 coverage observation: the embedded/library
# lead-agent construction path (``TianShuClient._ensure_agent``) must enforce
...
def _stub_client_assembly(monkeypatch) -> dict[str, str]:
    ...
    monkeypatch.setattr(
        "tianshu.client.create_chat_model",
        lambda **kwargs: captured.__setitem__("name", kwargs.get("name")) or object(),
    )
    monkeypatch.setattr("tianshu.client.create_agent", lambda **kwargs: object())
    monkeypatch.setattr("tianshu.client.build_middlewares", lambda *args, **kwargs: [])
    monkeypatch.setattr("tianshu.client.TianShuClient._get_tools", staticmethod(lambda *, model_name, subagent_enabled: []))  # noqa: ARG005
    monkeypatch.setattr("tianshu.client.get_enabled_skills_for_config", lambda app_config: [])  # noqa: ARG005
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(
        "tianshu.client.assemble_deferred_tools",
        lambda tools, *, enabled: ([], SimpleNamespace(deferred_names=frozenset())),  # noqa: ARG005
    )
    monkeypatch.setattr("tianshu.client.build_mcp_routing_middleware", lambda *args, **kwargs: None)  # noqa: ARG005
    monkeypatch.setattr("tianshu.client.get_mcp_routing_hints_prompt_section", lambda *args, **kwargs: "")  # noqa: ARG005
    monkeypatch.setattr("tianshu.client.apply_prompt_template", lambda **kwargs: "")  # noqa: ARG005
    monkeypatch.setattr("tianshu.client.get_thread_state_schema", lambda *args, **kwargs: object())  # noqa: ARG005
    monkeypatch.setattr("tianshu.client.normalize_middleware_state_schemas", lambda schemas, mode, freq: [])  # noqa: ARG005
    monkeypatch.setattr("tianshu.client.get_effective_user_id", lambda: "user-123")
    ...
    monkeypatch.setattr(
        "tianshu.authz.tool_filter.resolve_authorization_provider",
...
def _bare_client(app_config):
    """Construct a ``TianShuClient`` without running ``__init__``."""
    from tianshu.client import TianShuClient

    client = TianShuClient.__new__(TianShuClient)
    ...
    # Non-None so ``_ensure_agent`` skips the real (postgres) checkpointer
