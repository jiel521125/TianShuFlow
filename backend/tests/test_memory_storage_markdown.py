...
from tianshu.agents.memory import MemoryCorruptionError
...
from tianshu.agents.memory.backends.deermem.deermem.core import storage as storage_module
from tianshu.agents.memory.backends.deermem.deermem.core.paths import fact_file_path
...
def test_default_manager_read_auto_migrates_global_legacy_facts(storage: FileMemoryStorage, tmp_path: Path) -> None:
    ...
    manager = DeerMem(backend_config={"storage_class": "file", "storage_path": str(tmp_path)})
...
def test_max_facts_is_recomputed_after_competing_create(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    manager = DeerMem(backend_config={"storage_class": "file", "storage_path": str(tmp_path), "max_facts": 10})
