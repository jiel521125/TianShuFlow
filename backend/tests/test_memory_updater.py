...
from tianshu.agents.memory.backends.deermem.deermem.core.prompt import format_conversation_for_update
from tianshu.agents.memory.backends.deermem.deermem.core.storage import (
    MemoryManifestRevisionConflict,
    MemoryStorage,
)
from tianshu.agents.memory.backends.deermem.deermem.core.updater import (
    MemoryUpdater,
...
from tianshu.agents.memory.manager import LangfuseMemoryCallbacks
from tianshu.trace_context import get_current_trace_id, request_trace_context
...
def test_apply_updates_skips_same_batch_duplicates_and_keeps_source_metadata() -> None:
    ...
    update_data = {
        "newFacts": [
            ...
            {**_DURABLE_USER_FACT, "content": "User works on TianShu", "category": "context", "confidence": 0.87},
        ],
    }
...
def test_clear_memory_data_clears_facts_and_preserves_shared_summaries() -> None:
    ...
    memory["user"]["workContext"]["summary"] = "Working on TianShu"
    ...
    assert result["facts"] == []
    assert result["user"]["workContext"]["summary"] == "Working on TianShu"
...
class TestUpdateMemoryStructuredResponse:
    ...
    def test_schema_guard_ignores_invalid_update_fields(self):
        ...
        response = (
            ...
            '{"content": "User works on TianShu", "category": "context", "confidence": 0.91, '
        ...
        assert result is True
        assert [fact["content"] for fact in storage.memory["facts"]] == ["User works on TianShu"]
    ...
    def test_sync_update_memory_returns_false_when_executor_down(self):
        ...
        with (
            patch(
                "tianshu.agents.memory.backends.deermem.deermem.core.updater._SYNC_MEMORY_UPDATER_EXECUTOR.submit",
...
class TestUserIdForwarding:
    ...
    def test_sync_update_injects_tianshu_trace_metadata_when_langfuse_enabled(self, monkeypatch):
        ...
        assert metadata["tianshu_trace_id"] == "memory-trace-1"
...
class TestSyncUpdateBindsTraceContextVar:
    ...
    def test_binds_tianshu_trace_id_into_contextvar(self) -> None:
