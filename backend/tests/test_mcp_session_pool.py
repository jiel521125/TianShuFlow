...
from tianshu.mcp.session_pool import MCPSessionPool, get_session_pool, reset_session_pool
...
async def test_session_pool_tool_wrapping():
    ...
    from tianshu.mcp.tools import _make_session_pool_tool
...
async def test_session_pool_tool_pins_cwd_and_temp_env(tmp_path):
    ...
    from tianshu.mcp.tools import _MCP_TMP_SUBDIR, _make_session_pool_tool
    ...
    with (
        patch("tianshu.mcp.tools.get_paths", return_value=paths),
    ...
    # Directory permission bits are not meaningful on Windows' permission model.
    if os.name != "nt":
...
async def test_session_pool_tool_does_not_override_explicit_tmpdir(tmp_path):
    ...
    # _MCP_TMP_SUBDIR uses forward slashes; the injected path uses the OS
    # separator on Windows, so compare against the normalized suffix.
    assert session_connection["env"]["TMP"].endswith(
        _MCP_TMP_SUBDIR.replace("/", os.sep)
    )
...
async def test_session_pool_tool_skips_fs_work_for_non_stdio_transport(tmp_path):
    ...
    with (
        patch("tianshu.mcp.tools.get_paths", return_value=paths) as get_paths,
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm) as create_session,
    ):
...
async def test_session_pool_tool_skips_after_walk_when_no_text_content(tmp_path):
    ...
    with (
        ...
        patch("tianshu.mcp.tools._changed_workspace_files") as changed_files,
    ):
        wrapped = _make_session_pool_tool(original_tool, "playwright", connection)
        await wrapped.coroutine(runtime=mock_runtime, url="https://example.com")
...
async def test_session_pool_tool_runs_after_walk_when_text_content_present(tmp_path):
    ...
    with (
        patch("tianshu.mcp.tools.get_paths", return_value=paths),
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm),
        patch("tianshu.mcp.tools._changed_workspace_files", return_value=[]) as changed_files,
    ):
        wrapped = _make_session_pool_tool(original_tool, "playwright", connection)
        await wrapped.coroutine(runtime=mock_runtime, url="https://example.com")
...
async def test_session_pool_tool_get_config_fallback():
    ...
    with (
        patch("langchain_mcp_adapters.sessions.create_session", return_value=mock_cm),
        patch("tianshu.mcp.tools.get_config", return_value=fake_config),
    ):
        wrapped = _make_session_pool_tool(original_tool, "server", {"transport": "stdio", "command": "x", "args": []})
...
async def test_http_transport_tools_not_pooled():
    ...
    with (
        patch("tianshu.mcp.tools.ExtensionsConfig.from_file", return_value=extensions_config),
        patch("tianshu.mcp.tools.build_servers_config", return_value=servers_config),
        patch("tianshu.mcp.tools.get_initial_oauth_headers", return_value={}),
        patch("tianshu.mcp.tools.build_oauth_tool_interceptor", return_value=None),
...
async def test_non_stdio_tool_call_timeout_warns_that_it_is_ignored(caplog):
    ...
    from tianshu.config.extensions_config import McpServerConfig
    from tianshu.mcp.tools import get_mcp_tools
    ...
    with (
        ...
        patch("tianshu.mcp.tools.build_oauth_tool_interceptor", return_value=None),
        patch("langchain_mcp_adapters.client.MultiServerMCPClient") as MockClient,
        caplog.at_level(logging.WARNING, logger="tianshu.mcp.tools"),
    ):
        mock_client_instance = MockClient.return_value
...
async def test_mcp_tools_routed_to_source_server_with_prefix_overlap():
    ...
    with (
        ...
        patch("tianshu.mcp.tools.build_oauth_tool_interceptor", return_value=None),
        patch("langchain_mcp_adapters.client.MultiServerMCPClient") as MockClient,
        patch("tianshu.mcp.tools._make_session_pool_tool", side_effect=fake_wrap),
    ):
