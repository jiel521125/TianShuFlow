...
spec = importlib.util.spec_from_file_location("tianshu_detect_uv_extras", DETECT_SCRIPT_PATH)
...
def isolated_cwd(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("TIAN_SHU_CONFIG_PATH", raising=False)
...
def test_section_value_finds_nested_key():
    ...
    assert detect.section_value(yaml_lines, "checkpointer", "type") == "postgres"
...
def test_detect_from_config_memory_backend_returns_no_extras(tmp_path):
    cfg = tmp_path / "config.yaml"
    cfg.write_text("database:\n  backend: memory\n")
...
def test_resolve_extras_respects_explicit_config_path(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("TIAN_SHU_CONFIG_PATH", str(elsewhere))
