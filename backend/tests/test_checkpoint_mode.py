...
def test_inject_delta_mode_sets_internal_key_and_metadata_marker() -> None:
    ...
    assert config["configurable"]["__tianshu_checkpoint_channel_mode"] == "delta"
...
def test_inject_full_mode_does_not_claim_delta_metadata() -> None:
    ...
    assert config["configurable"]["__tianshu_checkpoint_channel_mode"] == "full"
...
@pytest.mark.anyio
async def test_full_mode_accessor_rejects_real_delta_checkpoint() -> None:
    """Fail-closed gate against a real saver, not mocks.

    Seeds a delta checkpoint (marker + LangGraph delta counters) into a real
    InMemorySaver, reopens the thread with a full-mode accessor, and asserts
    every accessor surface raises before state is read or written. This is the
    integration boundary the corruption-prevention design relies on; the
    mock-based tests above cannot catch a wiring regression between the
    accessor, the marker, and a real backend.
    """
    from langchain_core.messages import HumanMessage
    from langgraph.checkpoint.memory import InMemorySaver
    from langgraph.types import Overwrite

    from tianshu.runtime.checkpoint_state import CheckpointStateAccessor, build_state_mutation_graph
...
def test_yaml_mode_change_is_rejected_when_graph_is_reconstructed(tmp_path, monkeypatch: pytest.MonkeyPatch) -> None:
    from tianshu.agents.lead_agent import agent as lead_agent
    ...
    def write_config(mode: str) -> None:
        config_path.write_text(
            "\n".join(
                (
                    "sandbox:",
                    "  use: tianshu.sandbox.local.provider:LocalSandboxProvider",
                    "database:",
...
async def test_gateway_runtime_rejects_mode_different_from_frozen_process(
    ...
    monkeypatch.setattr(
        "tianshu.runtime.checkpointer.async_provider.make_checkpointer",
        resource,
    )
    monkeypatch.setattr("tianshu.runtime.make_stream_bridge", resource)
    monkeypatch.setattr("tianshu.runtime.make_store", resource)
    monkeypatch.setattr(
        "tianshu.persistence.engine.init_engine_from_config",
        noop,
    )
    monkeypatch.setattr("tianshu.persistence.engine.close_engine", noop)
    monkeypatch.setattr(
        "tianshu.persistence.engine.get_session_factory",
        lambda: None,
    )
    monkeypatch.setattr(
        "tianshu.runtime.events.store.make_run_event_store",
        lambda _config: object(),
    )
    monkeypatch.setattr(
        "tianshu.persistence.thread_meta.make_thread_store",
...
def test_direct_langgraph_request_cannot_select_delta_in_full_process(
    ...
    app_config = AppConfig.model_validate(
        {
            "sandbox": {"use": "tianshu.sandbox.local.provider:LocalSandboxProvider"},
