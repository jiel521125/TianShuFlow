...
@pytest.fixture()
async def _pg_auth_backend():
    """Temp-schema Postgres backend for DB-backed auth HTTP tests.

    Register/login/change-password/initialize/logout tests hit the real user
    repository through an ``httpx.AsyncClient`` running in the same event loop
    as the engine, against a throwaway schema (created/dropped per test) to
    keep tests off the live database. Skipped unless
    ``TIANSHU_TEST_POSTGRES_URL`` is set (the repo convention for live
    PostgreSQL tests; see ``tests/conftest.py::pg_test_engine``).
    """
    url = os.getenv("TIANSHU_TEST_POSTGRES_URL")
    if not url:
        pytest.skip("set TIANSHU_TEST_POSTGRES_URL to run live PostgreSQL auth tests")
    ...
    schema = f"pgtest_auth_{uuid.uuid4().hex[:8]}"
...
def _get_async_auth_client(*, base_url: str = "http://test"):
    """httpx AsyncClient for DB-backed auth HTTP tests.

    ``TestClient`` runs requests on its own portal event loop, which cannot
    share the asyncpg engine initialized by the test fixture. An
    ``httpx.AsyncClient`` via ``ASGITransport`` runs in the test's anyio
    loop — the same loop the engine lives in.
    """
    from httpx import ASGITransport, AsyncClient

    return AsyncClient(transport=ASGITransport(app=_make_auth_app()), base_url=base_url)
...
def test_api_auth_me_auth_disabled_returns_synthetic_user(monkeypatch):
    _setup_config()
    monkeypatch.setenv("TIAN_SHU_AUTH_DISABLED", "1")
    client = _get_auth_client()
...
@pytest.mark.anyio
async def test_api_login_bad_credentials_returns_structured_401(_pg_auth_backend):
    """Login with wrong password → 401 with {code: 'invalid_credentials'}."""
    _setup_config()
    async with _get_async_auth_client() as client:
...
@pytest.mark.anyio
async def test_api_login_success_no_token_in_body(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_api_register_duplicate_returns_structured_400(_pg_auth_backend):
    """Register with duplicate email → 400 with {code: 'email_already_exists'}."""
    _setup_config()
    async with _get_async_auth_client() as client:
        email = "dup-contract-test@test.com"
        # First register
        await client.post("/api/v1/auth/register", json={"email": email, "password": "Tr0ub4dor3a"})
        # Duplicate
        resp = await client.post("/api/v1/auth/register", json={"email": email, "password": "AnotherStr0ngPwd!"})
...
def test_session_cookie_policy_persists_on_https():
    ...
    _setup_config()
    request = Request(_make_request_scope(scheme="http", host="internal:8000", headers={"x-forwarded-proto": "https", "x-forwarded-host": "tianshu.example"}))
...
@pytest.mark.anyio
async def test_register_http_cookie_httponly_true_secure_false(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_register_https_cookie_httponly_true_secure_true(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_register_remember_me_false_keeps_access_and_csrf_session_only(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
        resp = await client.post(
            "/api/v1/auth/register",
        ...
        preference_cookies = [h for h in set_cookies if "tianshu_session_persistent=" in h]
        ...
        assert "tianshu_session_persistent=0" in preference_cookies[0].lower()


@pytest.mark.anyio
async def test_login_https_sets_secure_cookie(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_login_remember_me_false_keeps_access_and_csrf_session_only(_pg_auth_backend):
    """remember_me=false should make both access_token and csrf_token session cookies."""
    _setup_config()
    async with _get_async_auth_client(base_url="http://localhost:2026") as client:
...
@pytest.mark.anyio
async def test_login_remember_me_false_over_https_keeps_csrf_session_only(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
...
@pytest.mark.anyio
async def test_login_failure_uses_csrf_fallback_cookie_lifetime_on_https(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
        resp = await client.post(
            "/api/v1/auth/login/local",
...
@pytest.mark.anyio
async def test_login_remember_me_true_keeps_access_and_csrf_max_age_in_lockstep_on_localhost(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_change_password_preserves_session_only_preference(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
...
@pytest.mark.anyio
async def test_change_password_reissues_access_and_csrf_in_lockstep_when_preference_changes(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
...
@pytest.mark.anyio
async def test_initialize_remember_me_false_keeps_access_and_csrf_session_only(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
        resp = await client.post(
...
@pytest.mark.anyio
async def test_logout_clears_access_and_csrf_without_reissuing_csrf(_pg_auth_backend):
    _setup_config()
    async with _get_async_auth_client(base_url="https://tianshu.example") as client:
        await client.post(
            "/api/v1/auth/register",
...
@pytest.mark.anyio
async def test_csrf_cookie_secure_on_https(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_csrf_cookie_not_secure_on_http(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_csrf_cookie_persistent_on_https(_pg_auth_backend):
...
@pytest.mark.anyio
async def test_csrf_cookie_session_only_on_http(_pg_auth_backend):
