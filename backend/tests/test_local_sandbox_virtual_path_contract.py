...
externalises that translation to ``tianshu.sandbox.tools`` via ``thread_data``,
...
def _build_config(skills_dir: Path) -> SimpleNamespace:
    ...
    return SimpleNamespace(
        ...
        sandbox=SandboxConfig(use="tianshu.sandbox.local:LocalSandboxProvider", mounts=[]),
    )
...
def test_is_local_sandbox_accepts_generic_and_per_thread_id_formats():
    from tianshu.sandbox.tools import is_local_sandbox
...
def test_concurrent_acquire_same_thread_yields_single_instance(provider):
    ...
    from tianshu.sandbox.local import local_sandbox as local_sandbox_module
