...
from tianshu.runtime.runs import worker
...
from tianshu.runtime.stream_bridge.memory import MemoryStreamBridge
...
def real_executor_module():
    ...
    conftest.py mocks ``tianshu.subagents.executor`` to break a package-init
    import cycle; by the time this fixture runs every other tianshu module is
    already imported, so a fresh import of the real module is safe.
    """
    original = sys.modules.get("tianshu.subagents.executor")
    sys.modules.pop("tianshu.subagents.executor", None)
    subagents_pkg = sys.modules.get("tianshu.subagents")
    if subagents_pkg is not None and hasattr(subagents_pkg, "executor"):
        delattr(subagents_pkg, "executor")

    module = importlib.import_module("tianshu.subagents.executor")
    ...
    if original is not None:
        sys.modules["tianshu.subagents.executor"] = original
    else:
        sys.modules.pop("tianshu.subagents.executor", None)
    subagents_pkg = sys.modules.get("tianshu.subagents")
    if subagents_pkg is not None and hasattr(subagents_pkg, "executor"):
...
def _build_delegating_parent_graph(executor_module, monkeypatch, *, child_emits_error_fallback: bool = False):
    ...
    message carrying the ``tianshu_error_fallback`` marker (not as its final
        ...
        lambda _state: {
            "messages": [
                AIMessage(
                    ...
                    additional_kwargs={"tianshu_error_fallback": True},
                )
            ]
        },
    )
