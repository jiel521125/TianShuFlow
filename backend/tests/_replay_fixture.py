...
tools:
  - name: ls
    group: file:read
    use: tianshu.sandbox.tools:ls_tool
  - name: read_file
    group: file:read
    use: tianshu.sandbox.tools:read_file_tool
  - name: write_file
    group: file:write
    use: tianshu.sandbox.tools:write_file_tool
...
database:
  backend: postgres
  postgres_url: postgresql://localhost:5432/tianshu
"""
...
def prepare_hermetic_extras(home: Path) -> Path:
    ...
    ``TIAN_SHU_EXTENSIONS_CONFIG_PATH`` at it. Call before starting the gateway.
    """
