        ...
        "detail": [
            {
                ...
                "loc": ["body", field],
                "msg": f"Run option '{field}' is not supported by TianShu",
                "input": value,
                "ctx": {"option": field},
            }
        ]
...
def _sdk_default_payload(method: str) -> dict[str, Any]:
    ...
    if method == "stream":
        ...
        client.runs.stream("thread-id", "tianshu", input={"messages": []})
    else:
        client.runs.http.post = capture_post  # type: ignore[method-assign]
        asyncio.run(client.runs.create("thread-id", "tianshu", input={"messages": []}))
    return captured
