...
TIANSHU_ROOTS={shlex.quote(str(repo_root))}
FAKE_COMMAND={shlex.quote(command)}
FAKE_ARGS={shlex.quote(args)}
FAKE_TIANSHU_PID={1 if tianshu_pid else 0}
...
@pytest.mark.parametrize(
    ("command", "args", "tianshu_pid"),
    [
        ("nginx: worker process", "nginx: worker process", False),
        ("python", "python -m nginx /tmp/tian-shu/docker/nginx/nginx.local.conf", True),
    ],
)
