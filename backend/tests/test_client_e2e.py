"""End-to-end tests for TianShuClient.
...
Only TIAN_SHU_HOME is redirected to tmp_path for filesystem isolation.
...
from tianshu.client import TianShuClient, StreamEvent
...
def e2e_env(tmp_path, monkeypatch):
    ...
    - TIAN_SHU_HOME → tmp_path (all thread data lands in a temp dir)
    - TIAN_SHU_PROJECT_ROOT → repository root (shared skills/config assets
    ...
    monkeypatch.setenv(
        "TIAN_SHU_PROJECT_ROOT",
        str(Path(__file__).resolve().parents[2]),
    )
    monkeypatch.setattr("tianshu.config.paths._paths", None)
    monkeypatch.setattr("tianshu.sandbox.sandbox_provider._default_sandbox_provider", None)
    ...
    monkeypatch.setattr("tianshu.config.app_config._app_config", None)
    monkeypatch.setattr("tianshu.config.app_config._app_config_is_custom", False)
    config = _make_e2e_config()
    monkeypatch.setattr("tianshu.config.app_config._app_config", config)
    monkeypatch.setattr("tianshu.config.app_config._app_config_is_custom", True)
    monkeypatch.setattr("tianshu.client.get_app_config", lambda: config)

    # 3. Disable title generation (extra LLM call, non-deterministic)
    from tianshu.config.title_config import TitleConfig

    monkeypatch.setattr("tianshu.config.title_config._title_config", TitleConfig(enabled=False))

    # 4. Disable memory queueing (avoids background threads & file writes)
    from tianshu.config.memory_config import MemoryConfig

    monkeypatch.setattr(
        "tianshu.agents.middlewares.memory_middleware.get_memory_config",
    ...
    monkeypatch.setattr("tianshu.config.summarization_config._summarization_config", SummarizationConfig(enabled=False))
    ...
    from tianshu.agents.lead_agent.agent import build_middlewares as _original_build_middlewares
    ...
    monkeypatch.setattr("tianshu.client.build_middlewares", _sync_safe_build_middlewares)
...
@pytest.fixture()
def client(e2e_env):
    """A TianShuClient wired to the isolated e2e_env."""
    return TianShuClient(checkpointer=None, thinking_enabled=False)
...
class TestFileUploadIntegration:
    ...
    def test_upload_files(self, e2e_env, tmp_path):
        ...
        c = TianShuClient(checkpointer=None, thinking_enabled=False)
        tid = str(uuid.uuid4())
...
class TestLifecycleAndConfig:
    ...
    def test_plan_mode_config_key(self, e2e_env):
        ...
        c = TianShuClient(checkpointer=None, plan_mode=False)
        ...
        c2 = TianShuClient(checkpointer=None, plan_mode=True)
...
class TestSkillInstallation:
    ...
    def _allow_skill_security_scan(self, monkeypatch):
        ...
        monkeypatch.setattr("tianshu.skills.installer.scan_skill_content", _scan)
    ...
    def _isolate_skills_dir(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(
            "tianshu.skills.storage._default_skill_storage",
            local_storage,
        )
        monkeypatch.setattr(
            "tianshu.client.get_or_new_user_skill_storage",
