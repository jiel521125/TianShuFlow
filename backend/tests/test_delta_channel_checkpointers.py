...
Every contract runs against InMemorySaver and - when ``TEST_POSTGRES_URI``
is set - AsyncPostgresSaver, because each backend implements blob/version
...
from tianshu.agents.goal_state import GoalState
from tianshu.agents.thread_state import DeltaThreadState, merge_message_writes
from tianshu.runtime.checkpoint_mode import CHECKPOINT_MODE_METADATA_KEY, checkpoint_tuple_uses_delta
from tianshu.runtime.checkpoint_state import CheckpointStateAccessor
from tianshu.runtime.goal import write_thread_goal
from tianshu.runtime.runs.worker import _ensure_interrupted_title, persist_run_durations
...
class _SaverEnv:
    ...
    Reopening swaps in a brand-new saver over the same bytes (Postgres schema)
    so replay-after-reopen contracts prove persistence, not in-process caching.
    InMemorySaver keeps no external bytes, so reopen is a no-op stand-in there.
    """
...
async def saver_env(request: pytest.FixtureRequest) -> AsyncIterator[_SaverEnv]:
    kind = request.param
    if kind == "memory":
        saver = InMemorySaver()
...
async def test_delta_storage_shape_and_snapshot_cadence(saver_env: _SaverEnv) -> None:
    ...
    # resolves the versioned blob on the latest checkpoint, AsyncPostgresSaver
...
# ---------------------------------------------------------------------------
# InMemorySaver delta-history patch guards (tianshu.checkpoint_patches)
# ---------------------------------------------------------------------------
...
def test_inmemory_delta_history_patch_is_active() -> None:
    ...
    from tianshu import checkpoint_patches
