...
Run explicitly with real credentials:
    ...
    RUN_TIANSHU_LEDGER_LIVE=1 PYTHONPATH=. uv run pytest tests/test_delegation_ledger_live.py -v -s
"""
...
from tianshu.config.app_config import reload_app_config, reset_app_config, set_app_config
...
elif os.environ.get("RUN_TIANSHU_LEDGER_LIVE") != "1":
    _skip_reason = "Set RUN_TIANSHU_LEDGER_LIVE=1 to run this real-model test"
...
def real_subagent_executor():
    ...
    task_tool_module = sys.modules.get("tianshu.tools.builtins.task_tool")
    ...
    if original_executor_module is not None:
        sys.modules["tianshu.subagents.executor"] = original_executor_module
    else:
        sys.modules.pop("tianshu.subagents.executor", None)
...
def _stream_events(client: TianShuClient, thread_id: str, prompt: str) -> list[StreamEvent]:
...
def _state_values(client: TianShuClient, thread_id: str) -> dict[str, Any]:
