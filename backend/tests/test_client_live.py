"""Live integration tests for TianShuClient with real external APIs.
...
For direct pytest invocation, set the explicit opt-in flag:
    ...
    TIAN_SHU_RUN_LIVE_TESTS=1 PYTHONPATH=. uv run pytest tests/test_client_live.py -v -s
"""
...
_LIVE_TEST_OPT_IN = "TIAN_SHU_RUN_LIVE_TESTS"
...
def client():
    """Create a real TianShuClient (no mocks)."""
    return TianShuClient(thinking_enabled=False)
