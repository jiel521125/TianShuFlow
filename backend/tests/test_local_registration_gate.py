...
from tianshu.config.auth_config import AuthAppConfig, LocalAuthConfig

_TEST_SECRET = "test-secret-key-for-registration-gate-tests-only"


@pytest.fixture(autouse=True)
async def _persistence_engine(pg_test_engine):
    """Per-test PostgreSQL schema + clean ``deps._cached_*`` (mirrors test_auth_type_system).

    These tests drive the real ``/register`` handler, which reaches
    ``PostgresUserRepository`` → ``get_session_factory``. A fresh schema per test also means
...
@pytest.fixture
async def client(monkeypatch, _persistence_engine):
    """AsyncClient factory whose gate reads a config we control.
    ...
    globals ``app.gateway.deps`` caches for the rest of the session. An
    ``httpx.AsyncClient`` via ``ASGITransport`` runs in the test's anyio loop — the
    same loop the engine lives in — unlike ``TestClient``, whose portal loop cannot
    share the asyncpg engine.
    """
    ...
    def _make(*, allow_registration: bool) -> AsyncClient:
        ...
        monkeypatch.setattr("tianshu.config.app_config.get_app_config", lambda: patched)
...
@pytest.mark.anyio
async def test_register_succeeds_when_registration_allowed(client):
    """Default (allow_registration=True) keeps self-registration working."""
    resp = await client(allow_registration=True).post(
...
@pytest.mark.anyio
async def test_register_rejected_when_registration_disabled(client):
    """allow_registration=False closes the local self-registration path."""
    resp = await client(allow_registration=False).post(
        "/api/v1/auth/register",
...
@pytest.mark.anyio
async def test_register_rejected_before_the_account_is_created(client):
    ...
    denied = await client(allow_registration=False).post(
        "/api/v1/auth/register", json={"email": email, "password": "Tr0ub4dor3a!"}
    )
...
@pytest.mark.anyio
async def test_registration_defaults_to_allowed():
...
@pytest.mark.anyio
async def test_gate_falls_back_to_open_when_config_is_absent(monkeypatch):
...
@pytest.mark.anyio
async def test_setup_status_reports_registration_state(client, allowed):
    """The login page needs the flag to decide whether to offer a signup entry."""
    resp = await client(allow_registration=allowed).get("/api/v1/auth/setup-status")
    assert resp.status_code == 200
    assert resp.json()["registration_enabled"] is allowed


@pytest.mark.anyio
async def test_initialize_is_not_gated_by_the_registration_flag(client):
