...
against a real isolated PostgreSQL schema (so the index is actually
...
from tianshu.persistence.scheduled_task_runs import ActiveScheduledRunConflict, ScheduledTaskRunRepository
...
@pytest.mark.asyncio
async def test_two_concurrent_manual_dispatches_launch_exactly_once(pg_test_engine):
...
@pytest.mark.asyncio
async def test_scheduled_and_manual_dispatch_launch_exactly_once(pg_test_engine):
    sf = get_session_factory()
    assert sf is not None
    task_repo = ScheduledTaskRepository(sf)
    run_repo = _BarrierRunRepo(sf, asyncio.Barrier(2))
...
@pytest.mark.asyncio
async def test_natural_timing_concurrent_dispatch_launches_exactly_once(pg_test_engine):
...
@pytest.mark.asyncio
async def test_partial_unique_index_enforces_one_active_run_per_task(pg_test_engine):
