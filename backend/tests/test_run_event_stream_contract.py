...
from tianshu.runtime.events.catalog import (
...
from tianshu.subagents.step_events import SUBAGENT_STEP_MAX_CHARS, capture_step_message, subagent_run_event
    ...
    chunks = [
        {"type": "task_started", "task_id": "call-batch", "description": "research"},
        {
            ...
            "message": {
                "type": "ai",
                "content": "searching",
                "tool_calls": [{"name": "web_search", "args": {"query": "tianshu"}}],
            },
            "message_index": 1,
        },
...
def test_lower_level_run_event_modules_do_not_import_runtime(relative_path):
    module_path = REPO_ROOT / "backend" / "packages" / "harness" / "tianshu" / relative_path
    ...
    assert not [module for module in imports if module == "tianshu.runtime" or module.startswith("tianshu.runtime.")]
...
@pytest.mark.anyio
async def test_database_store_returns_contract_record_with_backend_fields(pg_test_engine):
...
@pytest.mark.anyio
async def test_run_end_backend_storage_semantics_match_contract(pg_test_engine, tmp_path):
    from tianshu.persistence.engine import get_session_factory
    from tianshu.runtime.events.store.db import DbRunEventStore
    from tianshu.runtime.events.store.jsonl import JsonlRunEventStore
...
def test_captured_subagent_message_survives_task_running_conversion():
    ...
    assert capture_step_message(
        AIMessage(
            content="searching",
            id="ai-step-1",
            tool_calls=[{"id": "call-1", "name": "web_search", "args": {"query": "tianshu"}}],
        ),
        captured,
        set(),
    )
    ...
    assert event["content"]["tool_calls"] == [{"name": "web_search", "args": {"query": "tianshu"}}]
...
@pytest.mark.anyio
async def test_subagent_batch_round_trip_matches_contract_for_database_store(pg_test_engine):
    from tianshu.persistence.engine import get_session_factory
    from tianshu.runtime.events.store.db import DbRunEventStore
...
async def test_workspace_change_producer_matches_catalog_and_payload(monkeypatch, tmp_path):
    from tianshu.workspace_changes import WorkspaceRoot, scan_workspace_roots
    from tianshu.workspace_changes import recorder as recorder_module
