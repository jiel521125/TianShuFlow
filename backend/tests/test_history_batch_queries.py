...
from tianshu.runtime.runs.manager import EditReplayVisibility
...
@pytest.mark.anyio
async def test_db_event_store_returns_global_last_non_middleware_ai_seq(pg_test_engine):
...
@pytest.mark.anyio
async def test_run_repository_batch_queries_are_unbounded_and_owner_scoped(pg_test_engine):
    from tianshu.persistence.engine import get_session_factory
...
async def test_run_manager_batch_history_methods_fail_closed_without_user_context():
    from tianshu.runtime import user_context
...
@pytest.mark.anyio
async def test_run_repository_lists_edit_replay_runs_unbounded_and_owner_scoped(pg_test_engine):
    from tianshu.persistence.engine import get_session_factory
    from tianshu.persistence.run import RunRepository
