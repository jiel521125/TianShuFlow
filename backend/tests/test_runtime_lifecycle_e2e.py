...
from tianshu.runtime.checkpoint_state import build_state_mutation_graph
...
database:
  backend: postgres
  postgres_url: $TIANSHU_TEST_POSTGRES_URL
...
@pytest.fixture
def isolated_tian_shu_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    if not os.getenv("TIANSHU_TEST_POSTGRES_URL"):
        pytest.skip("set TIANSHU_TEST_POSTGRES_URL to run live PostgreSQL tests")
    home = tmp_path / "tian-shu-home"
    home.mkdir()
    monkeypatch.setenv("TIAN_SHU_HOME", str(home))
    ...
    monkeypatch.setenv("TIAN_SHU_CONFIG_PATH", str(staged_config))
    ...
    monkeypatch.setenv("TIAN_SHU_EXTENSIONS_CONFIG_PATH", str(staged_extensions_config))
    return home
...
def _reset_process_singletons(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``config.yaml``/``extensions_config.json`` and ``TIAN_SHU_HOME``, so the
    caches below must be reset before app creation:

    - app_config / extensions_config: parsed config file caches.
    - paths: ``TIAN_SHU_HOME``-derived filesystem paths.
    - persistence.engine: SQLAlchemy engine/session factory for the postgres database.
    ...
    from tianshu.config import app_config as app_config_module
    from tianshu.config import extensions_config as extensions_config_module
    from tianshu.config import paths as paths_module
    from tianshu.persistence import engine as engine_module
...
@pytest.fixture
def isolated_app(isolated_tian_shu_home: Path, monkeypatch: pytest.MonkeyPatch):
...
def test_lifespan_uses_postgres_store_from_database_config(isolated_app, windows_selector_loop):
...
@pytest.fixture
def isolated_app_with_title(isolated_tian_shu_home: Path, monkeypatch: pytest.MonkeyPatch):
    config_path = isolated_tian_shu_home.parent / "config-title-enabled.yaml"
    config_path.write_text(_MINIMAL_CONFIG_YAML.replace("title:\n  enabled: false", "title:\n  enabled: true"), encoding="utf-8")
    monkeypatch.setenv("TIAN_SHU_CONFIG_PATH", str(config_path))
...
def _register_user(client, *, email: str | None = None) -> str:
    # These E2E tests share the postgres database, so registration emails must
    # be unique per run to avoid colliding with rows left by earlier runs.
    if email is None:
        email = f"runtime-e2e-{uuid.uuid4().hex[:12]}@example.com"
...
def test_stream_run_completes_and_persists_runtime_state(isolated_app, windows_selector_loop):
...
def test_stream_run_executes_real_lead_agent_setup_agent_business_path(isolated_app, isolated_tian_shu_home: Path, windows_selector_loop):
    ...
    with (
        patch(
            "tianshu.agents.lead_agent.agent.create_chat_model",
        ...
        expected_soul = isolated_tian_shu_home / "users" / auth_user_id / "agents" / agent_name / "SOUL.md"
        assert expected_soul.exists(), f"setup_agent did not write SOUL.md. tmp tree: {sorted(str(p.relative_to(isolated_tian_shu_home)) for p in isolated_tian_shu_home.rglob('SOUL.md'))}"
        assert f"Agent name: {agent_name}" in expected_soul.read_text(encoding="utf-8")
        assert not (isolated_tian_shu_home / "users" / "default" / "agents" / agent_name).exists()


def test_cancel_interrupt_stops_running_background_run(isolated_app, windows_selector_loop):
...
def test_cancel_interrupt_generates_missing_title_from_checkpoint(isolated_app_with_title, windows_selector_loop):
...
def test_cancel_wait_false_generates_title_from_graph_input_before_checkpoint(isolated_app_with_title, windows_selector_loop):
...
async def test_sse_consumer_disconnect_cancels_inflight_run():
    ...
    from tianshu.runtime import DisconnectMode, MemoryStreamBridge, RunManager, RunStatus
...
def test_cancel_rollback_restores_pre_run_checkpoint(isolated_app, windows_selector_loop):
