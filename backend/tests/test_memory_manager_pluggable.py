...
from tianshu.agents.memory import (
    MemoryManager,
    get_memory_manager,
    reset_memory_manager,
)
from tianshu.agents.memory.backends.deermem.deer_mem import DeerMem
from tianshu.agents.memory.backends.noop.noop_manager import NoopMemoryManager
from tianshu.config.memory_config import MemoryConfig, get_memory_config, set_memory_config
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("tianshu.agents.memory.backends.deermem.deer_mem.DeerMem", DeerMem),
        ("tianshu.agents.memory.backends.deermem.deer_mem:DeerMem", DeerMem),
        ("tianshu.agents.memory.backends.noop.noop_manager.NoopMemoryManager", NoopMemoryManager),
        ("tianshu.agents.memory.backends.noop.noop_manager:NoopMemoryManager", NoopMemoryManager),
    ],
)
...
def test_migration_drops_file_style_legacy_storage_path(caplog) -> None:
    ...
    from tianshu.config.memory_config import load_memory_config_from_dict

    with caplog.at_level("WARNING", logger="tianshu.config.memory_config"):
...
def test_empty_storage_path_factory_injects_runtime_home(tmp_path, monkeypatch) -> None:
    ...
    import tianshu.config.runtime_paths as rp

    monkeypatch.setattr(rp, "runtime_home", lambda: tmp_path)
    set_memory_config(MemoryConfig(manager_class="deermem", backend_config={"storage_class": "file"}))  # legacy file backend, no storage_path
    manager = get_memory_manager()
...
def test_deermem_shutdown_flush_drains_a_pending_update() -> None:
    ...
    from tianshu.agents.memory.backends.deermem.deermem.core.queue import ConversationContext
