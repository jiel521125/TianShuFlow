...
Exercises the synchronous SQL store against a live PostgreSQL database on an
isolated per-test schema: CRUD, the config-document round trip (name excluded
from the JSON, restored on read), per-user isolation, the UNIQUE(user_id, name)
conflict, and the change-token signature the GitHub registry keys its cache off.
"""
...
def store():
    ...
    import psycopg
    from tianshu.persistence.postgres_schema import dsn_with_search_path

    url = os.getenv("TIANSHU_TEST_POSTGRES_URL")
    if not url:
        pytest.skip("set TIANSHU_TEST_POSTGRES_URL to run live PostgreSQL agent store tests")

    # Unique schema per test → the store's per-URL engine cache never collides.
    schema = f"pgtest_{uuid.uuid4().hex[:12]}"
    with psycopg.connect(url, autocommit=True) as conn:
        conn.execute(f'CREATE SCHEMA "{schema}"')
    store_url = dsn_with_search_path(url, schema)
    engine = create_engine(store_url)
    Base.metadata.create_all(engine, tables=[AgentRow.__table__])
    try:
        yield SqlAgentStore(store_url)
    finally:
        engine.dispose()
        with psycopg.connect(url, autocommit=True) as conn:
...
def test_engine_cache_is_reused_per_url():
    ...
    from tianshu.persistence.agents.sql import _get_sessionmaker

    url = "postgresql+psycopg://postgres:postgres@localhost:5432/tianshu"
