...
from tianshu.agents.thread_state import merge_artifacts, merge_message_writes
...
from tianshu.runtime.runs.manager import CancelOutcome, ConflictError, RunManager
from tianshu.runtime.runs.schemas import RunStatus
from tianshu.runtime.runs.store.memory import MemoryRunStore
from tianshu.runtime.runs.worker import (
    RollbackPoint,
    RunContext,
...
def _stub_mutation_graph(monkeypatch, *, restored_config):
    ...
    monkeypatch.setattr(
        "tianshu.runtime.runs.worker.build_state_mutation_graph",
...
async def test_run_agent_marks_rollback_unusable_when_capture_fails():
    ...
    with patch(
        "tianshu.runtime.runs.worker._rollback_to_pre_run_checkpoint",
...
@pytest.mark.anyio
async def test_rollback_linearizes_delta_restore_inmemory_saver():
    """Same linear restore contract against InMemorySaver: the restored fork
    stays latest and the restored values survive a fresh read on the same saver."""
    checkpointer = InMemorySaver()
...
def test_agent_factory_supports_app_config_returns_false_when_signature_lookup_fails(monkeypatch):
    ...
    monkeypatch.setattr("tianshu.runtime.runs.worker.inspect.signature", lambda _obj: (_ for _ in ()).throw(ValueError("boom")))
...
async def test_run_agent_ignores_stale_llm_error_fallback_from_prior_run():
    ...
    classified transient and surfaced as a ``tianshu_error_fallback`` AIMessage)
...
async def test_ensure_interrupted_title_bumps_channel_version_and_declares_it_in_new_versions(monkeypatch):
    """Regression for #3859 review: DB-backed savers strip inline
...
@pytest.mark.anyio
async def test_ensure_interrupted_title_round_trip_with_inmemory_checkpointer():
    """Full round-trip against a real ``InMemorySaver``.

    Mirrors what Gateway constructs in production via ``make_checkpointer``. The
    fallback title must survive the write/read boundary on the same saver — this
    is the scenario the #3874 review flagged as broken before the
    ...
    await checkpointer.aput(thread_cfg, ck, {"source": "loop", "step": 1, "writes": {}}, {"messages": 1})

    # 2. Run the worker helper against the same saver — this is what
    ...
    # 3. Read the checkpoint back and confirm the title survives — this is the
    ...
    tup = await checkpointer.aget_tuple(thread_cfg)
...
async def test_worker_finally_block_swallows_helper_exceptions(monkeypatch):
    ...
    the helper itself.
    """
    import tianshu.runtime.runs.worker as worker_module
