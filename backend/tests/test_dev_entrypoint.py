...
def test_entrypoint_excludes_runtime_state_from_uvicorn_reload():
    ...
    assert ': "${TIAN_SHU_HOME:=/app/backend/.tian-shu}"' in content
    # sandbox must be created too, not just .tian-shu (#3459 / #3454).
    assert 'mkdir -p "$TIAN_SHU_HOME" /app/backend/.tian-shu /app/backend/sandbox' in content
    ...
    assert '--reload-exclude="$TIAN_SHU_HOME"' in content
    assert "--reload-exclude=/app/backend/.tian-shu" in content
