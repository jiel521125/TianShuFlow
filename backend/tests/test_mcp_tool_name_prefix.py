...
from tianshu.mcp.tools import _make_session_pool_tool, get_mcp_tools
from tianshu.tools.mcp_metadata import get_mcp_routing
...
async def test_mcp_tool_name_prefix_can_be_disabled_per_server_without_disabling_stdio_pooling() -> None:
    ...
    with (
        ...
        patch("tianshu.mcp.tools.get_initial_oauth_headers", new_callable=AsyncMock, return_value={}),
        ...
        patch("tianshu.mcp.tools._make_session_pool_tool", side_effect=lambda tool, *_args, **_kwargs: tool) as wrap_tool,
    ):
...
async def test_unprefixed_stdio_tool_keeps_server_like_original_name(tmp_path: Path) -> None:
    ...
    with (
        patch("tianshu.mcp.tools.get_session_pool", return_value=mock_pool),
        patch("tianshu.mcp.tools.get_paths", return_value=MagicMock()),
        patch(
            "tianshu.mcp.tools._prepare_stdio_workspace",
