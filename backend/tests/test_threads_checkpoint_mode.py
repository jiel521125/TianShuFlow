...
(``get_thread_state_schema(mode)``) on ``InMemorySaver``. Every assertion
compares the materialized state
...
async def test_thread_lifecycle_parity_separate_inmemory_savers() -> None:
    full = await _run_thread_lifecycle("full", InMemorySaver())
    delta = await _run_thread_lifecycle("delta", InMemorySaver())
    assert full == delta
...
async def test_delta_thread_avoids_per_step_full_message_blobs(tmp_path) -> None:
    ...
    async def _blob_checkpoint_counts(mode: str) -> tuple[int, int]:
        saver = InMemorySaver()
    ...
    delta_total, delta_blobs = await _blob_checkpoint_counts("delta")
    full_total, full_blobs = await _blob_checkpoint_counts("full")
...
async def test_delta_snapshot_frequency_controls_snapshot_cadence(tmp_path) -> None:
    ...
    async def _run(snapshot_frequency: int) -> tuple[int, list[tuple[str, str, str]]]:
        saver = InMemorySaver()
    ...
    low_cadence_blobs, low_cadence_messages = await _run(2)
    default_blobs, default_messages = await _run(10)
