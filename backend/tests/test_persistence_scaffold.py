...
4. Engine init/close lifecycle (memory + Postgres)
...
class TestDatabaseConfig:
    ...
    def test_sqlite_backend_was_removed(self):
        """sqlite is no longer a supported backend (memory/postgres only)."""
        from pydantic import ValidationError

        with pytest.raises(ValidationError):
            DatabaseConfig(backend="sqlite", sqlite_dir="./mydata")
    ...
    @pytest.mark.parametrize("schema", ["tianshu", "my_schema", "_private", "s", "a" * 63])
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            # quoted schema literally named ``tianshu\n`` while the unquoted
            # search_path folds to ``tianshu`` and misses it (tables land in
    ...
    def test_postgres_schema_does_not_pollute_url(self):
        c = DatabaseConfig(backend="postgres", postgres_url="postgresql://u:p@h:5432/db", postgres_schema="tianshu")
        url = c.app_sqlalchemy_url
        assert "tianshu" not in url.replace("/db", "")
        assert url.startswith("postgresql+asyncpg://")
...
class TestBaseToDictMixin:
    @pytest.mark.anyio
    async def test_to_dict_and_exclude(self, pg_test_engine):
        """Create a minimal model in the isolated test schema, verify to_dict."""
        from sqlalchemy import String
        from sqlalchemy.orm import Mapped, mapped_column

        from tianshu.persistence.base import Base
        from tianshu.persistence.engine import get_engine, get_session_factory
        ...
        async with engine.begin() as conn:
            await conn.run_sync(Base.metadata.create_all, tables=[_Tmp.__table__])
...
class TestEngineLifecycle:
    ...
    @pytest.mark.anyio
    async def test_postgres_engine_lifecycle_roundtrip(self, pg_test_engine):
        """Postgres init yields a usable session factory; close_engine resets it."""
        from tianshu.persistence.engine import close_engine, get_session_factory
    ...
    async def test_postgres_without_asyncpg_gives_actionable_error(self):
        ...
        from tianshu.persistence.engine import init_engine
...
class TestPostgresSchemaInit:
    ...
    async def test_passes_search_path_connect_args(self, monkeypatch):
        import tianshu.persistence.engine as engine_module
        ...
        monkeypatch.setattr("tianshu.persistence.bootstrap.bootstrap_schema", AsyncMock())
        ...
        assert captured["connect_args"] == {
            ...
            "server_settings": {"search_path": "tianshu"},
        }
    ...
    async def test_creates_schema_before_bootstrap(self, monkeypatch):
        ...
        monkeypatch.setattr("tianshu.persistence.bootstrap.bootstrap_schema", calls.bootstrap_schema)
        ...
        assert "tianshu" in str(execute_arg)
        await engine_module.close_engine()
