...
@pytest.mark.parametrize("mode", ["full", "delta"])
def test_postgres_smoke_case_reports_durable_and_logical_storage(mode: str, tmp_path: Path) -> None:
    if not os.getenv("TIANSHU_TEST_POSTGRES_URL"):
        pytest.skip("set TIANSHU_TEST_POSTGRES_URL to run live PostgreSQL tests")
    # ``_postgres_dsn()`` reads TIANSHU_BENCH_POSTGRES_URL; point it at the
    # shared test database so the smoke case runs against the same instance.
    os.environ["TIANSHU_BENCH_POSTGRES_URL"] = os.getenv("TIANSHU_TEST_POSTGRES_URL", "")
    case = bench.BenchmarkCase(
        mode=mode,
        backend="postgres",
        update_count=3,
