...
from tianshu.config.acp_config import load_acp_config_from_dict
from tianshu.config.agents_api_config import get_agents_api_config, load_agents_api_config_from_dict
from tianshu.config.app_config import AppConfig, get_app_config, reset_app_config
from tianshu.config.checkpointer_config import get_checkpointer_config, load_checkpointer_config_from_dict
from tianshu.config.database_config import DatabaseConfig
from tianshu.config.guardrails_config import get_guardrails_config, load_guardrails_config_from_dict
from tianshu.config.memory_config import get_memory_config, load_memory_config_from_dict
from tianshu.config.stream_bridge_config import get_stream_bridge_config, load_stream_bridge_config_from_dict
from tianshu.config.subagents_config import get_subagents_app_config, load_subagents_config_from_dict
from tianshu.config.summarization_config import get_summarization_config, load_summarization_config_from_dict
from tianshu.config.title_config import get_title_config, load_title_config_from_dict
from tianshu.config.tool_search_config import get_tool_search_config, load_tool_search_config_from_dict
...
def test_app_config_defaults_missing_database_to_postgres(tmp_path, monkeypatch):
    config_path = tmp_path / "config.yaml"
    extensions_path = tmp_path / "extensions_config.json"
    _write_extensions_config(extensions_path)
    _write_config(config_path, model_name="first-model", supports_thinking=False)

    monkeypatch.setenv("TIAN_SHU_EXTENSIONS_CONFIG_PATH", str(extensions_path))

    config = AppConfig.from_file(str(config_path))

    assert config.database.backend == "postgres"
    assert config.database.postgres_url == "postgresql://postgres:postgres@localhost:5432/tianshu"
...
def test_app_config_defaults_empty_database_to_postgres(tmp_path, monkeypatch):
    config_path = tmp_path / "config.yaml"
    extensions_path = tmp_path / "extensions_config.json"
    _write_extensions_config(extensions_path)
...
def test_app_config_warns_when_no_models_configured(tmp_path, monkeypatch, caplog):
    ...
    with caplog.at_level("WARNING", logger="tianshu.config.app_config"):
        AppConfig.from_file(str(config_path))
...
def test_get_app_config_reloads_when_config_path_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("TIAN_SHU_EXTENSIONS_CONFIG_PATH", str(extensions_path))
    monkeypatch.setenv("TIAN_SHU_CONFIG_PATH", str(config_a))
    reset_app_config()
    ...
    try:
        ...
        monkeypatch.setenv("TIAN_SHU_CONFIG_PATH", str(config_b))
...
def test_get_app_config_does_not_reset_persistence_singletons_when_database_changes(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("tianshu.runtime.checkpointer.reset_checkpointer", _reset_checkpointer)
    monkeypatch.setattr("tianshu.runtime.store.reset_store", _reset_store)
