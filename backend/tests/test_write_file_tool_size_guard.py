...
from tianshu.sandbox import tools as tools_module
from tianshu.sandbox.tools import write_file_tool
...
def test_env_override_raises_cap(monkeypatch: pytest.MonkeyPatch):
    """Setting TIANSHU_WRITE_FILE_MAX_BYTES lets deployments accept larger
    payloads when the underlying LLM/network can demonstrably handle them.
    """
    monkeypatch.setenv("TIANSHU_WRITE_FILE_MAX_BYTES", str(300 * 1024))
...
def test_env_override_zero_disables_guard(monkeypatch: pytest.MonkeyPatch):
    ...
    """
    monkeypatch.setenv("TIANSHU_WRITE_FILE_MAX_BYTES", "0")
...
def test_env_override_malformed_falls_back_to_default(monkeypatch: pytest.MonkeyPatch):
    ...
    """
    monkeypatch.setenv("TIANSHU_WRITE_FILE_MAX_BYTES", "lots")
