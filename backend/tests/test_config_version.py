...
def test_missing_version_treated_as_zero(caplog):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with caplog.at_level(logging.WARNING, logger="tianshu.config.app_config"):
...
def test_version_26_config_upgrades_to_checkpoint_channel_mode(tmp_path, caplog):
    ...
    user_config = {
        "config_version": 26,
        "sandbox": {"use": "tianshu.sandbox.local:LocalSandboxProvider"},
        "database": {"backend": "postgres", "postgres_url": "postgresql://localhost:5432/tianshu"},
    }
    ...
    env = {**os.environ, "TIAN_SHU_CONFIG_PATH": str(config_path)}
    ...
    assert upgraded["database"]["backend"] == "postgres"
    assert upgraded["database"]["postgres_url"] == "postgresql://localhost:5432/tianshu"
