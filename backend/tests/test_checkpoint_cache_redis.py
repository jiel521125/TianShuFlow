...
from tianshu.runtime.checkpoint_cache.provider import (
...
def _make_cache(monkeypatch: pytest.MonkeyPatch, fake: _FakeRedis, ttl_seconds: int = 60, **kwargs: Any):
    import tianshu.runtime.checkpoint_cache.redis as redis_mod
...
async def test_provider_memory_default():
    ...
    async with make_checkpoint_cache(_app_config({"backend": "memory"}), serde=JsonPlusSerializer()) as cache:
...
async def test_provider_zero_max_entries_disables_any_type():
    app_config = _app_config({"backend": "memory", "checkpoint_cache": {"type": "redis", "max_entries": 0}})
    from tianshu.runtime.checkpoint_cache.memory import MemoryCheckpointHistoryCache
...
def test_db_hash_distinguishes_backends_and_targets():
    ...
    memory_cfg = DatabaseConfig.model_validate({"backend": "memory"})
    ...
    assert checkpoint_cache_db_hash(memory_cfg) != checkpoint_cache_db_hash(pg_cfg)
...
def test_db_hash_stable_across_credential_rotation():
    ...
    before = DatabaseConfig.model_validate({"backend": "postgres", "postgres_url": "postgresql://alice:secret1@pg.internal:5432/tianshu"})
    rotated = DatabaseConfig.model_validate({"backend": "postgres", "postgres_url": "postgresql://bob:secret2@pg.internal:5432/tianshu"})
    driver_suffix = DatabaseConfig.model_validate({"backend": "postgres", "postgres_url": "postgresql+asyncpg://alice:secret1@pg.internal:5432/tianshu"})
...
def test_key_prefix_override_wins():
    app_config = _app_config({"backend": "memory", "checkpoint_cache": {"key_prefix": "custom:"}})
    assert checkpoint_cache_key_prefix(app_config) == "custom:"
    default = checkpoint_cache_key_prefix(_app_config({"backend": "memory"}))
