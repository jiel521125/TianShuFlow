...
from tianshu.persistence.bootstrap import _alembic_safe_url, _escape_url_for_alembic, _get_alembic_config
...
def test_postgres_url_does_not_double_percent_unnecessarily() -> None:
    # No percent in the URL -> no escaping needed -> output equals input.
    engine = _fake_engine("postgresql://alice:secret@db.example.com/app")
    safe = _alembic_safe_url(engine)
    assert safe == "postgresql://alice:secret@db.example.com/app"
...
def test_escape_url_for_alembic_doubles_only_percent_signs() -> None:
    ...
    assert _escape_url_for_alembic("postgresql://a:b@db.example.com/app") == "postgresql://a:b@db.example.com/app"
...
def test_alembic_config_forwards_postgres_schema_option() -> None:
    ...
    cfg = _get_alembic_config(engine, postgres_schema="tianshu")
    assert cfg.get_main_option("tianshu_pg_schema") == "tianshu"


def test_alembic_config_omits_schema_option_when_unset() -> None:
    engine = _fake_engine("postgresql://a:b@h/d")
    cfg = _get_alembic_config(engine)
    assert cfg.get_main_option("tianshu_pg_schema") is None


def test_env_module_pins_search_path_from_schema_option() -> None:
    """env.py must read ``tianshu_pg_schema`` and pin the alembic-spawned
    ...
    ``test_env_module_uses_async_postgres_engine``.
    """
    ...
    assert 'get_main_option("tianshu_pg_schema")' in src, "env.py must read the tianshu_pg_schema option set by _get_alembic_config"
