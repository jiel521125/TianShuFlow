...
import tianshu.skills.storage as skill_storage
from tianshu.config.paths import Paths
from tianshu.skills.storage import SkillStorage
...
def _patch_storage_resolution(monkeypatch, cls=SlowSkillStorage) -> None:
    monkeypatch.setattr("tianshu.config.get_app_config", lambda: _APP_CONFIG)
    monkeypatch.setattr("tianshu.reflection.resolve_class", lambda *args, **kwargs: cls)
...
def _patch_user_storage_resolution(monkeypatch, cls=SlowUserSkillStorage) -> None:
    monkeypatch.setattr("tianshu.config.get_app_config", lambda: _APP_CONFIG)
    monkeypatch.setattr("tianshu.config.paths.get_paths", lambda: Paths(base_dir=Path("/tmp")))
    ...
    monkeypatch.setattr("tianshu.skills.storage.UserScopedSkillStorage", cls)
