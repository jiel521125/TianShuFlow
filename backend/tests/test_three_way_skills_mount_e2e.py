...
from tianshu.sandbox.local.local_sandbox import PathMapping
...
from tianshu.skills.types import SKILL_MD_FILE, Skill, SkillCategory

_AIO_MODULE = "tianshu.community.aio_sandbox.aio_sandbox_provider"
...
class TestThreeWayMountEndToEnd:
    ...
    def test_local_public_skill_mounted(self, skills_fs):
        ...
        with patch("tianshu.config.get_app_config", return_value=cfg), patch("tianshu.config.paths.get_paths", return_value=paths):
    ...
    def test_local_acquire_recovers_public_mount_after_initial_projection_failure(self, skills_fs):
        ...
        with (
            patch("tianshu.config.get_app_config", return_value=cfg),
    ...
    def test_registry_to_sandbox_full_pipeline(self, skills_fs):
        ...
        with patch("tianshu.config.paths.get_paths", return_value=paths):
    ...
    def test_local_bash_observes_toggle_without_sandbox_recreation(self, tmp_path):
        ...
        with (
            patch("tianshu.config.get_app_config", return_value=cfg),
            patch("tianshu.config.paths.get_paths", return_value=paths),
            patch("tianshu.config.extensions_config.ExtensionsConfig.from_file", return_value=extensions),
            patch("tianshu.config.extensions_config.get_extensions_config", return_value=extensions),
        ):
    ...
    def test_aio_per_user_custom_skill_mount(self, skills_fs, aio_mod, monkeypatch):
        ...
        with patch(_AIO_GET_CONFIG, return_value=cfg), patch("tianshu.config.paths.get_paths", return_value=paths):
            mounts = aio_mod.AioSandboxProvider._get_skills_mounts(user_id="user-1")
        idx = {m[1]: m for m in mounts}
        assert "/mnt/skills/custom" in idx
    ...
    def test_aio_extra_mounts_translate_to_docker_bind_mounts(self, skills_fs, aio_mod, monkeypatch):
        ...
        from tianshu.community.aio_sandbox.local_backend import _format_container_mount
