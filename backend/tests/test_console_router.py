...
Uses a live-PostgreSQL test database on an isolated schema (NullPool, so seeding
in one event loop and serving TestClient requests in another never share a
connection).
"""
...
def session_factory():
    ...
    if not url:
        pytest.skip("set TIANSHU_TEST_POSTGRES_URL to run live PostgreSQL console router tests")
    ...
    engine = create_async_engine(url, poolclass=NullPool, connect_args=build_asyncpg_connect_args(schema))
