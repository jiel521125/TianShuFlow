...
from tianshu.persistence.scheduled_task_runs import ScheduledTaskRunRepository
from tianshu.persistence.scheduled_tasks import ScheduledTaskRepository

_POSTGRES_URL = os.getenv("TIANSHU_TEST_POSTGRES_URL")
...
async def pg_test_engine():
    ...
    Mirrors the shared ``pg_test_engine`` in ``tests/conftest.py``, but
    defined locally with ``pytest_asyncio.fixture`` so it is handled under
    pytest-asyncio's strict mode. Initializes the app engine (tables +
    migrations) against a fresh per-test schema and drops the schema with
    ``CASCADE`` in teardown.
    """
    ...
    try:
        await init_engine_from_config(
            DatabaseConfig(backend="postgres", postgres_url=_POSTGRES_URL, postgres_schema=schema)
        )
        yield schema
    finally:
        await close_engine()
...
@pytest.mark.asyncio
async def test_scheduled_task_repository_create_and_list(pg_test_engine):
...
@pytest.mark.asyncio
async def test_scheduled_task_run_repository_records_history(pg_test_engine):
    sf = get_session_factory()
    assert sf is not None
...
@pytest.mark.asyncio
async def test_mark_stale_active_runs_fails_orphaned_runs(pg_test_engine):
...
@pytest.mark.asyncio
async def test_update_status_protect_terminal_keeps_completion_result(pg_test_engine):
...
@pytest.mark.asyncio
async def test_has_active_runs_sees_only_queued_and_running(pg_test_engine):
    sf = get_session_factory()
    assert sf is not None
...
@pytest.mark.asyncio
async def test_cancel_stuck_once_tasks_reconciles_orphaned_running(pg_test_engine):
...
@pytest.mark.asyncio
async def test_update_after_launch_protect_terminal_keeps_hook_result(pg_test_engine):
...
@pytest.mark.asyncio
async def test_list_by_task_paginates(pg_test_engine):
    sf = get_session_factory()
    assert sf is not None
...
@pytest.mark.asyncio
async def test_list_by_user_and_thread_filters_in_sql(pg_test_engine):
    sf = get_session_factory()
    assert sf is not None
