...
from tianshu.persistence.thread_meta import THREAD_PINNED_METADATA_KEY, InvalidMetadataFilterError
from tianshu.persistence.thread_meta.memory import THREADS_NS, MemoryThreadMetaStore
...
def test_delete_thread_route_closes_browser_session(tmp_path):
    ...
    with (
        ...
        patch(
            "tianshu.community.browser_automation.get_browser_session_manager",
...
def test_branch_thread_real_mutation_graph_finishes_without_scheduling(monkeypatch) -> None:
    ...
    assert branch_snapshot.metadata["tianshu_branch"] is True
...
def _wire_extension_agent(monkeypatch, app, checkpointer, mode):
    ...
    from tianshu.agents.thread_state import get_thread_state_schema
...
def test_branch_thread_rejects_sidecar_threads() -> None:
    ...
    with TestClient(app) as client:
        created = client.post(
            "/api/threads",
            json={"thread_id": "sidecar-thread", "metadata": {"tianshu_sidecar": True}},
        )
        assert created.status_code == 200, created.text
...
def test_update_thread_state_inserts_new_checkpoint_each_call() -> None:
    ...
    checkpoint reused the previous checkpoint["id"], so InMemorySaver
