...
from tianshu.community.e2b_sandbox.capacity import (
    CapacityBackendError,
    ReserveStatus,
)
from tianshu.config.paths import Paths
from tianshu.config.sandbox_config import SandboxConfig
from tianshu.sandbox.exceptions import SandboxCapacityExceededError
...
class FakeOwnershipStore:
    ...
    def renew(self, sandbox_id: str):
        from tianshu.community.aio_sandbox.ownership import RenewOutcome
...
def _make_provider(*, replicas: int = 3, idle_timeout: int = 1800, overflow_policy: str = "wait", acquire_timeout: int = 30, burst_limit: int = 0) -> Any:
    ...
    mod = importlib.import_module("tianshu.community.e2b_sandbox.e2b_sandbox_provider")
...
def _install_shared_deployment_capacity(
    *providers,
    ...
    store = MagicMock()
    store.key = "tianshu:test:e2b-capacity"
...
def test_apply_mounts_uploads_only_enabled_skill_projection(monkeypatch, tmp_path):
    from tianshu.config.extensions_config import ExtensionsConfig, SkillStateConfig
    ...
    monkeypatch.setattr("tianshu.config.extensions_config.ExtensionsConfig.from_file", lambda *_args, **_kwargs: extensions)
    monkeypatch.setattr("tianshu.config.extensions_config.get_extensions_config", lambda: extensions)
...
def test_skill_projection_mounts_swallows_projection_failure(monkeypatch):
    ...
    monkeypatch.setattr(mod, "get_app_config", lambda: config)
    monkeypatch.setattr(
        "tianshu.skills.projection.ensure_skill_projections",
...
def _make_sandbox(client: FakeClient, *, sandbox_id: str | None = None) -> Any:
    mod = importlib.import_module("tianshu.community.e2b_sandbox.e2b_sandbox")
...
def _info(sandbox_id: str, user_id: str, thread_id: str):
    return SimpleNamespace(
        ...
        metadata={
            "tian_shu_provider": "e2b_sandbox_provider",
            "tian_shu_user": user_id,
            "tian_shu_thread": thread_id,
        },
    )
...
def test_reconcile_honors_wall_clock_budget(monkeypatch):
    ...
    provider_mod = importlib.import_module("tianshu.community.e2b_sandbox.e2b_sandbox_provider")
...
def test_reconcile_kills_metadata_orphan_only_after_ttl(monkeypatch):
    ...
    fake_cls.list_return = [
        SimpleNamespace(
            ...
            metadata={"tian_shu_provider": "e2b_sandbox_provider"},
        )
    ]
...
def _setup_paths(monkeypatch, tmp_path):
    paths_mod = importlib.import_module("tianshu.config.paths")
...
def test_download_file_streaming_raises_efbig_before_full_buffering():
    ...
    from tianshu.community.e2b_sandbox import e2b_sandbox as e2b_sb_mod
...
def test_deployment_capacity_reserves_commits_and_rejects_globally(monkeypatch) -> None:
    ...
    assert metadata["tian_shu_capacity_ledger"] == store.key
    assert metadata["tian_shu_capacity_reservation"]
    store.track.assert_called_once_with(
        sandbox_id,
        reservation_token=metadata["tian_shu_capacity_reservation"],
    )
...
def test_discovery_uses_sdk_query_and_tracks_without_reserving(monkeypatch) -> None:
    ...
    entry = SimpleNamespace(
        ...
        metadata={
            "tian_shu_provider": "e2b_sandbox_provider",
            "tian_shu_user": "user-a",
            "tian_shu_thread": "thread-a",
            "tian_shu_capacity_ledger": store.key,
        },
    )
    expected_query = {key: entry.metadata[key] for key in ("tian_shu_provider", "tian_shu_user", "tian_shu_thread")}
    ...
    sdk.list_return = [
        {
            ...
            "metadata": {
                "tian_shu_provider": "e2b_sandbox_provider",
                "tian_shu_capacity_ledger": store.key,
                "tian_shu_capacity_reservation": "reservation-crashed",
            },
        },
        {
            ...
            "metadata": {
                "tian_shu_provider": "e2b_sandbox_provider",
                "tian_shu_capacity_ledger": "tianshu:other:e2b-capacity",
            },
        },
    ]
