...
from tianshu.client import StreamEvent
from tianshu.tui.app import TianShuTUI
from tianshu.tui.cli import LaunchPlan
from tianshu.tui.view_state import SystemMessage
...
async def test_app_runs_a_turn_and_renders_streamed_assistant():
    app = TianShuTUI(_FakeSession(), LaunchPlan(mode="tui"))
...
async def test_help_text_matches_command_registry():
    from tianshu.tui.command_registry import format_command_help
...
async def test_clear_command_clears_display_without_resetting_thread_or_calling_agent():
    ...
    app = TianShuTUI(session, LaunchPlan(mode="tui"))
    async with app.run_test() as pilot:
        await pilot.pause()
