...
importing ``tianshu.agents`` no longer sets up telemetry at import time.
"""
...
from tianshu.config import is_monocle_tracing_enabled
from tianshu.config.tracing_config import get_tracing_config, reset_tracing_config
...
def clear_monocle_env(monkeypatch):
    ...
    monkeypatch.setattr("tianshu.tracing.monocle._setup_completed", False)
...
def test_toggles_on_and_sets_up(monkeypatch):
    ...
    assert captured == {"workflow_name": "tian-shu", "monocle_exporters_list": "file"}
...
def test_embedded_hint_when_enabled_but_uninitialized(monkeypatch, caplog):
    ...
    The embedded ``TianShuClient`` and the TUI never hit the Gateway lifespan,
    ...
    with caplog.at_level(logging.DEBUG, logger="tianshu.tracing.factory"):
...
def test_no_import_time_setup():
    """Regression: importing tianshu.agents must not install telemetry.

    The setup call used to live at module import in ``tianshu/agents/__init__``.
    ...
    script = textwrap.dedent(
        ...
        import tianshu.agents as agents
...
def test_gateway_lifespan_initializes_monocle():
    ...
    with (
        ...
        patch("tianshu.skills.projection.ensure_public_skill_projection"),
