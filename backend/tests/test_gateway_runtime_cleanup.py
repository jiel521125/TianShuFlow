...
def test_docker_dev_mounts_mutable_configs_through_project_directory():
    ...
    assert "TIAN_SHU_CONFIG_PATH=/app/project/config.yaml" in compose
    assert "TIAN_SHU_EXTENSIONS_CONFIG_PATH=/app/project/extensions_config.json" in compose


def test_local_dev_gateway_reload_excludes_runtime_state_with_absolute_dirs():
    serve_sh = _read("scripts/serve.sh")

    assert 'export TIAN_SHU_PROJECT_ROOT="$REPO_ROOT"' in serve_sh
    assert 'BACKEND_RUNTIME_HOME="$REPO_ROOT/backend/.tian-shu"' in serve_sh
    assert 'export TIAN_SHU_HOME="$BACKEND_RUNTIME_HOME"' in serve_sh
    ...
    assert 'mkdir -p "$TIAN_SHU_HOME" "$BACKEND_RUNTIME_HOME" "$REPO_ROOT/backend/sandbox"' in serve_sh
    assert "--reload-exclude='$TIAN_SHU_HOME'" in serve_sh
    assert "--reload-exclude='$BACKEND_RUNTIME_HOME'" in serve_sh
    assert "--reload-exclude='sandbox/'" not in serve_sh
    assert "--reload-exclude='.tian-shu/'" not in serve_sh
...
def test_frontend_rewrites_langgraph_prefix_to_gateway():
    ...
    assert "TIAN_SHU_INTERNAL_LANGGRAPH_BASE_URL" not in next_config
...
def test_smoke_test_docs_do_not_expect_standalone_langgraph_server():
    ...
    for path, content in smoke_files.items():
        ...
        assert "tian-shu-langgraph" not in content, path
