"""Fixtures for the TianShu Monocle behavioural tests.

Only fixtures live here. Paths and ``run_tianshu`` are in ``_helpers.py`` so
...
def run_agent() -> Callable[[str], str]:
    ...
    Skips unless ``MONOCLE_LIVE_TESTS=1`` is set, when the TianShu app is not
    ...
    """
    from _helpers import CONFIG_PATH, REPO_ROOT, live_tests_enabled, run_tianshu

    if not live_tests_enabled():
        pytest.skip("live tests are opt-in: set MONOCLE_LIVE_TESTS=1")
    pytest.importorskip("tianshu", reason="TianShu app not importable in this venv")
