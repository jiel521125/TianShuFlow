"""Helpers for the TianShu Monocle behavioural tests.
...
``monocle_trace_asserter`` fixture), so ``run_tianshu`` only drives the agent;
...
def run_tianshu(message: str) -> str:
    """Run the TianShu agent once and return its response text.

    The model is resolved from ``config.yaml`` (no hardcoded override) so the
    live test exercises TianShu's own model-resolution path.
    """
