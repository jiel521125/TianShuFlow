...
from tianshu.agents.memory.backends.deermem.deermem.core.paths import validate_agent_name
...
class TestCreateStorage:
    ...
    def test_default_storage_class_requires_postgres_url(self):
        """Default storage_class ('postgres') + empty postgres_url -> fail-fast.

        Memory is persistent state, so a missing PostgreSQL URL must raise
        instead of silently falling back to the legacy file backend.
        """
        with pytest.raises(ValueError, match="postgres_url"):
            create_storage(DeerMemConfig())

    def test_returns_postgres_memory_storage_when_url_configured(self, deermem_pg_backend_config):
        """Default storage_class ('postgres') + postgres_url -> PostgresMemoryStorage directly."""
        storage = create_storage(DeerMemConfig(**deermem_pg_backend_config))
    ...
    def test_dotted_storage_class_resolves(self):
        storage = create_storage(DeerMemConfig(storage_class="tianshu.agents.memory.backends.deermem.deermem.core.storage.FileMemoryStorage"))
