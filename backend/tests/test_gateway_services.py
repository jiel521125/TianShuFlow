...
def _make_start_run_request(run_manager, *, thread_store=None, auth_source=None):
    ...
    from tianshu.persistence.thread_meta.memory import MemoryThreadMetaStore
...
def test_to_langgraph_stream_modes_maps_alias_and_deduplicates(raw, expected):
    from tianshu.runtime.stream_modes import to_langgraph_stream_modes
...
def test_build_run_config_ceiling_is_configurable(_stub_app_config):
    ...
    set_app_config(AppConfig.model_validate({"sandbox": {"use": "tianshu.sandbox.local:LocalSandboxProvider"}, "max_recursion_limit": 300}))
    try:
        config = build_run_config("thread-1", {"recursion_limit": 100_000_000}, None)
...
def test_build_checkpoint_state_accessor_uses_frozen_mode_and_binds_runtime_persistence(
    ...
    from tianshu.runtime.checkpoint_mode import CHECKPOINT_MODE_METADATA_KEY, INTERNAL_CHECKPOINT_MODE_KEY
...
async def test_checkpoint_history_seed_guard_tolerates_missing_user_context():
    ...
    from app.gateway.services import ensure_checkpoint_history_seeded
    from tianshu.runtime.user_context import AUTO, _AutoSentinel
...
async def test_checkpoint_history_seed_guard_is_thread_scoped_under_user_context():
    ...
    from app.gateway.services import ensure_checkpoint_history_seeded
    from tianshu.runtime.user_context import AUTO
...
@pytest.mark.anyio
@pytest.mark.no_auto_user
async def test_checkpoint_history_seed_runs_exactly_once_across_principals(pg_test_engine):
    ...
    from tianshu.runtime.user_context import reset_current_user, set_current_user
...
async def test_start_run_checkpoint_validation_failure_does_not_admit_run(_stub_app_config):
    ...
    from tianshu.runtime import RunManager
...
async def test_thread_metadata_timeout_logs_and_run_still_starts(_stub_app_config, caplog, monkeypatch):
    ...
    from app.gateway.services import start_run
    from tianshu.runtime import RunManager
    from tianshu.runtime.runs.manager import RunStartOutcome
    from tianshu.runtime.runs.schemas import RunStatus
    from tianshu.runtime.runs.store.memory import MemoryRunStore
...
async def test_run_agent_invalid_stream_mode_finalizes_run_before_graph_invocation():
    ...
    from tianshu.runtime.runs.manager import RunManager
    from tianshu.runtime.runs.schemas import RunStatus
    from tianshu.runtime.runs.worker import RunContext, run_agent
...
async def test_run_agent_full_mode_rejects_delta_before_graph_invocation():
    ...
    from tianshu.runtime.runs.manager import RunRecord, RunStartOutcome
    from tianshu.runtime.runs.schemas import DisconnectMode, RunStatus
    from tianshu.runtime.runs.worker import RunContext, run_agent
...
async def test_run_agent_full_mode_checks_selected_checkpoint_before_graph():
    ...
    from tianshu.runtime.checkpoint_mode import CHECKPOINT_MODE_METADATA_KEY
    from tianshu.runtime.runs.manager import RunRecord, RunStartOutcome
    from tianshu.runtime.runs.schemas import DisconnectMode, RunStatus
    from tianshu.runtime.runs.worker import RunContext, run_agent
