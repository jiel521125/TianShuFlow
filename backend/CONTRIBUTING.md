# Contributing to TianShu Backend

Thank you for your interest in contributing to TianShu! This document provides guidelines and instructions for contributing to the backend codebase.
...
2. Clone your fork locally:
   ```bash
   git clone https://github.com/YOUR_USERNAME/tian-shu.git
   cd tian-shu
   ```
...
```
backend/
├── packages/harness/tianshu/  # tianshu-harness package (import: tianshu.*)
│   ├── agents/                 # Agent system
...
```python
import pytest
from tianshu.models.factory import create_chat_model
...
1. Create tool in `packages/harness/tianshu/tools/builtins/` or `packages/harness/tianshu/community/`:

```python
# packages/harness/tianshu/tools/builtins/my_tool.py
...
tools:
  - name: my_tool
    group: my_group
    use: tianshu.tools.builtins.my_tool:my_tool
```

### Adding New Middleware

1. Create middleware in `packages/harness/tianshu/agents/middlewares/`:

```python
# packages/harness/tianshu/agents/middlewares/my_middleware.py
...
2. Register in `packages/harness/tianshu/agents/lead_agent/agent.py`:
...
1. Update `packages/harness/tianshu/config/app_config.py` with new fields
...
Thank you for contributing to TianShu!
