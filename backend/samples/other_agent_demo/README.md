...
exactly **one** `from tianshu` line -- the ABC contract
(`from tianshu.agents.memory.manager import MemoryManager` in `deer_mem.py`).
Everything else is relative imports within the folder. So another agent can
adopt DeerMem in three steps, with **zero tian-shu code**.
...
`get_context` round-trip -- with **zero tian-shu dependency at runtime**.
