...
Thank you for your interest in contributing to TianShu! This guide will help you set up your development environment and understand our development workflow.
...
|---------|-----------|------------|-------|
| `make dev` on one machine | 4 vCPU, 8 GB RAM | 8 vCPU, 16 GB RAM | Best when TianShu uses hosted model APIs. |
...
`2 vCPU / 4 GB` environments often fail to start reliably or become unresponsive under normal TianShu workloads.
...
```text
unable to get image 'tian-shu-gateway': permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock
```
...
│   │   └── tianshu/       # Agents, tools, sandbox, MCP, skills, config
...
TianShu is an AI project and we welcome AI-assisted contributions. To help
...
# Live TianShuClient integration tests (explicit opt-in)
...
must also set `TIAN_SHU_RUN_LIVE_TESTS=1`.
...
`.tian-shu/support-bundles/`. The zip includes toolchain versions, sanitized
...
- Check existing [Issues](https://github.com/bytedance/tian-shu/issues)
- Read the [Documentation](backend/docs/)
- Ask questions in [Discussions](https://github.com/bytedance/tian-shu/discussions)

## License

By contributing to TianShu, you agree that your contributions will be licensed under the [MIT License](./LICENSE).
