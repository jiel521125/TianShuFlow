---
name: claude-to-tianshu
description: "Interact with TianShu AI agent platform via its HTTP API. Use this skill when the user wants to send messages or questions to TianShu for research/analysis, start a TianShu conversation thread, check TianShu status or health, list available models/skills/agents in TianShu, manage TianShu memory, upload files to TianShu threads, or delegate complex research tasks to TianShu. Also use when the user mentions tianshu, deer flow, or wants to run a deep research task that TianShu can handle."
---

# TianShu Skill

Communicate with a running TianShu instance via its HTTP API. TianShu is an AI agent platform
built on LangGraph that orchestrates sub-agents for research, code execution, web browsing, and more.

## Architecture

TianShu exposes two API surfaces behind an Nginx reverse proxy:

| Service        | Direct Port | Via Proxy                        | Purpose                          |
|----------------|-------------|----------------------------------|----------------------------------|
| Gateway API    | 8001        | `$TIANSHU_GATEWAY_URL`          | REST endpoints and embedded agent runtime |
| LangGraph-compatible API | 8001 | `$TIANSHU_LANGGRAPH_URL`       | Agent threads, runs, streaming   |
...
|-------------------------|------------------------------------------|------------------------------------|
| `TIANSHU_URL`          | `http://localhost:2026`                  | Unified proxy base URL             |
| `TIANSHU_GATEWAY_URL`  | `${TIANSHU_URL}`                        | Gateway API base (models, skills, memory, uploads) |
| `TIANSHU_LANGGRAPH_URL`| `${TIANSHU_URL}/api/langgraph`          | LangGraph API base (threads, runs) |

When making curl calls, always resolve the URL like this:

```bash
# Resolve base URLs from env (do this FIRST before any API call)
TIANSHU_URL="${TIANSHU_URL:-http://localhost:2026}"
TIANSHU_GATEWAY_URL="${TIANSHU_GATEWAY_URL:-$TIANSHU_URL}"
TIANSHU_LANGGRAPH_URL="${TIANSHU_LANGGRAPH_URL:-$TIANSHU_URL/api/langgraph}"
```
...
Verify TianShu is running:

```bash
curl -s "$TIANSHU_GATEWAY_URL/health"
```
...
```bash
curl -s -X POST "$TIANSHU_LANGGRAPH_URL/threads" \
...
```bash
curl -s -N -X POST "$TIANSHU_LANGGRAPH_URL/threads/<thread_id>/runs/stream" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -s "$TIANSHU_GATEWAY_URL/api/models"
```
...
```bash
curl -s "$TIANSHU_GATEWAY_URL/api/skills"
```
...
```bash
curl -s -X PUT "$TIANSHU_GATEWAY_URL/api/skills/<skill_name>" \
  -H "Content-Type: application/json" \
  -d '{"enabled": true}'
```
...
```bash
curl -s "$TIANSHU_GATEWAY_URL/api/agents"
```
...
```bash
curl -s "$TIANSHU_GATEWAY_URL/api/memory"
```
...
```bash
curl -s -X POST "$TIANSHU_GATEWAY_URL/api/threads/<thread_id>/uploads" \
...
```bash
curl -s "$TIANSHU_GATEWAY_URL/api/threads/<thread_id>/uploads/list"
```

### 11. Get Thread History

```bash
curl -s "$TIANSHU_LANGGRAPH_URL/threads/<thread_id>/history"
```

### 12. List Threads

```bash
curl -s -X POST "$TIANSHU_LANGGRAPH_URL/threads/search" \
  -H "Content-Type: application/json" \
...
```bash
bash /path/to/skills/claude-to-tianshu/scripts/chat.sh "Your question here"
```
...
- If health check fails, TianShu is not running. Inform the user they need to start it.
