#!/usr/bin/env bash
# chat.sh — Send a message to TianShu and collect the streaming response.
#
# Usage:
...
#   TIANSHU_URL=http://host:2026 bash chat.sh "hi"   # custom endpoint
#
# Environment variables:
#   TIANSHU_URL          — Unified proxy base URL (default: http://localhost:2026)
#   TIANSHU_GATEWAY_URL  — Gateway API base URL (default: $TIANSHU_URL)
#   TIANSHU_LANGGRAPH_URL — LangGraph API base URL (default: $TIANSHU_URL/api/langgraph)
#
...
GATEWAY_URL="${TIANSHU_GATEWAY_URL:-$TIANSHU_URL}"
LANGGRAPH_URL="${TIANSHU_LANGGRAPH_URL:-$TIANSHU_URL/api/langgraph}"
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: TianShu is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure TianShu is running. Start it with: cd <tianshu-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from TianShu: {data_str}", file=sys.stderr)
            sys.exit(1)
