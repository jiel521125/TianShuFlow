...
This document lists common issues encountered during TianShu smoke testing and how to resolve them.
...
3. Or stop TianShu services first:
   ```bash
   make stop
   ```
...
2. Check nginx logs:
   ```bash
   cd docker && docker compose -p tian-shu-dev -f docker-compose-dev.yaml logs nginx
   ```
...
```bash
docker exec -it tian-shu-gateway sh
```

#### Clean Up All TianShu-Related Containers and Images
```bash
make docker-stop
cd docker && docker compose -p tian-shu-dev -f docker-compose-dev.yaml down -v
```
...
1. Check the GitHub issues for the project: https://github.com/bytedance/tian-shu/issues
