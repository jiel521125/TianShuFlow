...
if [ -n "$port_2026_usage" ]; then
    ...
    while IFS= read -r pid; do
        ...
        case "$process_command" in
            *[Dd]eer[Ff]low*|*[Dd]eerflow*|*[Nn]ginx*tianshu*|*tianshu/*[Nn]ginx*)
                tianshu_process_found=1
                ;;
        esac
    done <<EOF
...
EOF
    ...
    if [ "$tianshu_process_found" -eq 1 ]; then
        echo "✓ Port 2026 is occupied by TianShu"
    else
        echo "✗ Port 2026 must be free before starting TianShu"
        exit 1
    fi
else
