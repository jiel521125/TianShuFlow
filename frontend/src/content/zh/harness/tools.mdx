...
<Callout type="info" emoji="🔧">
  工具是 Lead Agent 可以采取的行动。TianShu 提供内置工具、社区集成、MCP
...
安装：`cd backend && uv add 'tianshu-harness[tavily]'`
...
安装：`cd backend && uv add 'tianshu-harness[exa]'`
...
安装：`cd backend && uv add 'tianshu-harness[groundroute]'`
...
使用 Browserless `/screenshot` 渲染网页并截图，然后通过 TianShu 的 artifact
