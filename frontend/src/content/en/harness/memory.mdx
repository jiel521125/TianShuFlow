---
title: Memory
description: Memory is a runtime feature of the TianShu Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets TianShu carry useful information across sessions. The agent
...
Memory is a runtime feature of the TianShu Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
memory:
  ...
  backend_config:
    # Data root. Empty = tian-shu base_dir; per-user memory at
...
The backend hashes each TianShu user/agent scope into a distinct OpenViking
...
TianShu supports two levels of memory:
...
- Base directory: `backend/.tian-shu/`
- Global memory: `backend/.tian-shu/memory.json`
- Per-agent memory: `backend/.tian-shu/agents/{agent_name}/memory.json`
