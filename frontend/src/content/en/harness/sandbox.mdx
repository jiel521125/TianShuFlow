...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes TianShu capable of real action, not just
  conversation.
</Callout>
...
TianShu supports four sandbox modes. Choose the one that fits your deployment:
...
sandbox:
  ...
  # Optional: container name prefix (default: tian-shu-sandbox)
...
Install: `e2b-code-interpreter` is already a core dependency of `tianshu-harness`, so no extra install step is needed — just provide your API key and switch the provider in `config.yaml`.
...
<Callout type="info">
  Because e2b is a remote sandbox, TianShu's `/mnt/user-data` virtual prefix is
...
| `.tian-shu/threads/{thread_id}/user-data/` | `/mnt/user-data/`                            | Read-write |
...
provider runs inside the `tian-shu-gateway` container. In that mode,
...
If the configured `host_path` is not visible to the gateway process, TianShu
