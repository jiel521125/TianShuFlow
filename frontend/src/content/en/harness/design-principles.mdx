...
description: Understanding the design principles behind TianShu Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
---
...
<Callout type="info" emoji="🏗️">
  TianShu is built around one central idea: agent behavior should be composed
  from small, observable, replaceable pieces — not hardcoded into a fixed
  workflow graph.
</Callout>

Understanding the design principles behind TianShu Harness helps you use it effectively, extend it confidently, and reason about how your agents will behave in production.
...
TianShu is a harness because it bundles:
...
TianShu is designed for tasks that require more than a single prompt-response exchange. A useful long-horizon agent must:
...
Every architectural decision in TianShu is evaluated against this use case. Short, stateless exchanges are easy. Long, multi-step workflows under real-world pressure are the target.

## Middleware chain over inheritance

TianShu does not ask you to subclass an agent or override methods to change its behavior. Instead, it uses a **middleware chain** that wraps every LLM turn.
...
Skills also make the system extensible. Adding a new capability to TianShu means writing a new skill pack, not modifying the agent core.

## Sandbox is the execution environment

TianShu gives agents a **sandbox**: an isolated workspace where they can read files, write outputs, run commands, and produce artifacts.
...
Isolation is important because execution should be reproducible and controllable. The sandbox is the reason TianShu can support genuine action rather than only conversation.
...
TianShu addresses this through **context engineering** — deliberate control of what the agent sees, remembers, and ignores at each step:
...
All meaningful behaviors in TianShu are controlled through `config.yaml`. The system is designed so that operators can change how the agent behaves — which models to use, whether summarization is active, how subagents are limited, what tools are available — without touching code.
