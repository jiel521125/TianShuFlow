...
<Callout type="info" emoji="🔧">
  Tools are the actions the Lead Agent can take. TianShu provides built-in
...
TianShu organizes tools into four categories:
...
tools:
  ...
  - use: tianshu.sandbox.tools:bash_tool # requires host bash or container sandbox
```
...
Install: `cd backend && uv add 'tianshu-harness[tavily]'`
...
Install: `cd backend && uv add 'tianshu-harness[exa]'`
...
Install: `cd backend && uv add 'tianshu-harness[firecrawl]'`
...
Install: `cd backend && uv add 'tianshu-harness[groundroute]'`
...
tools:
  - name: web_fetch
    group: web
    use: tianshu.community.browserless.tools:web_fetch_tool
...
through TianShu's artifact system. This is useful for visual evidence, UI checks,
