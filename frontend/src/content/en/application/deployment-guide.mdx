...
description: "This guide covers all supported deployment methods for TianShu App: local development, Docker Compose, and production with Kubernetes-managed sandboxes."
---
...
This guide covers all supported deployment methods for TianShu App: local development, Docker Compose, and production with Kubernetes-managed sandboxes.

## Local development deployment

The local workflow is the fastest way to run TianShu. All services run as native processes on your machine.
...
```bash
# Set the absolute path to your tian-shu repo root
export TIAN_SHU_ROOT=/path/to/tian-shu
...
# .env
OPENAI_API_KEY=sk-...
TIAN_SHU_ROOT=/absolute/path/to/tian-shu
...
Thread data is stored in `backend/.tian-shu/threads/`. In Docker deployments, this directory is bind-mounted into the gateway container.

To avoid data loss when containers are recreated:

1. Set `TIAN_SHU_ROOT` to the absolute repo root path (or a stable host path).
...
In SQLite mode, TianShu stores LangGraph checkpoint data and application data in one `tianshu.db` file. WAL mode allows concurrent reads, but SQLite still permits only one writer at a time. When several users run agents at once, checkpoint writes and application writes can contend for the same file and raise `sqlite3.OperationalError: database is locked`.

For production or any multi-user deployment, use Postgres:

```bash
# .env
DATABASE_URL=postgresql://user:password@postgres:5432/tianshu
UV_EXTRAS=postgres
```
...
USERDATA_PVC_NAME=tian-shu-userdata-pvc
SKILLS_PVC_NAME=tian-shu-skills-pvc
```

When `USERDATA_PVC_NAME` is set, the provisioner automatically uses subPath (`tian-shu/users/{user_id}/threads/{thread_id}/user-data`) so each user's threads get their own directories in the PVC.
...
TianShu App uses [Better Auth](https://www.better-auth.com/) for session management. In production:
