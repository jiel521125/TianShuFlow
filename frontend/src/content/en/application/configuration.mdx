---
title: Configuration
description: TianShu App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
---
...
TianShu App is configured through two files and a set of environment variables. This page covers the application-level configuration that most operators need to set up before deploying.
...
Install Ollama provider: `cd backend && uv add 'tianshu-harness[ollama]'`
...
Install: `cd backend && uv add 'tianshu-harness[aio-sandbox]'`
...
tools:
  ...
  - use: tianshu.community.ddg_search.tools:web_search_tool # default, no key required
  ...
  - use: tianshu.sandbox.tools:str_replace_tool
  - use: tianshu.sandbox.tools:bash_tool
```

### Database backend

TianShu uses the `database` section for both LangGraph checkpoint data and application data such as runs, feedback, and thread metadata.

By default, TianShu uses SQLite for local, single-node persistence:

```yaml
database:
  backend: sqlite
  sqlite_dir: .tian-shu/data
```

SQLite mode stores everything in one `tianshu.db` file. This is fine for development or single-user deployments, but concurrent production traffic can hit SQLite's single-writer limit and raise `sqlite3.OperationalError: database is locked`.
...
Set `DATABASE_URL` in your environment, for example `postgresql://user:password@localhost:5432/tianshu`.
...
memory:
  ...
  backend_config:                  # DeerMem-private knobs (the backend self-interprets these)
    storage_path: ""               # empty = tian-shu base_dir (per-user memory under it)
    debounce_seconds: 30
    max_facts: 100
