...
description: This page covers day-to-day operational tasks and solutions to common problems when running TianShu App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running TianShu App.
...
When you pull a new version of TianShu, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
This usually means SQLite is handling concurrent writes from multiple users or long-running agent runs. SQLite supports concurrent reads, but only one writer can hold the database lock at a time. In TianShu, checkpoint writes and application writes share the same SQLite database file.
...
- Check that the storage path is writable: `ls -la backend/.tian-shu/`.
...
- `backend/.tian-shu/threads/` for thread working directories, uploads, and artifacts.
- `backend/.tian-shu/memory.json` and `backend/.tian-shu/agents/` for learned memory.
- `${database.sqlite_dir}/tianshu.db` when using `database.backend: sqlite` (default: `.tian-shu/data/tianshu.db`).
