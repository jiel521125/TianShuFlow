...
export function warnUnsupportedStreamModes(
  ...
  warn(
    `[tian-shu] Rejected unsupported LangGraph stream mode(s): ${unseenModes.join(", ")}`,
  );
}
...
export function sanitizeRunStreamOptions<T>(options: T): T {
  ...
  if ("streamResumable" in options) {
    ...
    if (!warnedUnsupportedStreamResumable) {
      ...
      console.warn(
        "[tian-shu] Dropped unsupported LangGraph run option: streamResumable",
      );
    }
  }
  ...
  if (droppedModes.length > 0) {
    ...
    throw new Error(
      `[tian-shu] Unsupported LangGraph stream mode(s): ${droppedModes.join(", ")}`,
    );
  }
