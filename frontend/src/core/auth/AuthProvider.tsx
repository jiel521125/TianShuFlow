...
export function AuthProvider({ children, initialUser }: AuthProviderProps) {
   ...
   *
   * Cancels any in-flight refresh when called again so stale responses
   * cannot clobber the latest state. Component-unmount also aborts the
   * request — this avoids the noisy `net::ERR_ABORTED` errors in the
   * console when the user navigates away mid-fetch.
   */
  const refreshUser = useCallback(async () => {
    if (staticMode) return;

    // Cancel any previous in-flight refresh so it can't overwrite us.
    ...
    try {
      ...
      // Network errors (TypeError: Failed to fetch) and explicit aborts
      // are expected when the browser cancels in-flight requests during
      // navigation, when the tab is quickly switched, or when the next
      // refresh aborts the previous one. These are not real auth failures
      // — don't clear the session or log them as errors.
  ...
  /** Holds the AbortController for any in-flight refreshUser() call. */
  const refreshAbortRef = React.useRef<AbortController | null>(null);

  // Abort any pending refresh when the provider unmounts so the browser
  // doesn't log `net::ERR_ABORTED` after navigation.
  useEffect(() => {
    return () => {
      refreshAbortRef.current?.abort();
    };
