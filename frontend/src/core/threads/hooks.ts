...
import {
  ...
  workspaceFolderBindingOfThread,
} from "./utils";
...
const SUMMARIZATION_MIDDLEWARE_UPDATE_KEYS = new Set([
  ...
  "TianShuSummarizationMiddleware.before_model",
]);
...
export function useThreadStream({
  ...
  const sendMessage = useCallback(
    async (
      ...
      setOptimisticThreadId(threadId ?? null);
      setLiveMessagesThreadId(threadId ?? null);
      ...
      try {
        ...
        await thread.submit(
          ...
          {
            // `threadId` here is the SDK's `submitOptions.threadId`, not the
            // hook's own state. When the hook is mid-creation (its `threadId`
            // state is undefined), the SDK calls `client.threads.create({threadId})`
            // to materialise the row on the backend before the run starts, so
            // the subsequent `/threads/<id>/runs/stream` request (which the
            // gateway guards with `require_existing=True`) succeeds.
  ...
  >({
    ...
    queryFn: async ({ pageParam, signal }) => {
      ...
      try {
        ...
        // React Query aborts in-flight fetches when the consumer unmounts or
        // the query is invalidated. Swallow AbortError so the unmount shows
        // up as `net::ERR_ABORTED` in the network panel without surfacing as
        // a toast/banner — there is nothing the user can do about it.
        if (
          error instanceof DOMException &&
          (error.name === "AbortError" || error.code === DOMException.ABORT_ERR)
        ) {
          return { data: [], has_more: false, next_before_seq: null };
        }
        throw error;
      }
    },
...
const INFINITE_THREADS_NEXT_PAGE_PARAM = Symbol(
  "tianshu.infiniteThreads.nextPageParam",
);
...
function threadMetadataQueryKey(threadId?: string | null): [string, string, string] {
  return ["thread", "metadata", threadId ?? ""];
}

/**
 * Read + mutate the thread's workspace-folder binding. The binding lives in
 * thread metadata (`tianshu_workspace_*` keys) so no backend schema change is
 * required. Names are denormalized into metadata at bind time for cheap
 * sidebar rendering; they are refreshed whenever the user re-binds.
 */
export function useThreadWorkspaceBinding(threadId?: string | null) {
  const queryClient = useQueryClient();
  const metadataQuery = useThreadMetadata(threadId);
  const binding = metadataQuery.data
    ? workspaceFolderBindingOfThread(metadataQuery.data)
    : null;
  ...
  const bind = useMutation({
    mutationFn: async (next: WorkspaceFolderBinding) => {
      if (!threadId) {
        throw new Error("Thread id is required to bind a workspace folder.");
      }
      await patchThreadMetadata(threadId, {
        [THREAD_WORKSPACE_ID_KEY]: next.workspaceId,
        [THREAD_WORKSPACE_FOLDER_ID_KEY]: next.folderId,
        [THREAD_WORKSPACE_NAME_KEY]: next.workspaceName,
        [THREAD_WORKSPACE_FOLDER_NAME_KEY]: next.folderName,
      });
    },
  ...
  const unbind = useMutation({
    mutationFn: async () => {
      if (!threadId) {
        throw new Error("Thread id is required to unbind a workspace folder.");
      }
      await patchThreadMetadata(threadId, {
        [THREAD_WORKSPACE_ID_KEY]: null,
        [THREAD_WORKSPACE_FOLDER_ID_KEY]: null,
        [THREAD_WORKSPACE_NAME_KEY]: null,
        [THREAD_WORKSPACE_FOLDER_NAME_KEY]: null,
      });
    },
    onSuccess: invalidate,
  });

  return { binding, metadataQuery, bind, unbind };
}
