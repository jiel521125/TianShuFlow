...
export function WorkspaceChannelsList() {
  ...
  return (
    <SidebarGroup className="pt-0">
      <SidebarGroupLabel className="cursor-pointer select-none flex items-center justify-between hover:text-sidebar-foreground transition-colors" onClick={() => setCollapsed(!collapsed)}>
        <span>{t.sidebar.channels}</span>
        <ChevronDownIcon className={cn("size-3 transition-transform duration-200", collapsed && "-rotate-90")} />
      </SidebarGroupLabel>
      {!collapsed && (
        <SidebarMenu>
