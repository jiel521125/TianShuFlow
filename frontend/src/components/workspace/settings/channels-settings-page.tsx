...
export function ChannelsSettingsPage() {
  ...
  // Per-user preference layer ("千人千面"): inherit_global=true follows
  // the admin-managed global provider list; turning it off activates the
  // per-user enabled_channels list below.
  const {
    settings,
  ...
  const inheritGlobal = settings?.channels.inherit_global ?? true;
  const enabledChannels = settings?.channels.enabled_channels ?? [];
  const prefsSaving = updatePrefs.isPending || resetPrefs.isPending;

  const prefsItems = visibleProviders.map((provider) => ({
    id: provider.provider,
    label: provider.display_name,
  }));

  const handleInheritChange = (inherit: boolean) => {
    // When the user stops inheriting the global config, default the
    // per-user list to the currently visible providers so nothing
      ...
      : {
          ...
          enabled_channels:
            enabledChannels.length > 0
              ? enabledChannels
              : visibleProviders.map((provider) => provider.provider),
        };
    updatePrefs.mutate(
      { section: "channels", value },
      {
        onError: (err) =>
          toast.error(
  ...
  const handleToggle = (id: string, checked: boolean) => {
    const next = checked
      ? [...enabledChannels, id]
      : enabledChannels.filter((item) => item !== id);
    updatePrefs.mutate(
      { section: "channels", value: { enabled_channels: next } },
      {
        onError: (err) =>
          toast.error(
            err instanceof Error ? err.message : t.settings.userPrefs.saveFailed,
  ...
  // The per-user preference card only makes sense when there are visible
  // providers to toggle; hide it otherwise so it never sits in the middle
  // of an empty section.
  const showPrefsCard =
    !prefsError && !providersLoading && !providersError && visibleProviders.length > 0;
