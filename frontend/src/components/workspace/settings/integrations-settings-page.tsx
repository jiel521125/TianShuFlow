...
export function IntegrationsSettingsPage() {
  ...
  // Per-user preference layer ("千人千面") on top of the admin-managed
  ...
  const inheritGlobal = settings?.integrations.inherit_global ?? true;
  const enabledIntegrations = settings?.integrations.enabled_integrations ?? [];
  const prefsSaving = updatePrefs.isPending || resetPrefs.isPending;
  // Currently supported integrations (kept in sync with the backend
  // integrations router, which exposes the Lark/Feishu integration).
  const prefsItems = [{ id: "lark", label: t.settings.integrations.lark.title }];
      ...
      : {
          ...
          enabled_integrations:
            enabledIntegrations.length > 0
              ? enabledIntegrations
              : prefsItems.map((item) => item.id),
        };
    updatePrefs.mutate(
      { section: "integrations", value },
      {
        onError: (err) =>
          toast.error(
  ...
  const handleToggle = (id: string, checked: boolean) => {
    const next = checked
      ...
      : enabledIntegrations.filter((item) => item !== id);
    updatePrefs.mutate(
      { section: "integrations", value: { enabled_integrations: next } },
      {
        onError: (err) =>
          toast.error(
            err instanceof Error ? err.message : t.settings.userPrefs.saveFailed,
  ...
  const handleReset = () => {
    resetPrefs.mutate("integrations", {
