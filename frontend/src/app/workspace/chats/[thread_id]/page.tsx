...
import { archiveArtifactsToFolder } from "@/components/workspace/thread-workspace-documents";
...
import {
  textOfMessage,
  THREAD_WORKSPACE_FOLDER_ID_KEY,
  THREAD_WORKSPACE_FOLDER_NAME_KEY,
  THREAD_WORKSPACE_ID_KEY,
  THREAD_WORKSPACE_NAME_KEY,
...
export default function ChatPage() {
  ...
  // While an auxiliary panel (git log / workflow result) is open above the
  // composer, drop the welcome-mode absolute layout so the panel pushes the
  // content instead of growing upward into the top header.
  const [auxPanelActive, setAuxPanelActive] = useState(false);
  // Folder binding staged from the welcome-mode toolbar while the thread does
  // not exist yet; applied to the real thread in `onStart`.
  const [pendingWorkspaceBinding, setPendingWorkspaceBinding] =
    useState<WorkspaceFolderBinding | null>(null);
  // Real binding persisted in thread metadata; pendingWorkspaceBinding covers
  // the first message before the onStart PATCH lands.
  const { binding: threadBinding } = useThreadWorkspaceBinding(threadId);
  const { data: boundFolderFiles } = useFiles(
    threadBinding?.workspaceId ?? null,
    threadBinding?.folderId ?? null,
  );
  // Keys of artifacts already archived into the bound folder this session, so
  // a finished run never re-archives (and re-fetches) the same files.
  const archivedArtifactKeysRef = useRef<Set<string>>(new Set());
  ...
  const {
    ...
    onStart: (createdThreadId) => {
      ...
      const pending = pendingWorkspaceBinding;
      if (pending) {
        setPendingWorkspaceBinding(null);
        // The thread now exists on the backend; persist the folder binding
        // that was staged while this was still a welcome-mode placeholder.
        void patchThreadMetadata(createdThreadId, {
          [THREAD_WORKSPACE_ID_KEY]: pending.workspaceId,
          [THREAD_WORKSPACE_FOLDER_ID_KEY]: pending.folderId,
          [THREAD_WORKSPACE_NAME_KEY]: pending.workspaceName,
          [THREAD_WORKSPACE_FOLDER_NAME_KEY]: pending.folderName,
        }).catch((error) => {
          toast.error(
            error instanceof Error
              ? error.message
              : t.userWorkspace.updateFailed,
          );
        });
      }
    },
    onFinish: (state) => {
      ...
      // Auto-archive artifacts produced by this turn into the bound folder.
      const boundFolder = threadBinding ?? pendingWorkspaceBinding;
      if (boundFolder && !isMock && threadId) {
        const artifactPaths = extractArtifactsFromThread(thread);
        const pending = artifactPaths.filter((filepath) => {
          const name = filepath.split("/").pop() ?? filepath;
          const key = `${boundFolder.workspaceId}:${boundFolder.folderId}:${name}`;
          if (archivedArtifactKeysRef.current.has(key)) {
            return false;
          }
          archivedArtifactKeysRef.current.add(key);
          return true;
        });
        if (pending.length > 0) {
          void archiveArtifactsToFolder({
            threadId,
            artifactPaths: pending,
            binding: boundFolder,
            existingNames: boundFolderFiles?.map((file) => file.name) ?? [],
            isMock,
          }).then((archivedNames) => {
            if (archivedNames.length > 0) {
              toast.success(t.threadWorkspace.archivedCount(archivedNames.length));
            }
          });
        }
      }
  ...
  const handleSubmit = useCallback(
    (message: PromptInputMessage, options?: InputBoxSubmitOptions) => {
      // While `isNewThread` is true, `threadId` is the client-side optimistic
      // UUID from `useThreadChat()`. The gateway gates the runs/stream
      // endpoint with `require_existing=True`, so passing it directly would
      // 404 and bounce the user back to /workspace/chats/new. Pass null so
      // the SDK routes to the stateless `/runs/stream` endpoint and lets the
      // backend create the thread.
      const targetThreadId = isNewThread ? null : threadId;
      const sendPromise = sendMessage(targetThreadId, message, undefined, options);
                ...
                >
                  ...
                  {mountedRef.current ? (
                    <InputBox
                      ...
                      onPendingWorkspaceBinding={setPendingWorkspaceBinding}
                      onAuxPanelChange={setAuxPanelActive}
                      threadId={threadId}
