...
test.describe("Localized documentation links", () => {
  test("keeps English card navigation in the English docs", async ({
    ...
    const card = page.locator("a.nextra-card", { hasText: "Why TianShu" });
    await expect(card).toHaveAttribute(
      "href",
      "/en/docs/introduction/why-tianshu",
    );

    await card.click();
    await expect(page).toHaveURL(/\/en\/docs\/introduction\/why-tianshu$/);
    await expect(page.locator("main h1")).toContainText("Why TianShu");
  });
  ...
  test("uses valid repository links for documentation feedback and edits", async ({
    ...
    ).toHaveAttribute("href", /github\.com\/bytedance\/tian-shu\/issues\/new/);
    await expect(
      page.getByRole("link", { name: "Edit this page" }),
    ).toHaveAttribute(
      "href",
      "https://github.com/bytedance/tian-shu/tree/main/frontend/src/content/en/application/quick-start.mdx",
    );
  });
});
