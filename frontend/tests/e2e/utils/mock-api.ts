...
export function mockLangGraphAPI(page: Page, options?: MockAPIOptions) {
  ...
  let larkIntegrationStatus = {
    ...
    install_path: "/tmp/tian-shu/integrations/skills/lark-cli",
    cli: {
      available: false,
  ...
  void page.route(/\/api\/threads\/[^/]+\/branches$/, (route) => {
    if (route.request().method() === "POST") {
      ...
      if (sourceThread?.metadata?.tianshu_branch === true) {
 ...
 * The stream returns a single AI message: "Hello from TianShu!".
 */
