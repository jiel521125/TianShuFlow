...
test("aboutMarkdown heading reflects the package version when env is unset", async () => {
  ...
  // empty or undefined APP_VERSION interpolation (`About TianShu ]` /
  // `About TianShu undefined]`), not just removal of the old literal.
  expect(aboutMarkdown).toContain(`[About TianShu ${APP_VERSION}]`);
});
