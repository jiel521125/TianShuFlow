...
const ENV_KEYS = [
  "NODE_ENV",
  "TIAN_SHU_INTERNAL_GATEWAY_BASE_URL",
  "TIAN_SHU_TRUSTED_ORIGINS",
] as const;
...
describe("getGatewayConfig", () => {
  ...
  beforeEach(() => {
    saved = snapshotEnv();
    setEnv("TIAN_SHU_INTERNAL_GATEWAY_BASE_URL", undefined);
    setEnv("TIAN_SHU_TRUSTED_ORIGINS", undefined);
  });
  ...
  test("uses env values verbatim when set, regardless of NODE_ENV", async () => {
    setEnv("NODE_ENV", "production");
    setEnv("TIAN_SHU_INTERNAL_GATEWAY_BASE_URL", "https://gw.example.com/");
    setEnv(
      "TIAN_SHU_TRUSTED_ORIGINS",
