...
describe("remember login helpers", () => {
  ...
  test("saves only email and preference when enabled", () => {
    ...
    expect(storage.values.get("tianshu.auth.remember_login")).toBe("1");
    expect(storage.values.get("tianshu.auth.remembered_email")).toBe(
      "admin@example.com",
    );
  ...
  test("clears saved email when disabled", () => {
    const storage = makeStorage({
      "tianshu.auth.remember_login": "1",
      "tianshu.auth.remembered_email": "admin@example.com",
    });
    rs.stubGlobal("localStorage", storage);
    ...
    expect(storage.values.get("tianshu.auth.remember_login")).toBe("0");
    expect(storage.values.has("tianshu.auth.remembered_email")).toBe(false);
  });
