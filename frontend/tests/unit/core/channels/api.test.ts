  ...
  test("starts a provider connection flow", async () => {
    mockedFetch.mockResolvedValueOnce(
      jsonResponse(200, {
        provider: "telegram",
        mode: "deep_link",
        url: "https://t.me/tianshu_bot?start=state",
        code: "state",
        instruction: "Send /start state to the TianShu Telegram bot.",
        expires_in: 600,
      }),
    );
  ...
  test("starts a binding-code connection flow", async () => {
    mockedFetch.mockResolvedValueOnce(
      jsonResponse(200, {
        ...
        url: null,
        code: "abc123",
        instruction: "Send /connect abc123 to the TianShu Slack bot.",
        expires_in: 600,
      }),
    );
